...
def save_rule_settings_endpoint(agency_code, file, errors, warnings):
    json_response = settings_handler.save_rule_settings(
        agency_code=agency_code, file=file, errors=errors, warnings=warnings
    )
    assert json_response.status_code == 200
    return json.loads(json_response.get_data().decode("UTF-8"))
...
def test_agency_has_settings(database):
    ...
    rule_settings = []
    for rule in (
        sess.query(RuleSql).filter(RuleSql.file_id == FILE_TYPE_DICT_LETTER_ID["A"]).order_by(RuleSql.rule_sql_id).all()
    ):
...
def test_list_rule_settings(database):
    ...
    rule_a1 = {"significance": 1, "label": "A1", "description": "A1 Description", "impact": "high"}
    rule_a3 = {"significance": 2, "label": "A3", "description": "A3 Description", "impact": "high"}
    rule_a2 = {"significance": 1, "label": "A2", "description": "A2 Description", "impact": "high"}
    rule_a4 = {"significance": 2, "label": "A4", "description": "A4 Description", "impact": "high"}
    rule_b1 = {"significance": 1, "label": "B1", "description": "B1 Description", "impact": "high"}
    rule_b3 = {"significance": 2, "label": "B3", "description": "B3 Description", "impact": "high"}
    rule_b2 = {"significance": 1, "label": "B2", "description": "B2 Description", "impact": "high"}
    rule_b4 = {"significance": 2, "label": "B4", "description": "B4 Description", "impact": "high"}
    # Normal results
...
def test_save_rule_settings(database):
    ...
    def get_rule_settings_results(agency_code, file, rule_type):
        query = (
            sess.query(RuleSetting.impact_id, RuleSql.rule_label)
            .join(
                RuleSql,
                and_(
            ...
            .filter(
                RuleSql.file_id == FILE_TYPE_DICT_LETTER_ID[file],
    ...
    # Normal results
    errors = [{"label": "A4", "impact": "low"}, {"label": "A2", "impact": "high"}]
    warnings = [{"label": "A3", "impact": "high"}, {"label": "A1", "impact": "medium"}]
    ...
    errors = [{"label": "A2", "impact": "medium"}, {"label": "A4", "impact": "low"}]
    warnings = [{"label": "A1", "impact": "low"}, {"label": "A3", "impact": "high"}]
    save_rule_settings_endpoint(agency_code="1125", file="A", errors=errors, warnings=warnings)
    ...
    errors = [{"label": "C2", "impact": "medium"}, {"label": "C1", "impact": "low"}]
    warnings = []
    ...
    errors = [{"label": "A4", "impact": "low"}, {"label": "A2", "impact": "high"}]
    warnings = [{"label": "A3", "impact": "high"}, {"label": "A1"}]
    ...
    errors = [{"label": "A4", "impact": "low"}, {"label": "A2", "impact": "high"}]
    warnings = [{"label": "A3", "impact": "high"}, {"impact": "low"}]
    expected_error_text = "Rules list provided doesn't match the rules expected: A3"
    with pytest.raises(ResponseError) as resp_except:
        save_rule_settings_endpoint(agency_code="1125", file="A", errors=errors, warnings=warnings)
    assert str(resp_except.value).startswith(expected_error_text)
    ...
    errors = [{"label": "A4", "impact": "3"}, {"label": "A2", "impact": "high"}]
    warnings = [{"label": "A3", "impact": "high"}, {"label": "A1", "impact": "medium"}]
