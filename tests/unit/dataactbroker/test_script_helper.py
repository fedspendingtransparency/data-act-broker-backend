...
def test_validate_load_dates(database, monkeypatch):
    ...
    start_date, end_date = script_helper.validate_load_dates(
        start_date, end_date, auto, load_type, arg_date_format, output_date_format
    )
    ...
    sess.add(
        ExternalDataLoadDate(external_data_type_id=EXTERNAL_DATA_TYPE_DICT[load_type], last_load_date_start=loader_date)
    )
    sess.commit()
    start_date, end_date = script_helper.validate_load_dates(
        start_date, end_date, auto, load_type, arg_date_format, output_date_format
