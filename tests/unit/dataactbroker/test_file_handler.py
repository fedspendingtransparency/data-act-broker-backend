...
from tests.unit.dataactcore.factories.job import (
    JobFactory,
    SubmissionFactory,
    PublishHistoryFactory,
    CommentFactory,
    CertifyHistoryFactory,
    PublishedFilesHistoryFactory,
)
...
def test_create_submission_already_pub_mon(database, monkeypatch):
    ...
    pub_mon1_sub = SubmissionFactory(
        user_id=1,
        number_of_warnings=1,
        cgac_code=cgac.cgac_code,
        reporting_fiscal_period=4,
        reporting_fiscal_year=2010,
        publish_status_id=2,
        is_quarter_format=False,
    )
    pub_mon2_sub = SubmissionFactory(
        user_id=1,
        number_of_warnings=1,
        cgac_code=cgac.cgac_code,
    ...
    request_params = {
        ...
        "_files": {
            "award_financial": UploadFile(AWARD_FILE_T[1]),
...
def test_create_submission_external_files(database, monkeypatch):
    ...
    external_files = sess.query(Job).filter(
        Submission.submission_id == new_same_sub.submission_id, Job.file_type_id.in_(ex_types)
    )
...
def test_list_submissions_sort_success(database, monkeypatch):
    ...
    sub1 = SubmissionFactory(
        ...
        submission_id=1,
        number_of_warnings=1,
        reporting_start_date=date(2010, 1, 1),
        publish_status_id=1,
        is_quarter_format=False,
    )
    sub2 = SubmissionFactory(
        ...
        submission_id=2,
        number_of_warnings=1,
        reporting_start_date=date(2010, 1, 2),
        publish_status_id=1,
        is_quarter_format=False,
    )
    sub3 = SubmissionFactory(
        ...
        submission_id=3,
        number_of_warnings=1,
        reporting_start_date=date(2010, 1, 3),
        publish_status_id=1,
        is_quarter_format=False,
    )
    sub4 = SubmissionFactory(
        ...
        submission_id=4,
        number_of_warnings=1,
        reporting_start_date=date(2010, 1, 4),
        publish_status_id=1,
        is_quarter_format=True,
    )
    sub5 = SubmissionFactory(
        user_id=2,
        submission_id=5,
        number_of_warnings=1,
        reporting_start_date=date(2010, 1, 5),
        publish_status_id=1,
        is_quarter_format=True,
    )
...
def test_list_submissions_success(database, monkeypatch):
    ...
    sub = SubmissionFactory(
        user_id=1, submission_id=1, publish_status_id=2, is_fabs=True, reporting_start_date=None, certified=False
    )
    job = JobFactory(
        submission=sub,
        job_status_id=JOB_STATUS_DICT["finished"],
    ...
    user = UserFactory(user_id=1)
    sub = SubmissionFactory(
        user_id=1, submission_id=1, publish_status_id=2, is_fabs=True, reporting_start_date=None, certified=True
    )
    job = JobFactory(
        submission=sub,
        job_status_id=JOB_STATUS_DICT["finished"],
    ...
    user = UserFactory(user_id=1)
    sub = SubmissionFactory(
        user_id=1, submission_id=1, publish_status_id=3, is_fabs=True, reporting_start_date=None, certified=True
    )
    job = JobFactory(
        submission=sub,
        job_status_id=JOB_STATUS_DICT["finished"],
...
def test_list_submissions_failure(database, monkeypatch):
    ...
    job = JobFactory(
        submission=sub,
        job_status_id=JOB_STATUS_DICT["failed"],
        job_type_id=JOB_TYPE_DICT["csv_record_validation"],
        file_type_id=FILE_TYPE_DICT["award"],
    )
    add_models(database, [user, sub, job])
    ...
    job = JobFactory(
        submission=sub,
        job_status_id=JOB_STATUS_DICT["invalid"],
        job_type_id=JOB_TYPE_DICT["csv_record_validation"],
        file_type_id=FILE_TYPE_DICT["award"],
    )
    add_models(database, [user, sub, job])
...
def test_comments(database):
    ...
    # Write some comments
    result = fileHandler.update_submission_comments(
        sub1, {"B": "BBBBBB", "E": "EEEEEE", "FABS": "This wont show up"}, CONFIG_BROKER["local"]
    )
    ...
    result = fileHandler.update_submission_comments(
        sub1, {"A": "AAAAAA", "E": "E2E2E2", "submission_comment": "SubC"}, CONFIG_BROKER["local"]
    )
    assert result.status_code == 200
    ...
    assert (
        str(resp_except.value) == "Submission must not be publishing, certifying, or reverting when updating "
        "comments"
    )
...
def test_get_comments_file(database):
    ...
    # Write some comments
    fileHandler.update_submission_comments(
        sub1, {"B": "BBBBBB", "E": "EEEEEE", "submission_comment": "SubC"}, CONFIG_BROKER["local"]
    )
...
def test_get_submission_zip(database):
    ...
    pub_dabs_sub = SubmissionFactory(
    ...
    test_files = {"file_a.txt": "A", "file_c.txt": "C", "file_f.txt": "F"}
    test_warning_files = {"A": "test_warning_a.txt", "C": "test_warning_c.txt"}
    for file_name, file_content in test_files.items():
        ...
        models.append(
            PublishedFilesHistoryFactory(
                publish_history=pub_1, submission=pub_dabs_sub, file_type_id=file_type_id, certify_history_id=None
            )
        )
        models.append(
            PublishedFilesHistoryFactory(
                publish_history=pub_2,
                submission=pub_dabs_sub,
                file_type_id=file_type_id,
                filename=file_name,
                warning_filename=warning_filename,
                certify_history=cert,
            )
        )
    ...
    expected_zip_name = "Broker_SubID-{}_PubID-{}_{}".format(
        pub_dabs_sub.submission_id, pub_2.publish_history_id, filename_fyp_sub_format(pub_dabs_sub)
    )
    ...
    assert resp.status_code == 200
    resp = json.loads(resp.get_data().decode("UTF-8"))
    expected_zip_name = "Broker_SubID-{}_CertID-{}_{}".format(
        pub_dabs_sub.submission_id, cert.certify_history_id, filename_fyp_sub_format(pub_dabs_sub)
    )
    assert expected_zip_name in resp["url"]
...
def test_build_file_map_string(monkeypatch):
    ...
    submission = SubmissionFactory(
        submission_id=3, reporting_fiscal_year="2022", reporting_fiscal_period="2", is_quarter_format=False
    )
    ...
    for file in upload_files:
        ...
        assert file.upload_name == "3/SubID-3_File-{}{}_{}_123456789{}".format(
            file.file_letter, fyp, split_filename[0], split_filename[1]
        )
...
def test_build_file_map_file(monkeypatch):
    ...
    file_dict = {
        "fabs": fabs_file,
        "award_financial": award_file,
        "program_activity": pa_file,
...
def test_move_published_files(database, monkeypatch):
    ...
    # Quarter Jobs
    appropriations_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/appropriations/file_a.csv",
        file_type_id=FILE_TYPE_DICT["appropriations"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    val_appropriations_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/appropriations/file_a.csv",
        file_type_id=FILE_TYPE_DICT["appropriations"],
        job_type_id=val_job,
    ...
    prog_act_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/prog/act/file_b.csv",
        file_type_id=FILE_TYPE_DICT["program_activity"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    val_prog_act_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/prog/act/file_b.csv",
        file_type_id=FILE_TYPE_DICT["program_activity"],
        job_type_id=val_job,
    ...
    award_fin_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/award/fin/file_c.csv",
        file_type_id=FILE_TYPE_DICT["award_financial"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    val_award_fin_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/award/fin/file_c.csv",
        file_type_id=FILE_TYPE_DICT["award_financial"],
        job_type_id=val_job,
    ...
    award_proc_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/award/proc/file_d1.csv",
        file_type_id=FILE_TYPE_DICT["award_procurement"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    award_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/award/file_d2.csv",
        file_type_id=FILE_TYPE_DICT["award"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    cross_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/award/cross_file.csv",
        file_type_id=FILE_TYPE_DICT["award_financial"],
        job_type_id=cross_job,
        job_status_id=finished_job,
    )
    exec_comp_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/exec/comp/file_e.csv",
        file_type_id=FILE_TYPE_DICT["executive_compensation"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    sub_award_job_qtr = JobFactory(
        submission=qtr_sub,
        filename="/path/to/sub/award/file_f.csv",
        file_type_id=FILE_TYPE_DICT["sub_award"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )

    award_fin_narr_qtr = CommentFactory(
        submission=qtr_sub, comment="Test comment", file_type_id=FILE_TYPE_DICT["award_financial"]
    )

    # Monthly jobs
    appropriations_job_mon = JobFactory(
        submission=mon_sub,
        filename="/path/to/appropriations/file_a.csv",
        file_type_id=FILE_TYPE_DICT["appropriations"],
        job_type_id=upload_job,
    ...
    val_appropriations_job_mon = JobFactory(
        submission=mon_sub,
        filename="/path/to/appropriations/file_a.csv",
        file_type_id=FILE_TYPE_DICT["appropriations"],
        job_type_id=val_job,
    ...
    val_prog_act_job_mon = JobFactory(
        submission=mon_sub,
        filename="/path/to/prog/act/file_b.csv",
        file_type_id=FILE_TYPE_DICT["program_activity"],
        job_type_id=val_job,
    ...
    val_award_fin_job_mon = JobFactory(
        submission=mon_sub,
        filename="/path/to/award/fin/file_c.csv",
        file_type_id=FILE_TYPE_DICT["award_financial"],
        job_type_id=val_job,
    ...
    fabs_job = JobFactory(
        submission=fabs_sub,
        filename="/path/to/appropriations/fabs-test.csv",
        file_type_id=FILE_TYPE_DICT["fabs"],
        job_type_id=upload_job,
        job_status_id=finished_job,
    )
    val_fabs_job = JobFactory(
        submission=fabs_sub,
        filename="/path/to/appropriations/fabs-test.csv",
        file_type_id=FILE_TYPE_DICT["fabs"],
        job_type_id=val_job,
    ...
    database.session.add_all(
        [
            ...
            pub_hist_remote_qtr,
            cert_hist_local,
            cert_hist_remote_qtr,
            appropriations_job_qtr,
            val_appropriations_job_qtr,
            prog_act_job_qtr,
            val_prog_act_job_qtr,
            award_fin_job_qtr,
            val_award_fin_job_qtr,
            award_proc_job_qtr,
            award_job_qtr,
            cross_job_qtr,
            exec_comp_job_qtr,
            ...
            award_fin_narr_qtr,
            pub_hist_remote_mon,
            cert_hist_remote_mon,
            appropriations_job_mon,
            val_appropriations_job_mon,
            prog_act_job_mon,
            val_prog_act_job_mon,
            award_fin_job_mon,
            val_award_fin_job_mon,
            award_proc_job_mon,
            award_job_mon,
            cross_job_mon,
            exec_comp_job_mon,
            sub_award_job_mon,
            pub_hist_remote_fabs,
            cert_hist_remote_fabs,
            fabs_job,
            val_fabs_job,
        ]
    )
    ...
    monkeypatch.setattr(
        fileHandler,
        "CONFIG_BROKER",
        {"aws_bucket": "original_bucket", "certified_bucket": "cert_bucket", "broker_files": "broker_files"},
    )
    ...
    c_cert_hist = (
        ...
        .filter_by(publish_history_id=local_id, file_type_id=FILE_TYPE_DICT["award_financial"])
        .one()
    )
    assert c_cert_hist.filename == "/path/to/award/fin/file_c.csv"
    expected_filename = "/path/to/error/reports/SubID-{}_File-C-warning-report_FY17Q2.csv".format(qtr_sub.submission_id)
    assert c_cert_hist.warning_filename == expected_filename
    assert c_cert_hist.comment == "Test comment"

    # cross-file warnings
    warning_cert_hist = (
        sess.query(PublishedFilesHistory).filter_by(publish_history_id=local_id, file_type=None, filename=None).all()
    )
    ...
    assert (
        "/path/to/error/reports/SubID-{}_File-A-to-B-crossfile-warning-report_FY17Q2.csv".format(qtr_sub.submission_id)
        in warning_cert_hist_files
    )
    ...
    c_cert_hist = (
        sess.query(PublishedFilesHistory)
        .filter_by(publish_history_id=remote_id, file_type_id=FILE_TYPE_DICT["award_financial"])
        .one()
    )
    assert c_cert_hist.filename == "zyxwv/2017/Q2/{}/file_c.csv".format(remote_id)
    assert c_cert_hist.warning_filename == "zyxwv/2017/Q2/{}/SubID-{}_File-C-warning-report_FY17Q2.csv".format(
        remote_id, qtr_sub.submission_id
    )
    ...
    assert c_cert_hist.warning_filename == "zyxwv/2017/P02/{}/SubID-{}_File-C-warning-report_FY17P01-P02.csv".format(
        remote_id, mon_sub.submission_id
    )
    ...
    c_cert_hist = (
        sess.query(PublishedFilesHistory)
        .filter_by(publish_history_id=remote_id, file_type_id=FILE_TYPE_DICT["fabs"])
        .one()
    )
    assert c_cert_hist.filename == "FABS/zyxwv/2022/01/SubID-{}_Published-FABS_123456789.csv".format(
        fabs_sub.submission_id
    )
...
def test_list_history(database):
    ...
    file_hist_1 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
    ...
    file_hist_2 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
    ...
    file_hist_3 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
        filename=None,
...
def test_list_history_order(database):
    ...
    file_hist_4 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_hist.publish_history_id,
        submission_id=sub_id,
    ...
    file_hist_5 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_hist.publish_history_id,
        submission_id=sub_id,
    ...
    file_hist_6 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
    ...
    file_hist_7 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
    ...
    file_hist_8 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
        filename=None,
        warning_filename="submission_1_cross_warning_appropriations_" "program_activity.csv",
        file_type_id=None,
    )
    file_hist_9 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
        filename=None,
        warning_filename="submission_1_cross_warning_program_activity_" "award_financial.csv",
        file_type_id=None,
    )
    file_hist_10 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
        filename=None,
        warning_filename="submission_1_cross_warning_award_financial_" "award_procurement.csv",
        file_type_id=None,
    )
    file_hist_11 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
        filename=None,
        warning_filename="submission_1_cross_warning_award_financial_" "award.csv",
        file_type_id=None,
    )
    file_hist_12 = PublishedFilesHistoryFactory(
        certify_history_id=cert_history_id,
        publish_history_id=pub_history_id,
        submission_id=sub_id,
...
def test_file_history_url(database, monkeypatch):
    ...
    file_hist = PublishedFilesHistoryFactory(
        certify_history_id=cert_hist.certify_history_id,
        publish_history_id=pub_hist.publish_history_id,
...
def test_get_status_dabs(database):
    ...
    job_6_up = JobFactory(
        ...
        original_filename="test_file_e.csv",
    )
    # Ready
    job_7_up = JobFactory(
        updated_at=now,
...
def test_process_job_status():
    ...
    job_1 = {
        "job_type": JOB_TYPE_DICT["file_upload"],
        "job_status": JOB_STATUS_DICT["waiting"],
        "error_message": "",
        "progress": 23,
        "file_name": "test_file.csv",
        "updated_at": str(now),
    ...
    job_2 = {
        "job_type": JOB_TYPE_DICT["csv_record_validation"],
        "job_status": JOB_STATUS_DICT["ready"],
        "error_message": "",
        "progress": 42.5,
        "file_name": "test_file.csv",
        "updated_at": str(now),
