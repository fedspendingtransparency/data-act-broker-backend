...
def test_submission_report_url_local(monkeypatch, tmpdir):
    ...
    assert url == os.path.join(file_path, 'submission4_crossfile_warning_File_C_to_D2_award_financial_award.csv')
...
def test_submission_report_url_s3(monkeypatch):
    ...
    assert s3_url_handler.return_value.get_signed_url.call_args == (
        ('errors', 'submission2_some_file_error_report.csv'),
...
def test_move_certified_files(database, monkeypatch):
    ...
    expected_filename = "/path/to/error/reports/submission{}_File_C_award_financial_warning_report.csv".\
    ...
    print(warning_cert_hist_files)
    assert "/path/to/error/reports/submission{}_crossfile_warning_File_A_to_B_appropriations_program_activity.csv".\
    ...
    assert c_cert_hist.warning_filename == "zyxwv/2017/2/{}/submission{}_File_C_award_financial_warning_report.csv". \
