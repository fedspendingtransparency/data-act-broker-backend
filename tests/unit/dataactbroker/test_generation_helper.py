...
def test_retrieve_cached_file_generation(database):
    ...
    file_generation = retrieve_cached_file_generation(job, "awarding", "123", "csv", False)
...
def test_retrieve_cached_file_generation_agency_type_diff(database):
    ...
    file_generation = retrieve_cached_file_generation(job, "funding", "123", "csv", False)
    assert file_generation is None
...
def test_copy_file_generation_to_job_attached(monkeypatch, database):
    ...
    submission = SubmissionFactory(
        submission_id=1, reporting_fiscal_year="2022", reporting_fiscal_period="4", is_quarter_format=False
    )
