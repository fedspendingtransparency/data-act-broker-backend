...
def test_list_agencies_limits(domain_app, database):
    ...
    user.affiliations = [
        UserAffiliation(cgac=cgac, frec=None, permission_type_id=PERMISSION_SHORT_DICT["w"]),
...
def test_list_agencies_perm_params(domain_app, database):
    ...
    f_cgac = CGACFactory()
    user.affiliations = [
        UserAffiliation(cgac=r_cgac, frec=None, permission_type_id=PERMISSION_SHORT_DICT["r"]),
    ...
    def call_list_agencies(perm_level="reader", perm_type="mixed"):
        result = domain_app.get(
            "/v1/list_agencies/?perm_level={}&perm_type={}".format(perm_level, perm_type)
        ).data.decode("UTF-8")
...
def test_list_sub_tier_agencies(domain_app, database):
    ...
    user.affiliations = [
        UserAffiliation(cgac=cgacs[0], frec=None, permission_type_id=PERMISSION_SHORT_DICT["f"]),
