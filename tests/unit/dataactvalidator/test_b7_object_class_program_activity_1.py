...
def test_success(database):
    ...
    op = ObjectClassProgramActivityFactory(
        gross_outlays_delivered_or_fyb=1, ussgl490800_authority_outl_fyb=1, prior_year_adjustment="x"
    )
    # Different values, Different PYA
    op2 = ObjectClassProgramActivityFactory(
        gross_outlays_delivered_or_fyb=0, ussgl490800_authority_outl_fyb=1, prior_year_adjustment="A"
    )
...
def test_failure(database):
    ...
    op = ObjectClassProgramActivityFactory(
        gross_outlays_delivered_or_fyb=1, ussgl490800_authority_outl_fyb=0, prior_year_adjustment="X"
    )
