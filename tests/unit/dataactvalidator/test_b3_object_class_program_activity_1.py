...
def test_success(database):
    ...
    ocpa = ObjectClassProgramActivityFactory(
        obligations_undelivered_or_fyb=value, ussgl480100_undelivered_or_fyb=value, prior_year_adjustment="X"
    )
    ocpa_null = ObjectClassProgramActivityFactory(
        obligations_undelivered_or_fyb=0, ussgl480100_undelivered_or_fyb=None, prior_year_adjustment="x"
    )
    # Different values, different PYA
    ocpa_pya = ObjectClassProgramActivityFactory(
        obligations_undelivered_or_fyb=1, ussgl480100_undelivered_or_fyb=2, prior_year_adjustment="A"
    )
...
def test_failure(database):
    ...
    ocpa = ObjectClassProgramActivityFactory(
        obligations_undelivered_or_fyb=Decimal("101.23"),
