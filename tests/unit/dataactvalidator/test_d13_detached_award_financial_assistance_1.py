...
_FILE = 'd13_detached_award_financial_assistance_1'


def test_column_headers(database):
    expected_subset = {"row_number", "record_type", "legal_entity_zip5"}
    actual = set(query_columns(_FILE, database))
    assert expected_subset == actual


def test_success(database):
    """ Test LegalEntityZIP5 must be blank for aggregate records (i.e., when RecordType = 1) record type 2 doesn't
        affect success """
