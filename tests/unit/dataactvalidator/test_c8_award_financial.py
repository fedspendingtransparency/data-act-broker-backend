...
def test_invalid_allocation_transfer_agency(database):
    """ Tests that validation processes when there's an invalid allocation transfer agency in File C (award financial).
        Per rule C24. """
    tas = _TAS
...
def test_valid_allocation_transfer_agency(database):
    """ Tests that rule is not applied when File C (award financial) record has a valid allocation transfer agency. """
    tas = _TAS
    cgac = CGACFactory(cgac_code='good')
    # Perform when there's a transaction obligated amount value in the field
    ...
    af_2 = AwardFinancialFactory(tas=tas, fain='abc', uri='xyz', allocation_transfer_agency='bad',
                                 agency_identifier='bad', transaction_obligated_amou=None)
