...
def test_success(database):
    ...
    (BusinessTypes includes 'P'). For AssistanceType F003, F004, F005, F006, F007, F008, F009, or F010, if the base
    award (the earliest record with the same unique award key) has an ActionDate prior to October 1, 2022, this will
    produce a warning rather than a fatal error.
    """
    # Note: for FABS 31.2.2, we're setting assistance types to F003, F004, F005, F006, F007, F008, F009, or F010 and
    #       having the base actiondate be less than October 1, 2032. This rule will not trigger if those *do* apply.
...
def test_failure(database):
    ...
    is an aggregate or PII-redacted non-aggregate record (RecordType = 1 or 3) or the recipient is an individual
    (BusinessTypes includes 'P'). For AssistanceType F003, F004, F005, F006, F007, F008, F009, or F010, if the base
    award (the earliest record with the same unique award key) has an ActionDate prior to October 1, 2032, this will
    produce a warning rather than a fatal error.
    """
    # Note: for FABS 31.2.2, we're setting assistance types to F003, F004, F005, F006, F007, F008, F009, or F010 and
    #       having the base actiondate be less than October 1, 2032. This rule will not trigger if those *do* apply.
