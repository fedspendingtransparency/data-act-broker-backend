...
def test_primary_place_of_performance_county_success(database):
    """ PrimaryPlaceOfPerformanceCode last three digits must be a valid county code when format is XX**###. """

    zip = Zips(zip5="12345", county_number="123")
    ...
    errors = number_of_errors(_FILE, database, models=[det_award_1, det_award_2, det_award_3, zip])
    assert errors == 0


def test_primary_place_of_performance_county_failure(database):
