...
def test_failure(database):
    ...
    caf_fain = CertifiedAwardFinancialFactory(submission_id=sub_1.submission_id, tas='test_tas', fain='abcd', uri=None,
                                              piid=None, parent_award_id=None, disaster_emergency_fund_code='N',
                                              gross_outlay_amount_by_awa_cpe=5)
    database.session.add_all([sub_1, caf_fain])
    database.session.commit()
    ...
    errors = number_of_errors(_FILE, database, models=[af_other], submission=sub_3)
    assert errors == 1
    ...
    errors = number_of_errors(_FILE, database, models=[af_null], submission=sub_4)
    assert errors == 1
