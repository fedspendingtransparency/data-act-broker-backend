...
def test_success(database):
    ...
    first_parent_award_id_row_one = AwardFinancialFactory(transaction_obligated_amou=1100,
                                                          parent_award_id=parent_award_id,
                                                          allocation_transfer_agency=None)
    first_parent_award_id_row_two = AwardFinancialFactory(transaction_obligated_amou=11,
                                                          parent_award_id=parent_award_id,
                                                          allocation_transfer_agency=None)
    # And add a row for a different parent_award_id
    second_parent_award_id_row_one = AwardFinancialFactory(transaction_obligated_amou=9999,
                                                           parent_award_id=parent_award_id_two,
                                                           allocation_transfer_agency=None)
    third_parent_award_id_row_one = AwardFinancialFactory(transaction_obligated_amou=8888,
    ...
    other_parent_award_id_ap_row = AwardProcurementFactory(parent_award_id=parent_award_id_two,
                                                           federal_action_obligation=-9999)
    third_parent_award_id_ap_row = AwardProcurementFactory(parent_award_id=parent_award_id_three,
                                                           federal_action_obligation=-9999)
