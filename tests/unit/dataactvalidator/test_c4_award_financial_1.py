...
def test_column_headers(database):
    expected_subset = {'row_number', 'obligations_delivered_orde_cpe', 'ussgl490100_delivered_orde_cpe',
                       ...
                       'uniqueid_DEFC', 'uniqueid_PIID', 'uniqueid_FAIN', 'uniqueid_URI'}
    actual = set(query_columns(_FILE, database))
    assert (actual & expected_subset) == expected_subset


def test_success(database):
    """ ObligationsDeliveredOrdersUnpaidTotal in File C = USSGL 4901 + 4931 + 4981 in File C for the same date context
        and TAS/DEFC combination (CPE)
    """
    ...
    af = AwardFinancialFactory(obligations_delivered_orde_cpe=3, ussgl490100_delivered_orde_cpe=1,
