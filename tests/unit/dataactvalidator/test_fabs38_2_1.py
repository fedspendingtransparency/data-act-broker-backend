...
def test_success(database):
    ...
    errors = number_of_errors(
        _FILE, database, models=[office_1, office_2, fabs_1, fabs_2, fabs_3, fabs_4, fabs_5, fabs_6]
    )
    assert errors == 0
...
def test_failure_inactive_office(database):
    ...
    fabs_1 = FABSFactory(
        funding_office_code="12345a", correction_delete_indicatr="", action_date=date.today() - relativedelta(days=1)
    )
    # Action date too late
    fabs_2 = FABSFactory(
        funding_office_code="12345a", correction_delete_indicatr="", action_date=date.today() + relativedelta(months=1)
    )
