...
def test_success(database):
    ...
    # Matching ATA
    ap1 = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency="abc", adjustments_to_unobligated_cpe=15
    )
    # Blank ATA
    ap2 = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency="", adjustments_to_unobligated_cpe=15
    )
    ap3 = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency=None, adjustments_to_unobligated_cpe=15
    )
    # Non-matching ATA with all monetary values 0 (with one None for good measure)
    ap4 = AppropriationFactory(
    ...
    ap = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency="123", adjustments_to_unobligated_cpe=15
    )
    ...
    # Matching ATA for a FREC
    ap = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency="021", adjustments_to_unobligated_cpe=15
    )
...
def test_failure(database):
    ...
    ap = AppropriationFactory(
        submission_id=sub.submission_id, allocation_transfer_agency="Not a match", adjustments_to_unobligated_cpe=1
    )
    ...
    ap = AppropriationFactory(
        submission_id=dod_sub.submission_id, allocation_transfer_agency="Not a match", adjustments_to_unobligated_cpe=1
    )
