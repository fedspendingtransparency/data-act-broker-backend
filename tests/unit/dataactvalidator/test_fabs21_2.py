...
def test_column_headers(database):
    expected_subset = {'row_number', 'funding_sub_tier_agency_co', 'funding_office_code', 'action_date',
...
def test_failure_inactive_office(database):
    ...
    fabs_1 = FABSFactory(funding_sub_tier_agency_co=agency.sub_tier_agency_code, funding_office_code=office.office_code,
                         correction_delete_indicatr='', action_date=date.today() - relativedelta(days=1))
    # Action date is after the end date
    fabs_2 = FABSFactory(funding_sub_tier_agency_co=agency.sub_tier_agency_code, funding_office_code=office.office_code,
                         correction_delete_indicatr='', action_date=date.today() + relativedelta(months=1))
