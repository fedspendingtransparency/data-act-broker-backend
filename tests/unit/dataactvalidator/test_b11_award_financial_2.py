...
def test_success(database):
    ...
    af = AwardFinancialFactory(object_class='123', by_direct_reimbursable_fun='X')
    assert number_of_errors(_FILE, database, models=[af]) == 0

    af = AwardFinancialFactory(object_class='1234', by_direct_reimbursable_fun='D')
    assert number_of_errors(_FILE, database, models=[af]) == 0

    af = AwardFinancialFactory(object_class='2234', by_direct_reimbursable_fun='r')
    assert number_of_errors(_FILE, database, models=[af]) == 0
...
def test_failure(database):
    ...
    af = AwardFinancialFactory(object_class='1234', by_direct_reimbursable_fun='x')
    assert number_of_errors(_FILE, database, models=[af]) == 1

    af = AwardFinancialFactory(object_class='2234', by_direct_reimbursable_fun='Y')
    assert number_of_errors(_FILE, database, models=[af]) == 1