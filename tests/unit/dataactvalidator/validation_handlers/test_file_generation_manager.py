...
from tests.unit.dataactcore.factories.job import (
    FileGenerationFactory,
    JobFactory,
    PublishStatusFactory,
    SubmissionFactory,
...
from tests.unit.dataactcore.factories.domain import (
    ...
    CGACFactory,
    FRECFactory,
    TASFailedEditsFactory,
    GTASBOCFactory,
)
from tests.unit.dataactcore.factories.staging import (
    AwardFinancialAssistanceFactory,
    AwardProcurementFactory,
    DetachedAwardProcurementFactory,
    PublishedFABSFactory,
...
def test_generate_a_gtas_status(database, monkeypatch):
    ...
    fail_1 = TASFailedEditsFactory(
        period=6, fiscal_year=year, tas=tas_1_str, severity="Fatal", approved_override_exists=False
    )
    fail_2 = TASFailedEditsFactory(
        period=6,
        fiscal_year=year,
        tas=tas_2_str,
        severity="Fatal",
        approved_override_exists=True,
        atb_submission_status="F",
    )
    fail_3 = TASFailedEditsFactory(
        period=6,
        fiscal_year=year,
        tas=tas_3_str,
        severity="Fatal",
        approved_override_exists=True,
        atb_submission_status="E",
    )
    fail_4 = TASFailedEditsFactory(
        period=6,
        fiscal_year=year,
        tas=tas_4_str,
        severity="Fatal",
        approved_override_exists=True,
        atb_submission_status="P",
    )
    fail_5 = TASFailedEditsFactory(
        period=6,
        fiscal_year=year,
        tas=tas_5_str,
        severity="Fatal",
        approved_override_exists=True,
        atb_submission_status="C",
    )
    fail_6 = TASFailedEditsFactory(
        period=6, fiscal_year=year, tas=tas_6_str, severity="Fatal", approved_override_exists=True
    )
    # Submission with no fail data available
    sub_window_no_fail = SubmissionWindowScheduleFactory(
        period=5, year=2021, period_start=datetime.now().date() - timedelta(days=2)
    )
    # Submission window that has started (GTAS window closed)
    sub_window_over = SubmissionWindowScheduleFactory(
        period=6, year=2021, period_start=datetime.now().date() - timedelta(days=2)
    )
    # Submission window that hasn't started (GTAS window still open)
    sub_window_progress = SubmissionWindowScheduleFactory(
        period=7, year=2021, period_start=datetime.now().date() + timedelta(days=2)
    )
...
def test_generate_boc(database, monkeypatch):
    ...
    boc3 = GTASBOCFactory(
        ...
        budget_object_class="1110",
        reimbursable_flag="D",
        prior_year_adjustment_code=None,
        **tas1_dict,
    )
    boc4 = GTASBOCFactory(
        period=6,
    ...
    boc10 = GTASBOCFactory(
        ...
        debit_credit="D",
        disaster_emergency_fund_code="Q",
        budget_object_class="999",
        reimbursable_flag="D",
        prior_year_adjustment_code="X",
        **tas1_dict,
    )
    # USSGL of 101000 should be ignored
    boc11 = GTASBOCFactory(
        period=6,
        fiscal_year=year,
        display_tas=tas1_str,
        dollar_amount=1.5,
    ...
    boc12 = GTASBOCFactory(
        period=6,
        fiscal_year=year,
        display_tas=tas5_str,
        dollar_amount=1.5,
        ussgl_number="480100",
        begin_end="B",
        debit_credit="D",
    ...
    pub_b1 = PublishedObjectClassProgramActivityFactory(
    ...
    pub_b2 = PublishedObjectClassProgramActivityFactory(
        submission_id=sub_id,
        display_tas=tas1_str,
        disaster_emergency_fund_code="Q",
        object_class="111",
    ...
    pub_b3 = PublishedObjectClassProgramActivityFactory(
        submission_id=sub_id,
        display_tas=tas2_str,
        disaster_emergency_fund_code="Q",
        object_class="111",
    ...
    pub_b4 = PublishedObjectClassProgramActivityFactory(
        ...
        row_number=1,
        prior_year_adjustment="X",
        begin_end_indicator="B",
        **tas1_dict,
    )
    # Financing account, should be ignored
    pub_b5 = PublishedObjectClassProgramActivityFactory(
        submission_id=sub_id,
        display_tas=tas5_str,
        disaster_emergency_fund_code="Q",
        object_class="111",
    ...
    sub = SubmissionFactory(
        ...
        cgac_code=agency_cgac,
    )
    job = JobFactory(
        job_status_id=JOB_STATUS_DICT["running"],
        job_type_id=JOB_TYPE_DICT["file_upload"],
        file_type_id=FILE_TYPE_DICT["boc_comparison"],
        filename=None,
    ...
    assert job.filename == os.path.join(
        CONFIG_BROKER["broker_files"], f"GTAS-Comparison-Report_FY17P06_CGAC-{agency_cgac}_123456789.csv"
    )
    ...
    expected2 = (
        [tas2_str]
        + list(tas2_dict.values())
        + [boc5.budget_object_class, "D", "Q", "", boc5.begin_end, str(year), "6", boc5.ussgl_number, "25", "0", "25"]
    )

    # TAS 3
    expected3 = (
        [tas3_str]
        + list(tas3_dict.values())
        + [boc7.budget_object_class, "D", "Q", "X", boc7.begin_end, str(year), "6", boc7.ussgl_number, "4", "0", "4"]
    )

    # TAS 4
    expected4 = (
        [tas4_str]
        + list(tas4_dict.values())
        + [
            boc9.budget_object_class,
            "D",
            "Q",
            "X",
            boc9.begin_end,
    ...
    expected9 = (
        [tas5_str]
        + list(tas5_dict.values())
        + [
            boc12.budget_object_class,
            "D",
            "Q",
            "X",
            boc12.begin_end,
...
def test_generate_sub_d1(database, monkeypatch):
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "SubID-4_File-D1_FY22P04_20170101_20170131_awarding_123456789.csv"
    )
...
def test_generate_sub_d2(database, monkeypatch):
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "SubID-4_File-D2_FY22P04_20170101_20170131_funding_123456789.txt"
    )
...
def test_generate_awarding_d1(database, monkeypatch):
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "File-D1_20170101_20170131_awarding_123456789.csv"
    )
...
def test_generate_funding_d1(database, monkeypatch):
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "File-D1_20170101_20170131_funding_123456789.csv"
    )
...
def test_generate_awarding_d2(database, monkeypatch):
    ...
    pub_fabs_1 = PublishedFABSFactory(
        awarding_agency_code="123", action_date="20170101", afa_generated_unique="unique1", is_active=True
    )
    pub_fabs_2 = PublishedFABSFactory(
        awarding_agency_code="123", action_date="20170131", afa_generated_unique="unique2", is_active=True
    )
    pub_fabs_3 = PublishedFABSFactory(
        awarding_agency_code="123", action_date="20161231", afa_generated_unique="unique3", is_active=True
    )
    pub_fabs_4 = PublishedFABSFactory(
        awarding_agency_code="123", action_date="20170201", afa_generated_unique="unique4", is_active=True
    )
    pub_fabs_5 = PublishedFABSFactory(
        awarding_agency_code="123", action_date="20170115", afa_generated_unique="unique5", is_active=False
    )
    pub_fabs_6 = PublishedFABSFactory(
        awarding_agency_code="234", action_date="20170115", afa_generated_unique="unique6", is_active=True
    )
    file_gen = FileGenerationFactory(
        request_date=datetime.now().date(),
        start_date="01/01/2017",
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "File-D2_20170101_20170131_awarding_123456789.csv"
    )
...
def test_generate_funding_d2(database, monkeypatch):
    ...
    pub_fabs_1 = PublishedFABSFactory(
        funding_agency_code="123", action_date="20170101", afa_generated_unique="unique1", is_active=True
    )
    pub_fabs_2 = PublishedFABSFactory(
        funding_agency_code="123", action_date="20170131", afa_generated_unique="unique2", is_active=True
    )
    pub_fabs_3 = PublishedFABSFactory(
        funding_agency_code="123", action_date="20161231", afa_generated_unique="unique3", is_active=True
    )
    pub_fabs_4 = PublishedFABSFactory(
        funding_agency_code="123", action_date="20170201", afa_generated_unique="unique4", is_active=True
    )
    pub_fabs_5 = PublishedFABSFactory(
        funding_agency_code="123", action_date="20170115", afa_generated_unique="unique5", is_active=False
    )
    pub_fabs_6 = PublishedFABSFactory(
        funding_agency_code="234", action_date="20170115", afa_generated_unique="unique6", is_active=True
    )
    file_gen = FileGenerationFactory(
        request_date=datetime.now().date(),
        start_date="01/01/2017",
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "File-D2_20170101_20170131_funding_123456789.csv"
    )
...
def test_generate_txt_d1(database, monkeypatch):
    ...
    assert file_gen.file_path == os.path.join(
        CONFIG_BROKER["broker_files"], "File-D1_20170101_20170131_awarding_123456789.txt"
    )
...
def test_generate_e_file(mock_broker_config_paths, database):
    ...
    same_uei = AwardProcurementFactory(
        submission_id=sub.submission_id, awardee_or_recipient_uei=model.awardee_or_recipient_uei
    )
    ...
    expected = [
        [
            ...
            uei.ultimate_parent_uei,
            uei.ultimate_parent_legal_enti,
            uei.high_comp_officer1_full_na,
            uei.high_comp_officer1_amount,
            uei.high_comp_officer2_full_na,
            uei.high_comp_officer2_amount,
            uei.high_comp_officer3_full_na,
            uei.high_comp_officer3_amount,
            uei.high_comp_officer4_full_na,
            uei.high_comp_officer4_amount,
            uei.high_comp_officer5_full_na,
