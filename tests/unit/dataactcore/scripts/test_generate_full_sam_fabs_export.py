...
def test_cumulative_sum(database):
    ...
    sess = database.session
    pf1 = PublishedFABSFactory(
        action_date=date(2000, 3, 22),
        is_active=True,
        fain="seconddate",
        assistance_type="07",
        original_loan_subsidy_cost=52,
        federal_action_obligation=20,
        unique_award_key="abc",
    )
    pf2 = PublishedFABSFactory(
        action_date=date(2000, 3, 22),
        is_active=True,
        fain="seconddate",
        assistance_type="07",
        original_loan_subsidy_cost=8,
        federal_action_obligation=20,
        unique_award_key="abc",
    )
    pf3 = PublishedFABSFactory(
        action_date=date(2001, 1, 1),
        is_active=True,
        fain="thirddate",
        assistance_type="02",
        original_loan_subsidy_cost=9.5,
        federal_action_obligation=11,
        unique_award_key="abc",
    )
    pf4 = PublishedFABSFactory(
        action_date=date(2000, 1, 30),
        is_active=True,
        fain="firstdate",
        assistance_type="06",
        original_loan_subsidy_cost=13,
        federal_action_obligation=5,
        unique_award_key="abc",
    )
    pf5 = PublishedFABSFactory(
        action_date=date(2000, 1, 30),
        is_active=True,
        fain="otherkey",
        assistance_type="08",
        original_loan_subsidy_cost=5,
        federal_action_obligation=9,
        unique_award_key="def",
    )
