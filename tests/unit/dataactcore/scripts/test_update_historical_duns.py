...
def test_update_sam_props(monkeypatch):
    ...
    recp_df = pd.DataFrame.from_dict({"awardee_or_recipient_uniqu": ["000000001", "000000002", "000000003"]})
    ...
    assert expected_df.sort_index(inplace=True) == update_historical_duns.update_existing_recipients(
        recp_df
...
def test_update_sam_props_empty(monkeypatch):
    ...
    monkeypatch.setattr("dataactcore.utils.sam_recipient.get_sam_props", mock_get_sam_props)
    recp_df = pd.DataFrame.from_dict({"awardee_or_recipient_uniqu": ["000000003"]})
