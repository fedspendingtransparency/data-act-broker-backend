...
def reference_data(sess):
    reference_data_dict = {
        "sub_recipient": SAMRecipientFactory(
            uei="987654321CCC", legal_business_name="TEST SUB RECIPIENT", awardee_or_recipient_uniqu="987654321"
        ),
        "sub_parent_recipient": SAMRecipientFactory(
            uei="987654322PPP", legal_business_name="TEST SUB PARENT RECIPIENT", awardee_or_recipient_uniqu="987654322"
        ),
        "prime_recipient": SAMRecipientFactory(
            uei="123456789CCC",
            legal_business_name="TEST PRIME RECIPIENT",
...
def compare_contract_results(
    ...
    updated_at,
    sub_recipient=None,
    sub_parent_recipient=None,
    prime_recipient=None,
    debug=True,
):
    ...
    attr = {
        ...
        "legal_entity_zip": (
            fpds_latest.legal_entity_zip4 if fpds_latest.legal_entity_country_code.upper() == "USA" else None
        ),
        ...
        "sub_legal_entity_state_code": (
            sub_contract.legal_entity_state_code if sub_contract.legal_entity_state_code.upper() != "ZZ" else None
        ),
        "sub_legal_entity_state_name": (
            sub_contract.legal_entity_state_name if sub_contract.legal_entity_state_code.upper() != "ZZ" else None
        ),
        "sub_legal_entity_zip": (
            sub_contract.legal_entity_zip_code if sub_contract.legal_entity_country_code.upper() == "USA" else None
        ),
        ...
        "sub_business_types": (
            ",".join(sub_contract.business_types_names) if sub_contract.business_types_names else None
        ),
        "sub_place_of_perform_city_name": sub_contract.ppop_city_name,
        "sub_place_of_perform_state_code": (
            sub_contract.ppop_state_code if sub_contract.ppop_state_code.upper() != "ZZ" else None
        ),
        "sub_place_of_perform_state_name": (
            sub_contract.ppop_state_name if sub_contract.ppop_state_code.upper() != "ZZ" else None
        ),
...
def test_generate_f_file_queries_contracts(database):
    ...
    ref_data_cont["prime_ppop"] = Location(
        country=ref_data_cont["dom_country"], zip=ref_data_cont["dom_zip5"], county=ref_data_cont["dom_county_zip5"]
    )
    ref_data_cont["prime_le"] = Location(country=ref_data_cont["int_country"], zip=None, county=None)
    ref_data_cont["sub_ppop"] = Location(
        country=ref_data_cont["dom_country"], zip=ref_data_cont["dom_zip9"], county=ref_data_cont["dom_county_zip9"]
    )
    ...
    fpds_latest = DetachedAwardProcurementFactory(
        ...
        legal_entity_county_code=(
            ref_data_cont["prime_le"].county.county_number if ref_data_cont["prime_le"].county else None
        ),
        place_of_perform_country_c=ref_data_cont["prime_ppop"].country.country_code,
        place_of_performance_zip4a=(ref_data_cont["prime_ppop"].zip.zip5 if ref_data_cont["prime_ppop"].zip else None),
        place_of_perform_county_co=(
            ref_data_cont["prime_ppop"].county.county_number if ref_data_cont["prime_ppop"].county else None
        ),
    )
    sub_contract = SAMSubcontractFactory(
        ...
        ppop_zip_code=(ref_data_cont["sub_ppop"].zip.zip5 if ref_data_cont["sub_ppop"].zip else None),
    ...
    drop_ref_fields = [
        ...
        "sub_ppop",
        "sub_le",
        "assistance_listing_1",
        "assistance_listing_2",
    ]
    for drop_ref_field in drop_ref_fields:
        ref_data_cont.pop(drop_ref_field)

    assert (
        compare_contract_results(
            contracts_results[0], fpds_base, fpds_latest, sub_contract, created_at, updated_at, **ref_data_cont
        )
        is True
    )


def compare_grant_results(
    sub,
    fabs_base,
    fabs_latest,
    fabs_grouped,
    ...
    attr = {
        ...
        "legal_entity_foreign_posta": (
            fabs_latest.legal_entity_foreign_posta if fabs_latest.legal_entity_country_code != "USA" else None
        ),
        ...
        "sub_legal_entity_state_code": (
            sub_grant.legal_entity_state_code if sub_grant.legal_entity_state_code.upper() != "ZZ" else None
        ),
        "sub_legal_entity_state_name": (
            sub_grant.legal_entity_state_name if sub_grant.legal_entity_state_code.upper() != "ZZ" else None
        ),
        "sub_legal_entity_zip": (
            sub_grant.legal_entity_zip_code if sub_grant.legal_entity_country_code.upper() == "USA" else None
        ),
        "sub_legal_entity_county_code": None,
        "sub_legal_entity_county_name": None,
        ...
        "sub_place_of_perform_state_code": (
            sub_grant.ppop_state_code if sub_grant.ppop_state_code.upper() != "ZZ" else None
        ),
        "sub_place_of_perform_state_name": (
            sub_grant.ppop_state_name if sub_grant.ppop_state_code.upper() != "ZZ" else None
        ),
...
def test_generate_f_file_queries_assistance(database):
    ...
    # Setting the test location data - can be varied
    ref_data_assist["prime_ppop"] = Location(
        country=ref_data_assist["dom_country"],
        zip=ref_data_assist["dom_zip5"],
    ...
    ref_data_assist["sub_ppop"] = Location(
        country=ref_data_assist["dom_country"],
        zip=ref_data_assist["dom_zip9"],
    ...
    fabs_latest = PublishedFABSFactory(
        ...
        legal_entity_county_code=(
            ref_data_assist["prime_le"].county.county_number if ref_data_assist["prime_le"].county else None
        ),
        place_of_perform_country_c=ref_data_assist["prime_ppop"].country.country_code,
        place_of_performance_zip4a=(
            ref_data_assist["prime_ppop"].zip.zip5 if ref_data_assist["prime_ppop"].zip else None
        ),
        place_of_perform_county_co=(
            ref_data_assist["prime_ppop"].county.county_number if ref_data_assist["prime_ppop"].county else None
        ),
    )
    sub_grant = SAMSubgrantFactory(
        ...
        ppop_zip_code=(ref_data_assist["sub_ppop"].zip.zip5 if ref_data_assist["sub_ppop"].zip else None),
    )
    ...
    for fabs in all_fabs:
        if fabs.unique_award_key not in fabs_grouped.keys():
            fabs_grouped[fabs.unique_award_key] = {
                "award_amount": fabs.federal_action_obligation,
                "assistance_listing_num": fabs.assistance_listing_number,
                "assistance_listing_title": ref_data_assist["assistance_listing_1"].program_title,
            }
        else:
    ...
    assert (
        compare_grant_results(
            grants_results[0],
            fabs_base,
            fabs_latest,
            fabs_grouped[fabs_latest.unique_award_key],
            sub_grant,
            created_at,
            updated_at,
            **ref_data_assist,
...
def test_fix_broken_links(database):
    ...
    # Setting the test location data - can be varied
    ref_data["prime_ppop"] = Location(
        country=ref_data["dom_country"], zip=ref_data["dom_zip5"], county=ref_data["dom_county_zip5"]
    )
    ref_data["prime_le"] = Location(country=ref_data["int_country"], zip=None, county=None)
    ref_data["sub_ppop"] = Location(
        country=ref_data["dom_country"], zip=ref_data["dom_zip9"], county=ref_data["dom_county_zip9"]
    )
    ...
    fabs_latest = PublishedFABSFactory(
        ...
        legal_entity_county_code=(ref_data["prime_le"].county.county_number if ref_data["prime_le"].county else None),
        place_of_perform_country_c=ref_data["prime_ppop"].country.country_code,
        place_of_performance_zip4a=(ref_data["prime_ppop"].zip.zip5 if ref_data["prime_ppop"].zip else None),
        place_of_perform_county_co=(
            ref_data["prime_ppop"].county.county_number if ref_data["prime_ppop"].county else None
        ),
    )
    sub_grant = SAMSubgrantFactory(
        unique_award_key=fabs_base.unique_award_key,
        ...
        ppop_zip_code=(ref_data["sub_ppop"].zip.zip5 if ref_data["sub_ppop"].zip else None),
    )
    ...
    for fabs in all_fabs:
        if fabs.unique_award_key not in fabs_grouped.keys():
            fabs_grouped[fabs.unique_award_key] = {
                "award_amount": fabs.federal_action_obligation,
                "assistance_listing_num": fabs.assistance_listing_number,
                "assistance_listing_title": ref_data["assistance_listing_1"].program_title,
            }
        else:
            fabs_grouped[fabs.unique_award_key]["award_amount"] += fabs.federal_action_obligation
            fabs_grouped[fabs.unique_award_key]["assistance_listing_num"] += ", " + fabs.assistance_listing_number
    ...
    contracts_results = (
        sess.query(Subaward).order_by(Subaward.id).filter(Subaward.subaward_type == "sub-contract").all()
    )
    grants_results = sess.query(Subaward).order_by(Subaward.id).filter(Subaward.subaward_type == "sub-grant").all()
    ...
    contracts_results = (
        sess.query(Subaward).order_by(Subaward.unique_award_key).filter(Subaward.subaward_type == "sub-contract").all()
    )
    grants_results = (
        sess.query(Subaward).order_by(Subaward.unique_award_key).filter(Subaward.subaward_type == "sub-grant").all()
    )
