...
def test_derive_pls_data(monkeypatch):
    ...
    pd.testing.assert_series_equal(
        load_defc.derive_pls_data("Nonemergency PLs 123-456, 789-12, 0-111"), expected_series
    )
    ...
    expected_series = pd.Series(
        {
            ...
            "Public Laws": [
                "Wildfire Suppression P.L. 123-456",
                "Wildfire Suppression P.L. 789-12",
    ...
    pd.testing.assert_series_equal(
        load_defc.derive_pls_data("Wildfire Suppression PLs 123-456, 789-12, 0-111"), expected_series
    )
    ...
    test_df = pd.DataFrame(
        {
            "DEFC": ["L", "1"],
            "Public Law": [
                "Nonemergency PLs 123-456, 789-12, 0-111",
                "Wildfire Suppression PLs 123-456, 789-12, 0-111",
            ],
        }
    )
    ...
    expected_df = pd.DataFrame(
        {
            "DEFC": ["L", "1"],
            "Public Law": [
                ["Non-emergency P.L. 123-456", "Non-emergency P.L. 789-12", "Non-emergency P.L. 0-111"],
                [
                    "Wildfire Suppression P.L. 123-456",
                    "Wildfire Suppression P.L. 789-12",
                    "Wildfire Suppression P.L. 0-111",
...
def add_relevant_data_types(sess):
    ...
    if len(data_types) == 0:
        defc = ExternalDataType(
            external_data_type_id=20, name="defc", description="disaster emergency fund code data loaded"
        )
        sess.add(defc)
        sess.commit()
