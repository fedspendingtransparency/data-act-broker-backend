...
def sum_range(tas, start, end, target, join_array, failed_validations, tas_str, extra_line=None):
    ...
    if line_target_amount != sum_lines_start_to_end:
        ...
        if extra_line:
            error_message = "Sum of lines {start} through {end} + {extra_line} != {target}".format(
                start=start, end=end, extra_line=extra_line, target=target
            )
        failed_validations.append(
            ",".join(
                join_array
                + [error_message, "{0:.2f}".format(line_target_amount), "{0:.2f}".format(sum_lines_start_to_end)]
            )
        )
...
def test_sf133_files(database):
    ...
    failed_validations = [
        "file,aid,ata,availability_type_code,bpoa,epoa,main_account,sub_account," "error_type,value1,value2"
    ]

    # get a list of SF 133 files to test
    sf133_list = get_prefixed_file_list(
        os.path.join(CONFIG_BROKER["path"], "dataactvalidator", "config", "to_validate"), "sf_133"
    )
    ...
    for sf133 in sf133_list:
        ...
        if file_match:
            ...
            for _, tas in data.groupby(["tas"]):
                ...
                tas_list = [
                    current_tas["agency_identifier"],
                    current_tas["allocation_transfer_agency"],
                    current_tas["availability_type_code"],
                    current_tas["beginning_period_of_availa"],
                    current_tas["ending_period_of_availabil"],
                    current_tas["main_account_code"],
                ...
                if line_1900_amount != line_amount_list_sum:
                    failed_validations.append(
                        ",".join(
                            join_array
                            + [
                                "1160 + 1180 + 1260 + 1280 + 1340 + 1440 + 1540 + " "1640 + 1750 + 1850 != 1900",
                                line_1900_amount,
                ...
                if line_1910_amount != line_amount_list_sum:
                    failed_validations.append(
                        ",".join(join_array + ["1050 + 1900 != 1910", line_1910_amount, line_amount_list_sum])
                    )
                ...
                key_list = [
                    "sum_lines_1100_through_1153",
                    "sum_lines_1170_through_1176",
                    "sum_lines_1200_through_1252",
                    "sum_lines_1270_through_1276",
                    "sum_lines_1300_through_1330",
                    "sum_lines_1400_through_1430",
                    "sum_lines_1500_through_1531",
                    "sum_lines_1600_through_1631",
                    "sum_lines_1700_through_1742",
                    "sum_lines_1800_through_1842",
                ]
                key_list_sum = sum_list(tas_str, key_list)
                if line_1900_amount != key_list_sum:
                    failed_validations.append(
                        ",".join(join_array + ["Sum of the sum of lines != 1900", line_1900_amount, key_list_sum])
                    )
                ...
                if line_1910_amount != key_list_sum:
                    failed_validations.append(
                        ",".join(join_array + ["Sum of the sum of lines != 1910", line_1910_amount, key_list_sum])
                    )
                ...
                if line_2190_amount != line_amount_sum:
                    failed_validations.append(
                        ",".join(join_array + ["2004 + 2104 != 2190", line_2190_amount, line_amount_sum])
                    )
                ...
                if line_2190_amount != line_amount_sum:
                    failed_validations.append(
                        ",".join(join_array + ["2170 + 2180 != 2190", line_2190_amount, line_amount_sum])
                    )
                ...
                if line_2490_amount != sum_lines:
                    failed_validations.append(
                        ",".join(
                            join_array
                            + [
                                "2201 + 2202 + 2203 + 2301 + 2302 + 2303 + 2401 + " "2402 + 2403 + 2413 = 2490",
                                line_2490_amount,
                                sum_lines,
                            ]
                        )
                ...
                if line_1910_amount != line_amount:
                    failed_validations.append(",".join(join_array + ["1910 != 2500", line_1910_amount, line_amount]))
