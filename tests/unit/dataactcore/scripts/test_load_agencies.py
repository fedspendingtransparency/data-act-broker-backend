...
def mock_get_agency_file_cgac(base_path):
    ...
    with open(test_file, 'w') as csv_file:
        csv_file.write("CGAC AGENCY CODE,AGENCY NAME,AGENCY ABBREVIATION,FREC,FREC Entity Description,"
...
def test_load_agencies(database, monkeypatch):
    ...
    with pytest.raises(ValueError) as val_error:
        load_agencies.load_agency_data(base_path)
    assert str(val_error.value) == 'Custom CGAC code found in agency list: 999.' \
    ...
    with pytest.raises(ValueError) as val_error:
        load_agencies.load_agency_data(base_path)
    assert str(val_error.value) == 'Custom Subtier code found in agency list: TFVA.' \
