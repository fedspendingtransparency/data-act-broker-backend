...
def test_ignore_duplicates(database):
    ...
    pf1 = PublishedFABSFactory(
        ...
        afa_generated_unique="activerecord",
        is_active=False,
        award_description="older entry",
    )
...
def test_grouping(database):
    ...
    pf1 = PublishedFABSFactory(
        ...
        unique_award_key="record",
        action_date=date(2000, 3, 22),
        is_active=True,
        federal_action_obligation=25.5,
    )
    pf2 = PublishedFABSFactory(
        updated_at=date(2024, 2, 1),
        unique_award_key="record",
        action_date=date(2003, 3, 22),
        is_active=True,
        federal_action_obligation=30.25,
    )
    pf3 = PublishedFABSFactory(
        updated_at=date(2024, 2, 4),
        unique_award_key="record",
        action_date=date(2023, 3, 22),
        is_active=True,
        federal_action_obligation=4.3,
    )
    pf4 = PublishedFABSFactory(
        updated_at=date(2024, 1, 4),
        unique_award_key="record",
        action_date=date(2023, 3, 22),
        is_active=False,
