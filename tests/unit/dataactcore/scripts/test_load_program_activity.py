...
def test_export_program_activity(
    mocked_get_pa_file, mocked_get_current_date, mocked_get_stored_date, mocked_set_stored_date, database, monkeypatch
):
    ...
    expected_headers = (
        "REPORTING_PERIOD,AGENCY_IDENTIFIER_NAME,ALLOCATION_TRANSFER_AGENCY_IDENTIFIER_CODE,"
...
def test_set_get_pa_last_upload_existing(monkeypatch, database):
    ...
    function_bag.update_external_data_load_date(
        datetime.datetime(2017, 12, 31, 0, 0, 0), datetime.datetime(2017, 12, 31, 0, 0, 0), "program_activity_upload"
    )
    ...
    function_bag.update_external_data_load_date(
        datetime.datetime(2016, 12, 31, 0, 0, 0), datetime.datetime(2016, 12, 31, 0, 0, 0), "program_activity_upload"
    )
...
@patch("dataactcore.scripts.pipeline.load_program_activity.get_date_of_current_pa_upload")
@patch("dataactcore.scripts.pipeline.load_program_activity.get_program_activity_file")
def test_load_program_activity_data(
    mocked_get_pa_file, mocked_get_current_date, mocked_get_stored_date, mocked_set_stored_date, database, monkeypatch
):
...
@patch("dataactcore.scripts.pipeline.load_program_activity.get_date_of_current_pa_upload")
@patch("dataactcore.scripts.pipeline.load_program_activity.get_program_activity_file")
def test_load_program_activity_data_only_header(
    mocked_get_pa_file, mocked_get_current_date, mocked_get_stored_date, mocked_set_stored_date, monkeypatch
):
    """Test actually loading the program activity data"""
    monkeypatch.setattr(load_program_activity, "CONFIG_BROKER", {"use_aws": False})
...
@patch("dataactcore.scripts.pipeline.load_program_activity.get_date_of_current_pa_upload")
@patch("dataactcore.scripts.pipeline.load_program_activity.get_program_activity_file")
def test_load_program_activity_data_no_header(
    mocked_get_pa_file, mocked_get_current_date, mocked_get_stored_date, mocked_set_stored_date, monkeypatch
):
    """Test actually loading the program activity data"""
    monkeypatch.setattr(load_program_activity, "CONFIG_BROKER", {"use_aws": False})
...
@patch("dataactcore.scripts.pipeline.load_program_activity.get_date_of_current_pa_upload")
@patch("dataactcore.scripts.pipeline.load_program_activity.get_program_activity_file")
def test_load_program_activity_data_empty_file(
    mocked_get_pa_file, mocked_get_current_date, mocked_get_stored_date, mocked_set_stored_date, monkeypatch
):
    """Test actually loading the program activity data"""
    monkeypatch.setattr(load_program_activity, "CONFIG_BROKER", {"use_aws": False})
