...
class ErrorWarningTests(BaseTestValidator):
    ...
    def setUpClass(cls):
        ...
        with create_app().app_context():
            ...
            cls.submission_id = insert_submission(
                sess, admin_user.user_id, cgac_code="SYS", start_date="01/2001", end_date="03/2001", is_quarter=True
            )
    ...
    def test_single_file_warnings(self):
        for chunk_size, parallel, batch_sql in self.CONFIGS:
            ...
            self.monkeypatch.setattr(
                dataactvalidator.validation_handlers.validationManager, "BATCH_SQL_VAL_RESULTS", batch_sql
            )
            self.monkeypatch.setattr(
                dataactvalidator.validation_handlers.validator, "SQL_VALIDATION_BATCH_SIZE", chunk_size
            )
    ...
    def single_file_warnings(self):
        ...
        error_count = (
            self.session.query(ErrorMetadata)
            .filter_by(job_id=self.val_job.job_id, severity_id=RULE_SEVERITY_DICT["warning"])
            .count()
        )
    ...
    def single_file_errors(self):
        ...
        # Header Error
        report_headers, report_content = self.generate_file_report(
            HEADER_ERROR, "appropriations", warning=False, ignore_error=True
        )
        appro_count = self.session.query(Appropriation).filter_by(submission_id=self.submission_id).count()
        ...
        expected_values = [
            {"Error type": "Duplicated header", "Header name": "AllocationTransferAgencyIdentifier"},
            {"Error type": "Missing header", "Header name": "AdjustmentsToUnobligatedBalanceBroughtForward_CPE"},
            {"Error type": "Missing header", "Header name": "AgencyIdentifier"},
            {"Error type": "Missing header", "Header name": "BudgetAuthorityUnobligatedBalanceBroughtForward_FYB"},
            {"Error type": "Missing header", "Header name": "DeobligationsRecoveriesRefundsOfPriorYearByTAS_CPE"},
            {"Error type": "Missing header", "Header name": "GrossOutlayAmountByTAS_CPE"},
            {"Error type": "Missing header", "Header name": "ObligationsIncurredTotalByTAS_CPE"},
            {"Error type": "Missing header", "Header name": "StatusOfBudgetaryResourcesTotal_CPE"},
        ]
        assert report_content == expected_values
        self.cleanup()
        ...
        expected_values = [
            {
                ...
                " OtherBudgetaryResourcesAmount_CPE + SF 133 Line 1902. The Broker cannot distinguish"
                " which item is incorrect for this rule. Refer to related rule errors and warnings in"
                " this report (rules A3, A6, A7, A8, A12) to distinguish which elements may be"
                " incorrect.",
                "Difference": "1.00",
                "Flex Field": "flex_field_a: FLEX_A, flex_field_b: FLEX_B",
                "Row Number": "10",
    ...
    def cross_file_warnings(self):
        ...
        # Valid
        report_headers, report_content = self.generate_cross_file_report(
            [(CROSS_FILE_A, "appropriations"), (CROSS_FILE_B, "program_activity")], warning=True
        )
        error_count = (
            self.session.query(ErrorMetadata)
            .filter_by(job_id=self.val_job.job_id, severity_id=RULE_SEVERITY_DICT["warning"])
        ...
        # SQL Validation
        report_headers, report_content = self.generate_cross_file_report(
            [(INVALID_CROSS_A, "appropriations"), (INVALID_CROSS_B, "program_activity")], warning=True
        )
        warnings = list(
            self.session.query(ErrorMetadata)
            .filter_by(job_id=self.val_job.job_id, severity_id=RULE_SEVERITY_DICT["warning"])
        ...
        expected_values = [
            {
                ...
                "Rule Message": "The GrossOutlayAmountByTAS_CPE amount in the appropriations account file (A) must"
            ...
            },
            {
                ...
                " must equal the negative (additive inverse) of the sum of the corresponding"
                " ObligationsIncurredByProgramObjectClass_CPE values in the object class and program"
                ' activity file (B) where PYA = "X".',
            ...
            },
            {
                ...
                " account file (A) must equal the sum of the corresponding"
    ...
    def cross_file_errors(self):
        ...
        # Valid
        report_headers, report_content = self.generate_cross_file_report(
            [(CROSS_FILE_A, "appropriations"), (CROSS_FILE_B, "program_activity")], warning=False
        )
        error_count = (
            self.session.query(ErrorMetadata)
            .filter_by(job_id=self.val_job.job_id, severity_id=RULE_SEVERITY_DICT["fatal"])
        ...
        # SQL Validation
        report_headers, report_content = self.generate_cross_file_report(
            [(INVALID_CROSS_A, "appropriations"), (INVALID_CROSS_B, "program_activity")], warning=False
        )
        warnings = list(
            self.session.query(ErrorMetadata)
            .filter_by(job_id=self.val_job.job_id, severity_id=RULE_SEVERITY_DICT["fatal"])
    ...
    def test_validation_parallelize_error(self):
        ...
        self.validator.file_type = (
            self.session.query(FileType).filter_by(file_type_id=FILE_TYPE_DICT["appropriations"]).one()
        )
        ...
        self.validator.error_file_name = report_file_name(
            self.validator.submission_id, False, self.validator.file_type.name
        )
        self.validator.error_file_path = "".join([CONFIG_SERVICES["error_report_path"], self.validator.error_file_name])
        self.validator.warning_file_name = report_file_name(
            self.validator.submission_id, True, self.validator.file_type.name
        )
        self.validator.warning_file_path = "".join(
            [CONFIG_SERVICES["error_report_path"], self.validator.warning_file_name]
        )

        self.validator.fields = (
            self.session.query(FileColumn)
            .filter(FileColumn.file_id == FILE_TYPE_DICT[self.validator.file_type.name])
            .order_by(FileColumn.gsdm_name.asc())
            .all()
        )
        self.validator.expected_headers, self.validator.parsed_fields = parse_fields(
            self.session, self.validator.fields
        )
        self.validator.csv_schema = {row.name_short: row for row in self.validator.fields}

        with (
            open(self.validator.error_file_path, "w", newline="") as error_file,
        ...
        self.validator.reader.open_file(
            None,
            None,
            self.validator.file_name,
            self.validator.fields,
            None,
        ...
        reader_obj = pd.read_csv(
            self.validator.reader.file, dtype=str, delimiter=",", on_bad_lines="skip", na_filter=False, chunksize=2
        )
