...
from dataactcore.aws.sqsHandler import sqs_queue, SQSMockMessage
...
class SQSWorkDispatcherTests(BaseTestValidator):
    ...
    def test_worker_process_error_exception_data(self):
        ...
        fake_queue_message = SQSMockMessage(SQS(message='Fake Message', attributes=None))
        ...
        with self.assertRaises(QueueWorkerProcessError) as err_ctx1:
            raise_exc_no_args()
        self.assertTrue(
            (unknown_worker in str(err_ctx1.exception)),
            "QueueWorkerProcessError did not mention '{}'.".format(unknown_worker)
        )
        self.assertTrue(
            ("Queue Message None or not provided" in str(err_ctx1.exception)),
            "QueueWorkerProcessError did not mention '{}'.".format(no_queue_message),
        )
        ...
        self.assertTrue(
            (unknown_worker in str(err_ctx3.exception)),
            "QueueWorkerProcessError did not mention '{}'.".format(unknown_worker)
        )
