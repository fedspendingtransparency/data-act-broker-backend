...
class ListSubmissionTests(BaseTestAPI):
    ...
    def test_list_submissions_dabs_admin(self):
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "mixed"}, headers={"x-session-id": self.session_id}
        )
        self.assertEqual(
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "false"}, headers={"x-session-id": self.session_id}
        )
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "true"}, headers={"x-session-id": self.session_id}
        )
    ...
    def test_list_submissions_fabs_admin(self):
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "mixed", "fabs": True}, headers={"x-session-id": self.session_id}
        )
        self.assertEqual(
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "false", "fabs": True}, headers={"x-session-id": self.session_id}
        )
        ...
        response = self.app.post_json(
            "/v1/list_submissions/", {"published": "true", "fabs": True}, headers={"x-session-id": self.session_id}
        )
    ...
    def test_list_submissions_filter_id(self):
        ...
        post_json = {
            "published": "mixed",
            "filters": {"submission_ids": [self.non_admin_dabs_sub_id, self.admin_fabs_sub_id]},
        }
    ...
    def test_list_submissions_filter_date(self):
        ...
        post_json = {
            "published": "mixed",
            "filters": {"last_modified_range": {"start_date": "12/31/2009", "end_date": "01/30/2010"}},
        }
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.non_admin_dabs_sub_id})

        # Time frame with no submission updates
        post_json["filters"] = {"last_modified_range": {"start_date": "12/31/2010", "end_date": "01/30/2011"}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), set())

        # One day date range (shows inclusivity)
        post_json["filters"] = {"last_modified_range": {"start_date": "01/01/2010", "end_date": "01/01/2010"}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.non_admin_dabs_sub_id})
        ...
        post_json = {
            "published": "mixed",
            "filters": {"last_modified_range": {"start_date": self.MAX_UPDATED_AT, "end_date": self.MAX_UPDATED_AT}},
        }
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.certified_dabs_sub_id})

        # Works if one of the date filters isn't provided and the other is
        post_json["filters"] = {"last_modified_range": {"start_date": "01/01/2010"}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 200)

        # Breaks if something other than start_date and end_date is passed and neither of the correct ones is
        post_json["filters"] = {"last_modified_range": {"start": "01/01/2010"}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        ...
        post_json["filters"] = {"last_modified_range": {"start_date": "30/30/2010", "end_date": "01/01/2010"}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json["message"], "Start or end date cannot be parsed into a date of format " "MM/DD/YYYY"
        )

        # Format check still happens if there's only one date
        post_json["filters"] = {"last_modified_range": {"start_date": "30/30/2010"}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        post_json["filters"] = {"last_modified_range": {"start_date": "01/02/2010", "end_date": "01/01/2010"}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        post_json["filters"] = {"last_modified_range": [123, 456]}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_list_submissions_filter_agency(self):
        ...
        post_json = {"published": "mixed", "filters": {"agency_codes": ["000"]}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.admin_dabs_sub_id})

        self.login_user()
        # Not returning a result if the user doesn't have access to the submission
        post_json = {"published": "mixed", "fabs": True, "filters": {"agency_codes": ["SYS"]}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), set())

        self.login_admin_user()
        # Invalid agency code, valid length
        post_json = {"published": "mixed", "filters": {"agency_codes": ["111"]}}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        post_json["filters"] = {"agency_codes": ["12345"]}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        post_json["filters"] = {"agency_codes": [["123", "456", "789"], "SYS"]}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        post_json["filters"] = {"agency_codes": "SYS"}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_list_submissions_filter_filename(self):
        ...
        post_json = {"published": "mixed", "filters": {"file_names": ["file"]}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        ...
        post_json = {"published": "mixed", "fabs": True, "filters": {"file_names": ["test"]}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.admin_fabs_sub_id})

        # Ignores the ID (despite it being part of the file path, but not the name)
        post_json["filters"] = {"file_names": [str(self.admin_fabs_sub_id)]}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), set())
    ...
    def test_list_submissions_filter_user_id(self):
        ...
        # Listing only the relevant submissions, even when an ID is provided that can't be reached
        post_json = {"published": "mixed", "filters": {"user_ids": [self.other_user_id, -1]}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.non_admin_dabs_sub_id})
    ...
    def test_list_submissions_filter_test_sub(self):
        ...
        post_json = {"published": "mixed", "filters": {"submission_type": "TeSt"}}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(self.sub_ids(response), {self.test_sub_id})

        # Listing only certifiable submissions
        post_json["filters"] = {"submission_type": "CertiFiaBle"}
        response = self.app.post_json("/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id})
        self.assertEqual(
            self.sub_ids(response), {self.non_admin_dabs_sub_id, self.admin_dabs_sub_id, self.certified_dabs_sub_id}
        )

        # Anything other than the allowed values throws an error
        post_json["filters"] = {"submission_type": "Both"}
        response = self.app.post_json(
            "/v1/list_submissions/", post_json, headers={"x-session-id": self.session_id}, expect_errors=True
        )
        self.assertEqual(response.status_code, 400)
