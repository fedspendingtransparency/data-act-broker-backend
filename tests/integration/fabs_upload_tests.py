...
class FABSUploadTests(BaseTestAPI):
    ...
    def setUpClass(cls):
        ...
        with create_app().app_context():
            ...
            cls.fabs_submission = cls.insert_submission(
                sess, cls.admin_user_id, cgac_code="SYS", start_date="10/2015", end_date="12/2015", is_quarter=True
            )

            cls.fabs_submission_2 = cls.insert_submission(
                sess, cls.agency_user_id, cgac_code="SYS", start_date="10/2015", end_date="12/2015", is_quarter=True
            )
            ...
            cls.other_submission = cls.insert_submission(
                sess,
                cls.admin_user_id,
                cgac_code="SYS",
                start_date="07/2015",
            ...
            cls.insert_job(
                sess, cls.running_submission, JOB_STATUS_DICT["running"], JOB_TYPE_DICT["csv_record_validation"]
            )
    ...
    def test_successful_publish_fabs_file(self):
        ...
        response = self.app.post_json("/v1/publish_fabs_file/", submission, headers={"x-session-id": self.session_id})
    ...
    def test_owner_no_permissions(self):
        ...
        response = self.app.post_json(
            "/v1/publish_fabs_file/", submission, headers={"x-session-id": self.session_id}, expect_errors=True
        )
    ...
    def test_upload_fabs_file_wrong_permissions_right_user(self):
        ...
        response = self.app.post(
            ...
            upload_files=[("fabs", "fabs.csv", open("tests/integration/data/fabs.csv", "rb").read())],
            headers={"x-session-id": self.session_id},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_upload_fabs_file_missing_fabs(self):
        response = self.app.post(
            "/v1/upload_fabs_file/",
            {"agency_code": "WRONG"},
            upload_files=[("not_fabs", "not_fabs.csv", open("tests/integration/data/fabs.csv", "rb").read())],
            headers={"x-session-id": self.session_id},
            expect_errors=True,
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_upload_published_fabs_submission(self):
        response = self.app.post(
            "/v1/upload_fabs_file/",
            {"existing_submission_id": str(self.published_submission)},
            upload_files=[("fabs", "fabs.csv", open("tests/integration/data/fabs.csv", "rb").read())],
            headers={"x-session-id": self.session_id},
            expect_errors=True,
        )
    ...
    def test_upload_fabs_duplicate_running(self):
        ...
        self.session.add(
            Job(
                ...
                submission_id=str(self.fabs_submission_2),
    ...
    def test_upload_fabs_file_invalid_format(self):
        ...
        response = self.app.post(
            ...
            {"existing_submission_id": str(self.test_agency_user_submission_id)},
            upload_files=[
                ("fabs", "invalid_file_format.md", open("tests/integration/data/invalid_file_format.md", "rb").read())
            ],
            headers={"x-session-id": self.session_id},
            expect_errors=True,
        )
