...
from dataactcore.models.lookups import (
    ALL_PERMISSION_TYPES_DICT,
    PERMISSION_SHORT_DICT,
    DABS_PERMISSION_ID_LIST,
    ...
    def inner(fn):
        ...
        def wrapped(submission_id, *args, **kwargs):
            ...
            if not active_user_can_on_submission(permission, submission, check_owner):
                raise ResponseError(
                    "User does not have permission to access that submission", StatusCode.PERMISSION_DENIED
                )
...
def requires_agency_perms(perm):
    ...
    def inner(fn):
        ...
        def wrapped(*args, **kwargs):
            ...
            if (
                req_args["existing_submission_id"] is None
                and req_args["cgac_code"] is None
                and req_args["frec_code"] is None
            ):
                raise ResponseError(
            ...
            if req_args["existing_submission_id"] is not None:
                ...
                if not active_user_can(perm, cgac_code=req_args["cgac_code"], frec_code=req_args["frec_code"]):
                    raise ResponseError(
                        "User does not have permissions to write to that agency", StatusCode.PERMISSION_DENIED
                    )
    ...
    def inner(fn):
        ...
        def wrapped(*args, **kwargs):
            ...
            try:
                ...
                raise ResponseError(
                    "Bad request: agency_code or existing_submission_id not included properly", StatusCode.CLIENT_ERROR
                )

            if req_args["agency_code"] is None and req_args["existing_submission_id"] is None:
                raise ResponseError(
                    "Missing required parameter: agency_code or existing_submission_id", StatusCode.CLIENT_ERROR
                )
            if not isinstance(req_args["agency_code"], str) and not isinstance(req_args["existing_submission_id"], str):
                raise ResponseError(
                    "Bad request: agency_code or existing_submission_id" + "required and must be strings",
                    StatusCode.CLIENT_ERROR,
                )
            if req_args["existing_submission_id"] is not None:
                check_existing_submission_perms(perm, req_args["existing_submission_id"])
                ...
                sub_tier_agency = (
                    sess.query(SubTierAgency)
                    .filter(SubTierAgency.sub_tier_agency_code == req_args["agency_code"])
                    .one_or_none()
                )

                if sub_tier_agency is None:
                    raise ResponseError("sub_tier_agency must be a valid sub_tier_agency_code", StatusCode.CLIENT_ERROR)
                ...
                if not active_user_can(perm, cgac_code=cgac_code, frec_code=frec_code):
                    raise ResponseError(
                        "User does not have permissions to write to that subtier agency", StatusCode.PERMISSION_DENIED
                    )
...
def check_existing_submission_perms(perm, submission_id):
    ...
    if submission is None:
        raise ResponseError("existing_submission_id must be a valid submission_id", StatusCode.CLIENT_ERROR)

    # Check permissions for the submission
    if not active_user_can_on_submission(perm, submission):
        raise ResponseError("User does not have permissions to write to that submission", StatusCode.PERMISSION_DENIED)
