...
def validate_load_dates(
    arg_start_date, arg_end_date, arg_auto, load_type, arg_date_format="%m/%d/%Y", output_date_format="%m/%d/%Y"
):
    ...
    if arg_auto and not (arg_start_date or arg_end_date):
        ...
        last_update = (
            ...
            .filter_by(external_data_type_id=EXTERNAL_DATA_TYPE_DICT[load_type])
            .one_or_none()
        )
    ...
    if (
        ...
        and end_date
        and (
            datetime.datetime.strptime(start_date, output_date_format)
