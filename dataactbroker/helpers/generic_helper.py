...
def generate_raw_quoted_query(queryset):
    ...
    """
    return str(queryset.statement.compile(dialect=LiteralDialect(), compile_kwargs={"literal_binds": True})).replace(
        "\n", " "
    )
...
def zip_dir(dir_path, name):
    ...
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            zip_h.write(
                os.path.join(root, file), os.path.relpath(os.path.join(root, file), os.path.join(dir_path, ".."))
            )
    zip_h.close()
    return zip_path
