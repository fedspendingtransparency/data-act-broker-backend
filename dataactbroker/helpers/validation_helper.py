...
def derive_fabs_afa_generated_unique(row):
    ...
    """
    return (
        (row["awarding_sub_tier_agency_c"] or "-none-")
        + "_"
        + (row["fain"] or "-none-")
        + "_"
        + (row["uri"] or "-none-")
        + "_"
        + (row["assistance_listing_number"] or "-none-")
        + "_"
...
def valid_type_bool_vector(series: pd.Series, csv_schema, type_field=None, match_invalid=False):
    ...
    if not field:
        raise ValueError(
            "Cannot lookup field type info from schema because the given Series has no name and "
...
def update_val_progress(sess, job, validation_progress, tas_progress, sql_progress, final_progress):
    ...
    val_mult = 0.2 if FILE_TYPE_DICT_ID[job.file_type_id] != "fabs" else 0.4
    tas_mult = 0.25 if FILE_TYPE_DICT_ID[job.file_type_id] != "fabs" else 0
    sql_mult = 0.5
