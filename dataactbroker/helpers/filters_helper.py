...
def agency_filter(sess, query, cgac_model, frec_model, agency_list):
    ...
    if len(cgac_codes) + len(frec_codes) != len(agency_list):
        raise ResponseError("All codes in the agency_codes filter must be valid agency codes", StatusCode.CLIENT_ERROR)
    ...
    if files:
        for file_type in files:
            ...
            if file_type in ["A", "B", "C"]:
                file_type_filters.append(and_(file_id == FILE_TYPE_DICT_LETTER_ID[file_type], target_file_id.is_(None)))
            else:
                file_types = file_type.split("-")[1]
                # Append both orders of the source/target files to the list
                file_type_filters.append(
                    and_(
                        file_id == FILE_TYPE_DICT_LETTER_ID[file_types[:1]],
...
def tas_agency_filter(sess, agency_code, filter_model):
    ...
    if not frec_provided:
        frec_list = (
            sess.query(FREC.frec_code)
            .select_from(outerjoin(CGAC, FREC, CGAC.cgac_id == FREC.cgac_id))
            .filter(CGAC.cgac_code == agency_code)
            .all()
        )
    ...
    if agency_code in ["097", "020", "077", "089"]:
        agency_filters.append(
            and_(filter_model.allocation_transfer_agency.is_(None), filter_model.agency_identifier.in_(agency_array))
        )
    elif agency_code == "1100":
        agency_filters.append(
            and_(
                filter_model.allocation_transfer_agency.is_(None),
                or_(filter_model.agency_identifier == "256", filter_model.fr_entity_type == "1100"),
            )
        )
    elif not frec_provided:
        agency_filters.append(
            and_(filter_model.allocation_transfer_agency.is_(None), filter_model.agency_identifier == agency_code)
        )
    else:
        agency_filters.append(
            and_(filter_model.allocation_transfer_agency.is_(None), filter_model.fr_entity_type == agency_code)
        )
