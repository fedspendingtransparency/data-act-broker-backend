...
def configure_spark_session(
    ...
    **options,
    ...
    if not CONFIG_BROKER["use_aws"]:  # i.e. running in a "local" [development] environment
...
def get_jdbc_connection_properties(fix_strings=True) -> dict:
    ...
    jdbc_props = {"driver": "org.postgresql.Driver", "fetchsize": str(SPARK_PARTITION_ROWS)}
    if fix_strings:
...
def configure_s3_credentials(
    ...
    if temporary_creds:
        ...
        conf.set("spark.hadoop.fs.s3a.assumed.role.sts.endpoint", "sts.us-gov-west-1.amazonaws.com")
