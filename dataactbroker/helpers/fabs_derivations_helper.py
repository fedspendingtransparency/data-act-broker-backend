...
def derive_ppop_location_data(obj, sess, ppop_code, ppop_state_code, county_dict):
    ...
    if obj['place_of_performance_zip4a'] and obj['place_of_performance_zip4a'].upper() != 'CITY-WIDE':
        ...
        if ppop_code and (re.match('^[A-Z]{2}\d{5}$', ppop_code.upper()) or
                          re.match('^[A-Z]{2}\d{4}R$', ppop_code.upper())):
    ...
    # if there is no valid ppop zip4, we need to try to derive county/city info from the ppop code
        ...
        if re.match('^[A-Z]{2}\*\*\d{3}$', ppop_code.upper()):
            # getting county name
        ...
        # if ppop_code is in city format
        elif re.match('^[A-Z]{2}\d{5}$', ppop_code.upper()) and not re.match('^[A-Z]{2}0{5}$', ppop_code.upper()):
            ...
            if (obj['place_of_performance_zip4a'] and obj['place_of_performance_zip4a'].upper() == 'CITY-WIDE'):
                ...
                obj['place_of_performance_scope'] = None
        elif re.match('^[A-Z]{2}\d{4}R$', ppop_code.upper()):
            if (obj['place_of_performance_zip4a'] and obj['place_of_performance_zip4a'].upper() == 'CITY-WIDE'):
                obj['place_of_performance_scope'] = "City-wide"
            else:
                obj['place_of_performance_scope'] = None
        elif re.match('^[A-Z]{2}\*{5}$', ppop_code.upper()):
            obj['place_of_performance_scope'] = "State-wide"
        elif re.match('^00\*{5}$', ppop_code.upper()):
            obj['place_of_performance_scope'] = "Multi-state"
        elif ppop_code.upper() == '00FORGN':
