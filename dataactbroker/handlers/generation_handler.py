...
def generate_file(submission, file_type, start, end, agency_type, file_format):
    ...
    job = (
        ...
        .filter(
            ...
            Job.job_type_id == lookups.JOB_TYPE_DICT["file_upload"],
        )
        .one()
    )
    logger.info(
    ...
    if not generation_helper.check_generation_prereqs(submission.submission_id, file_type):
        return JsonResponse.error(
            ResponseError("Must wait for completion of prerequisite validation job", StatusCode.CLIENT_ERROR),
...
def generate_detached_file(
    file_type, cgac_code, frec_code, start_date, end_date, year, period, agency_type, file_format, element_numbers
):
    ...
    if not cgac_code and not frec_code:
        return JsonResponse.error(
            ValueError("Detached file generation requires CGAC or FR Entity Code"), StatusCode.CLIENT_ERROR
        )
    ...
    if file_type in ["D1", "D2"]:
        ...
        if not start_date or not end_date:
            return JsonResponse.error(
                ValueError("Must have a start and end date for D file generation."), StatusCode.CLIENT_ERROR
            )
        ...
        if agency_type not in ["awarding", "funding"]:
            return JsonResponse.error(
                ValueError("agency_type must be either awarding or funding."), StatusCode.CLIENT_ERROR
            )

        if file_format not in ["csv", "txt"]:
            return JsonResponse.error(ValueError("file_format must be either csv or txt."), StatusCode.CLIENT_ERROR)
    else:
        # Make sure both year and period are provided
        if not (year and period):
            return JsonResponse.error(
                ValueError("Must have a year and period for A or BOC file generation."), StatusCode.CLIENT_ERROR
            )
        ...
        if file_type == "BOC":
            ...
            if len(missing_data) > 0:
                return JsonResponse.error(
                    ValueError(
                        f'There is no {" or ".join(missing_data)} for the selected'
    ...
    try:
        if file_type in ["D1", "D2"]:
            generation_helper.start_d_generation(
                new_job,
                start_date,
                end_date,
                agency_type,
