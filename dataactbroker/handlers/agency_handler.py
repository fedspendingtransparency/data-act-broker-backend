...
def get_accessible_agencies(perm_level="reader", perm_type="mixed"):
    ...
    if not g.user.website_admin:
        ...
        if perm_type != "mixed":
            perm_type_map = {"dabs": DABS_PERMISSION_ID_LIST, "fabs": FABS_PERMISSION_ID_LIST}
        ...
        if perm_level != "reader":
            perm_level_map = {"writer": WRITER_ID_LIST, "submitter": SUBMITTER_ID_LIST}
            cgac_affiliations = [
                aff for aff in cgac_affiliations if aff.permission_type_id in perm_level_map[perm_level]
            ]
            frec_affiliations = [
                aff for aff in frec_affiliations if aff.permission_type_id in perm_level_map[perm_level]
            ]

        # Start with an object of objects to prevent duplicates
        cgac_list = {
            aff.cgac.cgac_code: {"agency_name": aff.cgac.agency_name, "cgac_code": aff.cgac.cgac_code}
            for aff in cgac_affiliations
        }
        frec_list = {
            aff.frec.frec_code: {"agency_name": aff.frec.agency_name, "frec_code": aff.frec.frec_code}
            for aff in frec_affiliations
        }
...
def get_user_agency_codes():
    ...
    return list(
        set([cgac["cgac_code"] for cgac in user_agencies["cgac_agency_list"]])
...
def organize_sub_tier_agencies(sub_tier_agencies):
    ...
    for sub_tier in sub_tier_agencies:
        ...
        agencies.append(
            {
                "agency_name": "{}: {}".format(agency_name, sub_tier.sub_tier_agency_name),
                "agency_code": sub_tier.sub_tier_agency_code,
