...
from dataactcore.interfaces.function_bag import (
    sum_number_of_errors_for_job_list,
    get_last_validated_date,
    get_fabs_meta,
    get_error_type,
    get_error_metrics_by_job_id,
...
def get_submission_metadata(submission):
    ...
    number_of_rows = (
        sess.query(func.sum(case([(Job.number_of_rows > 0, Job.number_of_rows - 1)], else_=0)))
        .filter_by(submission_id=submission.submission_id)
        .scalar()
        or 0
    )

    total_size = sess.query(func.sum(Job.file_size)).filter_by(submission_id=submission.submission_id).scalar() or 0
...
def get_latest_publication_period():
    ...
    last_pub_period = (
        sess.query(
            SubmissionWindowSchedule.period, SubmissionWindowSchedule.year, SubmissionWindowSchedule.publish_deadline
        )
        .filter(SubmissionWindowSchedule.period_start <= datetime.today())
        .order_by(SubmissionWindowSchedule.period_start.desc())
        .first()
    )
    return {
...
def job_to_dict(job):
    ...
    if file_results is None:
        ...
        job_info["error_data"] = get_error_metrics_by_job_id(
            job.job_id, job.job_type_name == "validation", severity_id=RULE_SEVERITY_DICT["fatal"]
        )
        job_info["warning_data"] = get_error_metrics_by_job_id(
            job.job_id, job.job_type_name == "validation", severity_id=RULE_SEVERITY_DICT["warning"]
        )
    return job_info
...
def delete_all_submission_data(submission):
    ...
    if submission.publish_status_id != PUBLISH_STATUS_DICT["unpublished"]:
        return JsonResponse.error(
            ValueError("Submissions that have been published cannot be deleted"), StatusCode.CLIENT_ERROR
        )
    ...
    running_jobs = (
        sess.query(Job)
        .filter(Job.submission_id == submission.submission_id, Job.job_status_id == JOB_STATUS_DICT["running"])
        .all()
    )
    if running_jobs:
        return JsonResponse.error(
            ValueError("Submissions with running jobs cannot be deleted"), StatusCode.CLIENT_ERROR
        )
    ...
    sess.query(SubmissionSubTierAffiliation).filter(
        SubmissionSubTierAffiliation.submission_id == submission.submission_id
...
def check_current_submission_page(submission):
    ...
    if submission.is_fabs:
        data = {"message": "This submission is currently on the /FABSaddData page.", "step": "6"}
        return JsonResponse.create(StatusCode.OK, data)

    # /reviewData
    # Checks that both E and F files are finished
    review_data = sess.query(Job).filter(
        Job.submission_id == submission_id, Job.file_type_id.in_([6, 7]), Job.job_status_id == 4
    )

    # Need to check that cross file is done as well
    generate_ef = sess.query(Job).filter(
        Job.submission_id == submission_id, Job.job_type_id == 4, Job.number_of_errors == 0, Job.job_status_id == 4
    )

    if review_data.count() == 2 and generate_ef.count() > 0:
        data = {"message": "This submission is currently on the /reviewData page.", "step": "5"}
        return JsonResponse.create(StatusCode.OK, data)

    # /generateEF
    if generate_ef.count() > 0:
        data = {"message": "This submission is currently on the /generateEF page.", "step": "4"}
        return JsonResponse.create(StatusCode.OK, data)

    validate_cross_file = sess.query(Job).filter(
        Job.submission_id == submission_id,
        Job.file_type_id.in_([4, 5]),
        Job.job_type_id == 2,
    ...
    generate_files = sess.query(Job).filter(
        Job.submission_id == submission_id,
        Job.file_type_id.in_([1, 2, 3]),
        Job.job_type_id == 2,
        Job.number_of_errors == 0,
        Job.file_size.isnot(None),
        Job.job_status_id == 4,
    ...
    if validate_cross_file.count() == 2 and generate_files.count() == 3:
        data = {"message": "This submission is currently on the /validateCrossFile page.", "step": "3"}
        return JsonResponse.create(StatusCode.OK, data)

    # /generateFiles
    if generate_files.count() == 3:
        data = {"message": "This submission is currently on the /generateFiles page.", "step": "2"}
        return JsonResponse.create(StatusCode.OK, data)

    # /validateData
    validate_data = sess.query(Job).filter(
        Job.submission_id == submission_id,
        Job.file_type_id.in_([1, 2, 3]),
        Job.job_type_id == 2,
        Job.number_of_errors != 0,
    ...
    check_header_errors = sess.query(Job).filter(
        Job.submission_id == submission_id,
        Job.file_type_id.in_([1, 2, 3]),
        Job.job_type_id == 2,
        Job.job_status_id != 4,
    ...
    if validate_data.count() or check_header_errors.count() > 0:
        data = {"message": "This submission is currently on the /validateData page.", "step": "1"}
        return JsonResponse.create(StatusCode.OK, data)
...
def get_published_submission_ids(
    cgac_code, frec_code, reporting_fiscal_year, reporting_fiscal_period, is_quarter_format, submission_id=None
):
    ...
    pub_subs = get_submissions_in_period(
        cgac_code,
        frec_code,
        int(reporting_fiscal_year),
        int(reporting_fiscal_period),
        is_quarter_format,
        submission_id=submission_id,
        filter_published="published",
    )
    published_submissions = [
        {"submission_id": pub_sub.submission_id, "is_quarter": pub_sub.is_quarter_format} for pub_sub in pub_subs
    ]
    return JsonResponse.create(StatusCode.OK, {"published_submissions": published_submissions})


def get_submissions_in_period(
    cgac_code,
    frec_code,
    ...
    mon_subs = filter_submissions(
        cgac_code,
        frec_code,
        reporting_fiscal_year,
        reporting_fiscal_period,
        submission_id,
        filter_quarter=is_quarter_format,
        filter_published=filter_published,
...
def filter_submissions(
    ...
    if not filter_quarter:
        ...
        submission_query = submission_query.filter(
            (func.ceil(cast(Submission.reporting_fiscal_period, Numeric) / 3) == reporting_fiscal_quarter)
        )
...
def move_published_data(sess, submission_id, direction="publish"):
    ...
    """
    table_types = {
        "appropriation": [Appropriation, PublishedAppropriation, "submission"],
        ...
        "award_financial_assistance": [AwardFinancialAssistance, PublishedAwardFinancialAssistance, "submission"],
    ...
    for table_type, table_object in table_types.items():
        ...
        insert_string = """
            ...
            WHERE
        """.format(
            source=source_table.__table__.name, target=target_table.__table__.name, cols=col_string
        )
...
def publish_checks(submission):
    ...
    if reval_thresh and reval_thresh >= last_validated:
        raise ValueError(
            "This submission has not been validated since before the revalidation threshold ({}), it must"
    ...
    if not sub_schedule:
        raise ValueError(
            "No submission window for this year and period was found. If this is an error, please contact"
            " the Service Desk."
        )
    ...
    if last_validated < sub_schedule.period_start:
        raise ValueError(
            "This submission was last validated or its D files generated before the start of the"
            " submission window ({}). Please revalidate before publishing.".format(
                sub_schedule.period_start.strftime("%m/%d/%Y")
            )
        )
    ...
    blank_files = (
        sess.query(Job)
        .filter(
            Job.file_type_id.in_([FILE_TYPE_DICT["appropriations"], FILE_TYPE_DICT["program_activity"]]),
            Job.number_of_rows_valid == 0,
...
def process_dabs_publish(submission, file_manager):
    ...
    if first_publish:
        ...
        unpub_subs = get_submissions_in_period(
            submission.cgac_code,
            submission.frec_code,
            submission.reporting_fiscal_year,
            submission.reporting_fiscal_period,
...
def process_dabs_certify(submission):
    ...
    max_pub_history = (
        sess.query(func.max(PublishHistory.publish_history_id).label("max_id"))
        .filter(PublishHistory.submission_id == submission.submission_id)
        .one()
    )

    if max_pub_history.max_id is None:
        raise ValueError(
            "There is no publish history associated with this submission. Submission must be published"
...
def publish_dabs_submission(submission, file_manager):
    ...
    if submission.is_quarter_format:
        return JsonResponse.error(
            ValueError(
                "Quarterly submissions cannot be published separate from certification."
    ...
    if submission.certified:
        return JsonResponse.error(
            ValueError(
                "Submissions that have been certified cannot be republished separately."
    ...
    if sub_schedule and sub_schedule.certification_deadline < get_utc_now():
        return JsonResponse.error(
            ValueError(
                "Monthly submissions past their certification deadline must be published"
                " and certified at the same time. Use the"
                " publish_and_certify_dabs_submission endpoint."
            ),
            StatusCode.CLIENT_ERROR,
        )
...
def certify_dabs_submission(submission):
    ...
    if submission.is_quarter_format:
        return JsonResponse.error(
            ValueError(
                "Quarterly submissions cannot be certified separate from publication."
                " Use the publish_and_certify_dabs_submission endpoint to publish and"
                " certify."
            ),
            StatusCode.CLIENT_ERROR,
    ...
    if submission.certified:
        return JsonResponse.error(
            ValueError(
                "Submissions that have been certified cannot be recertified separately."
...
def publish_and_certify_dabs_submission(submission, file_manager):
    ...
    if submission.is_quarter_format and submission.reporting_fiscal_year >= 2022:
        return JsonResponse.error(
            ValueError("Quarterly submissions from FY22 onward cannot be published."), StatusCode.CLIENT_ERROR
        )
    try:
        process_dabs_publish(submission, file_manager)
    except ValueError as e:
...
def revert_to_published(submission, file_manager):
    ...
    if submission.publish_status_id != PUBLISH_STATUS_DICT["updated"]:
        raise ResponseError(
            "Submission has not been published or has not been updated since publication.",
    ...
    max_pub_history = (
        sess.query(func.max(PublishHistory.publish_history_id), func.max(PublishHistory.updated_at))
        .filter(PublishHistory.submission_id == submission.submission_id)
        .one()
    )
    remove_timestamp = [
        str(FILE_TYPE_DICT["appropriations"]),
        str(FILE_TYPE_DICT["program_activity"]),
    ...
    """.format(
        history_id=max_pub_history[0],
        filepath=filepath,
        ef_path=ef_path,
        remove_timestamp=", ".join(remove_timestamp),
        submission_id=submission.submission_id,
    )
    ...
    sess.query(Job).filter_by(submission_id=submission.submission_id).update(
        {
            "number_of_errors": 0,
            "number_of_warnings": 0,
            "job_status_id": JOB_STATUS_DICT["finished"],
            "last_validated": max_pub_history[1],
            "error_message": None,
    ...
    job_list = (
        ...
        .filter(
            ...
            Job.file_type_id.notin_([FILE_TYPE_DICT["sub_award"], FILE_TYPE_DICT["executive_compensation"]]),
        )
        .all()
    )
    ...
    return JsonResponse.create(
        StatusCode.OK,
        {"message": "Submission {} successfully reverted to published status.".format(submission.submission_id)},
    )
