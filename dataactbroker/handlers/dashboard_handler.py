...
def list_rule_labels(files, error_level="warning", fabs=False):
    ...
    if invalid_files:
        return JsonResponse.error(
            ValueError("The following are not valid file types: {}".format(", ".join(invalid_files))),
...
def validate_historic_dashboard_filters(filters, graphs=False):
    ...
    if missing_filters:
        raise ResponseError(
            "The following filters were not provided: {}".format(", ".join(missing_filters)),
    ...
    if wrong_filter_types_list:
        raise ResponseError(
            "The following filters were not lists: {}".format(", ".join(wrong_filter_types_list)),
            status=StatusCode.CLIENT_ERROR,
        )
    ...
    for period in filters["periods"]:
        if period not in range(2, 13):
            raise ResponseError(
                "Periods must be a list of integers, each ranging 2-12, or an empty list.",
                status=StatusCode.CLIENT_ERROR,
            )
    ...
    for fiscal_year in filters["fys"]:
        if fiscal_year not in range(2017, current_fy + 1):
            raise ResponseError(
                "Fiscal Years must be a list of integers, each ranging from 2017 through the"
    ...
    for agency in filters["agencies"]:
        if not isinstance(agency, str):
            raise ResponseError("Agencies must be a list of strings, or an empty list.", status=StatusCode.CLIENT_ERROR)

    if graphs:
        for file_type in filters["files"]:
            if file_type not in FILE_TYPES:
                raise ResponseError(
                    "Files must be a list of one or more of the following, or an empty list: {}".format(
                        ", ".join(FILE_TYPES)
                    ),
                    status=StatusCode.CLIENT_ERROR,
                )
        ...
        for rule in filters["rules"]:
            if not isinstance(rule, str):
                raise ResponseError(
                    "Rules must be a list of strings, or an empty list.", status=StatusCode.CLIENT_ERROR
                )
...
def validate_table_properties(page, limit, order, sort, sort_options):
    ...
    if sort not in sort_options:
        raise ResponseError("Sort must be one of: {}".format(", ".join(sort_options)), status=StatusCode.CLIENT_ERROR)
...
def historic_dabs_warning_graphs(filters):
    ...
    if sub_ids:
        ...
        total_warnings_query = (
            ...
            .join(Job, Job.job_id == PublishedErrorMetadata.job_id)
            .filter(Job.submission_id.in_(sub_ids))
            .group_by(
                Job.submission_id, PublishedErrorMetadata.file_type_id, PublishedErrorMetadata.target_file_type_id
            )
        )
        ...
        for query_result in error_metadata_query.all():
            ...
            warning = {"label": query_result.label, "instances": query_result.instances, "percent_total": 0}
...
def historic_dabs_warning_table(filters, page, limit, sort="period", order="desc"):
    ...
    response = {"results": [], "page_metadata": {"total": total_metadata, "page": page, "limit": limit}}
...
def active_submission_overview(submission, file, error_level):
    ...
    # Getting rule counts
    rule_query = (
        sess.query(func.sum(ErrorMetadata.occurrences).label("total_instances"), func.count(1).label("number_of_rules"))
...
def get_impact_counts(submission, file, error_level):
    ...
    impact_query = (
        sess.query(
            ErrorMetadata.original_rule_label,
            ErrorMetadata.occurrences,
            ErrorMetadata.rule_failed,
        ...
        .join(
            RuleSetting,
            and_(
                ErrorMetadata.original_rule_label == RuleSetting.rule_label,
...
def get_significance_counts(submission, file, error_level):
    ...
    # Basic data that can be gathered from just the submission and passed filters
    response = {"total_instances": 0, "rules": []}
    ...
    significance_query = (
        ...
        .join(
            RuleSql,
            and_(
                RuleSql.rule_label == ErrorMetadata.original_rule_label,
...
def active_submission_table(submission, file, error_level, page=1, limit=5, sort="significance", order="desc"):
    ...
    table_query = (
        sess.query(
            ...
            RuleSql.category,
            RuleSetting.priority,
            RuleImpact.name.label("impact_name"),
        )
        .join(Job, Job.job_id == ErrorMetadata.job_id)
        .join(
            RuleSql,
