...
class FileHandler:
    ...
    def validate_upload_dabs_files(self):
        ...
        if not (start_date is None or end_date is None):
            ...
            if not is_quarter:
                ...
                if (
                    formatted_start_date.month != formatted_end_date.month
                    and formatted_start_date.month != period1
    ...
    def validate_submit_file_params(request_params):
        ...
        if existing_submission_id and param_count == 0:
            raise ResponseError("Must include at least one file for an existing submission", StatusCode.CLIENT_ERROR)
    ...
    def submit(self, sess):
        ...
        try:
            ...
            if existing_submission_id:
                ...
                if existing_submission_obj.publish_status_id in (
                    PUBLISH_STATUS_DICT["publishing"],
                    PUBLISH_STATUS_DICT["reverting"],
                ):
                    raise ResponseError(
                        "Existing submission must not be publishing, certifying, or reverting", StatusCode.CLIENT_ERROR
                    )
            ...
            if not existing_submission:
                ...
                if cgac_code != "" and frec_code != "":
                    raise ResponseError(
                        "New DABS submissions must have either a CGAC or a FREC code but not both",
            ...
            if not existing_submission:
                ...
                if reporting_fiscal_year >= 2022 and submission_data["is_quarter_format"]:
                    raise ResponseError(
                        "Quarterly submissions may not be created for FY22 or later", StatusCode.CLIENT_ERROR
                    )
            ...
            if not existing_submission:
                for ext_file_type in FileHandler.EXTERNAL_FILE_TYPES:
                    ...
                    if ext_file_type == "A":
                        ...
                        fillin_vals.update(
                            {"raw_filename": "Broker{}".format(curr_date.strftime("%Y%m%d")), "ext": ".csv"}
                        )
            ...
            if not existing_submission:
                ...
                generate_file(
                    submission,
                    "A",
                    end_date.replace(day=1).strftime("%m/%d/%Y"),
                ...
                jobs = (
                    ...
                    .filter(
                        ...
                        Job.file_type_id.in_(
                            [
                                FILE_TYPE_DICT_LETTER_ID["A"],
                                FILE_TYPE_DICT_LETTER_ID["B"],
                                FILE_TYPE_DICT_LETTER_ID["C"],
                            ]
                        ),
                    )
                    .all()
    ...
    def check_submission_dates(start_date, end_date, is_quarter, existing_submission=None):
        ...
        if None in (start_date, end_date, is_quarter) and existing_submission is None:
            raise ResponseError(
                "An existing submission is required when start/end date " "or is_quarter aren't supplied",
    ...
    def upload_fabs_file(self, fabs):
        ...
        if fabs is None:
            return JsonResponse.error(
                Exception("fabs field must be present and contain a file"), StatusCode.CLIENT_ERROR
            )
        ...
        try:
            ...
            if existing_submission_obj is not None:
                ...
                sub_tier_agency = (
                    sess.query(SubTierAgency).filter_by(sub_tier_agency_code=request_params["agency_code"]).one()
                )
            ...
            if existing_submission_obj is None:
                ...
                sub_tier_affiliation = SubmissionSubTierAffiliation(
                    submission_id=submission.submission_id, sub_tier_agency_id=sub_tier_agency_id
                )
            ...
            if json_response.status_code != StatusCode.OK and submission:
                fabs_job = (
                    ...
                    .filter(
                        ...
                        Job.job_type_id == JOB_TYPE_DICT["file_upload"],
                        Job.job_status_id == JOB_STATUS_DICT["running"],
                        Job.file_type_id == FILE_TYPE_DICT_LETTER_ID["FABS"],
                    )
                    .one_or_none()
                )
                if fabs_job:
    ...
    def publish_fabs_submission(submission):
        ...
        try:
            ...
            valid_sub_rows = (
                sess.query(FABS.afa_generated_unique)
                .filter(FABS.submission_id == submission_id, FABS.is_valid.is_(True))
                .cte("valid_sub_rows")
            )
            publishing_subs = (
                sess.query(FABS.submission_id)
                .join(
                    valid_sub_rows,
            ...
            if publishing_subs:
                ...
                raise ResponseError(
                    "1 or more rows in this submission are currently publishing (in a separate "
            ...
            colliding_rows = (
                ...
                .join(
                    PublishedFABS,
                    and_(
                        func.upper(FABS.afa_generated_unique) == func.upper(PublishedFABS.afa_generated_unique),
            ...
            if colliding_rows > 0:
                raise ResponseError(
                    "1 or more rows in this submission were already published (in a separate "
            ...
            log_derivation(
                "Starting derivations for FABS submission (total count: {})".format(total_count), submission_id
            )
            ...
            create_table_sql = """
                ...
                ALTER TABLE tmp_fabs_{submission_id} ADD COLUMN published_fabs_id
                    SERIAL PRIMARY KEY;
            """.format(
                submission_id=submission_id, cols=published_col_string
            )
            ...
            sess.query(Submission).filter_by(submission_id=submission_id).update(
                {"publish_status_id": PUBLISH_STATUS_DICT["unpublished"], "updated_at": get_utc_now()},
                synchronize_session=False,
            )
            sess.commit()
        ...
        sess.query(Submission).filter_by(submission_id=submission_id).update(
            {
                ...
                "publishing_user_id": g.user.user_id,
        ...
        FileHandler.move_published_files(
            FileHandler, submission, publish_history, certify_history.certify_history_id, g.is_local
        )
    ...
    def build_file_map(self, file_dict, file_type_list, upload_files, submission):
        ...
        for file_type in file_type_list:
            ...
            try:
                ...
                return JsonResponse.error(
                    Exception("{} parameter must be a file in binary form".format(file_type)), StatusCode.CLIENT_ERROR
                )
            if file_name:
    ...
    def restart_validation(submission, fabs):
        ...
        for job in jobs:
            if job.job_status_id == JOB_STATUS_DICT["running"]:
                return JsonResponse.error(
                    ValueError("Submission is still uploading or validating"), StatusCode.CLIENT_ERROR
                )

            if job.job_type_id == JOB_TYPE_DICT["file_upload"] and job.file_type_id in [
        ...
        upload_jobs = [
            job
            for job in jobs
            if job.job_type_id in [JOB_TYPE_DICT["file_upload"]] and job.file_type_id in initial_file_types
        ]
    ...
    def move_published_files(self, submission, publish_history, certify_history_id, is_local):
        ...
        if submission.is_fabs:
            ...
            time_period = (
                "P{}".format(str(submission.reporting_fiscal_period).zfill(2))
                if not submission.is_quarter_format
                else "Q{}".format(submission.reporting_fiscal_period // 3)
            )
            route_vars = [
                agency_code,
        ...
        for job in jobs:
            ...
            if job.file_type_id in possible_warning_files:
                ...
                if not is_local:
                    ...
                    self.s3manager.copy_file(
                        original_bucket=original_bucket,
                        new_bucket=new_bucket,
                        original_path="errors/" + warning_file_name,
                        new_path=warning_file,
                    )
                else:
                    warning_file = CONFIG_SERVICES["error_report_path"] + report_file_name(
                        submission_id, True, job.file_type.name
                    )
            ...
            if submission.is_fabs:
                ...
                comment = (
                    sess.query(Comment)
                    .filter_by(submission_id=submission_id, file_type_id=job.file_type_id)
                    .one_or_none()
                )
                if comment:
        ...
        if not submission.is_fabs:
            ...
            for key, value in cross_list.items():
                ...
                if not is_local:
                    ...
                    warning_file = CONFIG_SERVICES["error_report_path"] + report_file_name(
                        submission_id, True, first_file, second_file
                    )
            ...
            if pub_comments.count() > 0:
                ...
                if format_change and created_at < format_change.change_date:
                    ...
                    filename = "SubID-{}_comments_{}.csv".format(
                        str(submission_id), filename_fyp_sub_format(submission)
                    )
                if not is_local:
    ...
    def revert_published_error_comment_files(self, sess, publish_history_id, submission_id):
        ...
        """
        warning_files = (
            sess.query(PublishedFilesHistory.warning_filename)
            .filter(
                PublishedFilesHistory.publish_history_id == publish_history_id,
                PublishedFilesHistory.warning_filename.isnot(None),
            )
            .all()
        )
        comment_file = (
            sess.query(PublishedFilesHistory.filename)
            .filter(
                PublishedFilesHistory.publish_history_id == publish_history_id,
                PublishedFilesHistory.filename.like("%_comments%"),
                PublishedFilesHistory.file_type_id.is_(None),
            )
            .one_or_none()
        )
        for warning in warning_files:
            ...
            if not self.is_local:
                ...
                S3Handler.copy_file(
                    original_bucket=certified_bucket,
                    new_bucket=submission_bucket,
                    original_path=warning,
                    new_path="errors/" + warning_file_name,
                )
            else:
        ...
        if comment_file and not self.is_local:
            ...
            S3Handler.copy_file(
                original_bucket=certified_bucket,
                new_bucket=submission_bucket,
                original_path=comment_file.filename,
                new_path=str(submission_id) + "/" + file_name,
            )
...
def update_submission_comments(submission, comment_request, is_local):
    ...
    if submission.publish_status_id in (PUBLISH_STATUS_DICT["publishing"], PUBLISH_STATUS_DICT["reverting"]):
        raise ResponseError(
            "Submission must not be publishing, certifying, or reverting when updating comments",
            StatusCode.CLIENT_ERROR,
        )
    ...
    for file_type_id, letter in FILE_TYPE_DICT_LETTER.items():
        if letter in comments_json and letter not in ("FABS", "BOC"):
            comments.append(
                Comment(
                    submission_id=submission.submission_id, file_type_id=file_type_id, comment=comments_json[letter]
                )
            )
    submission_comment = comments_json.get("SUBMISSION_COMMENT")
    if submission_comment:
        comments.append(Comment(submission_id=submission.submission_id, file_type_id=None, comment=submission_comment))
    ...
    comment_query = (
        sess.query(
            case([(FileType.name.isnot(None), FileType.name), (FileType.name.is_(None), "Submission Comment")]),
            Comment.comment,
        )
...
def get_comments_file(submission, is_local):
    ...
    if comments.count() > 0:
        ...
        if is_local:
            ...
            url = S3Handler().get_signed_url(
                str(submission.submission_id), filename, url_mapping=CONFIG_BROKER["submission_bucket_mapping"]
            )
        return JsonResponse.create(StatusCode.OK, {"url": url})
    return JsonResponse.error(
        ValueError("This submission does not have any comments associated with it"), StatusCode.CLIENT_ERROR
    )
...
def get_submission_zip(submission, publish_history_id, certify_history_id, is_local):
    ...
    if publish_history_id:
        ...
        return JsonResponse.error(
            ValueError("A publish_history_id or certify_history_id is required."), StatusCode.CLIENT_ERROR
        )
...
def get_status(submission, file_type=""):
    ...
    if file_type and (is_fabs and file_type != "fabs") or (not is_fabs and file_type == "fabs"):
        return JsonResponse.error(
            ValueError(file_type + " is not a valid file type for this submission"), StatusCode.CLIENT_ERROR
        )
...
def add_list_submission_filters(query, filters, submission_updated_view):
    ...
    if "last_modified_range" in filters:
        ...
        if mod_dates and isinstance(mod_dates, dict):
            ...
            if not start_date and not end_date:
                raise ResponseError(
                    "At least start_date or end_date must be provided when using " "last_modified_range filter",
                    StatusCode.CLIENT_ERROR,
                )
            ...
            if start_date and end_date and start_date >= end_date:
                raise ResponseError(
                    "Last modified start date cannot be greater than the end date", StatusCode.CLIENT_ERROR
                )
            if start_date:
    ...
    if "file_names" in filters:
        ...
        if file_list and isinstance(file_list, list):
            ...
            sub_query = (
                sess.query(Job.submission_id.label("job_sub_id"))
...
def list_submissions(page, limit, published, sort="modified", order="desc", is_fabs=False, filters=None):
    ...
    submission_columns = [
        ...
        Submission.frec_code,
        Submission.user_id,
        Submission.publish_status_id,
        Submission.is_fabs,
        Submission.number_of_warnings,
        Submission.number_of_errors,
        Submission.updated_at,
        Submission.reporting_start_date,
        Submission.reporting_end_date,
        Submission.publishing_user_id,
        Submission.reporting_fiscal_year,
        Submission.reporting_fiscal_period,
        Submission.is_quarter_format,
        Submission.published_submission_ids,
    ...
    user_columns = [
        User.user_id,
        User.name,
        publishing_user.user_id.label("publishing_user_id"),
    ...
    max_pub = (
        sess.query(PublishHistory.submission_id, func.max(PublishHistory.created_at).label("max_date"))
        .group_by(PublishHistory.submission_id)
        .cte("max_pub")
    )
    max_cert = (
        sess.query(CertifyHistory.submission_id, func.max(CertifyHistory.created_at).label("max_date"))
        .group_by(CertifyHistory.submission_id)
        .cte("max_cert")
    )
    pub_query = (
        sess.query(
            max_pub.c.submission_id,
            case(
                [(func.coalesce(max_cert.c.max_date, "1/1/1973") > max_pub.c.max_date, max_cert.c.max_date)],
                else_=max_pub.c.max_date,
            ).label("last_pub_or_cert"),
        )
        .outerjoin(max_cert, max_pub.c.submission_id == max_cert.c.submission_id)
        .cte("pub_query")
    )

    columns_to_query = (
        submission_columns + cgac_columns + frec_columns + user_columns + view_columns + [pub_query.c.last_pub_or_cert]
    )

    # Base query that is shared among all submission lists
    query = (
        sess.query(*columns_to_query)
    ...
    min_mod_query = (
        sess.query(func.min(submission_updated_view.updated_at).label("min_last_mod_date"))
    ...
    if sort == "agency":
        sort_order = case(
            [(FREC.agency_name.isnot(None), FREC.agency_name), (CGAC.agency_name.isnot(None), CGAC.agency_name)]
        )
        ...
        for file in file_history:
            ...
            if file.warning_filename is not None:
                ...
                if file.file_type_id:
                    ...
                    for replace_name, replace_letter in sorted(
                        list(FILE_TYPE_DICT_NAME_LETTER.items()), key=lambda ftype: ftype[1]
                    ):
...
def list_history(submission):
    ...
    if submission.is_fabs:
        return JsonResponse.error(
            ValueError("FABS submissions do not have a publication history"), StatusCode.CLIENT_ERROR
        )
    ...
    certify_history = (
        sess.query(CertifyHistory)
        .filter_by(submission_id=submission.submission_id)
        .order_by(CertifyHistory.created_at.desc())
        .all()
    )
    ...
    return JsonResponse.create(
        StatusCode.OK,
        {"submission_id": submission.submission_id, "publications": publications, "certifications": certifications},
    )
...
def file_history_url(file_history_id, is_warning, is_local, submission=None):
    ...
    if submission:
        if file_history.submission_id != submission.submission_id:
            return JsonResponse.error(
                ValueError("Requested published_files_history_id does not " "match submission_id provided"),
                StatusCode.CLIENT_ERROR,
            )
...
def list_published_files(sub_type, agency=None, year=None, period=None):
    ...
    if "type" in filters_provided:
        selects = [
            case([(FREC.frec_code.isnot(None), FREC.frec_code), (CGAC.cgac_code.isnot(None), CGAC.cgac_code)]).label(
                "agency_code"
            ),
            case(
                [(FREC.agency_name.isnot(None), FREC.agency_name), (CGAC.agency_name.isnot(None), CGAC.agency_name)]
    ...
    if "year" in filters_provided:
        ...
        filters += [
            (
                Submission.reporting_fiscal_year == str(year)
                if sub_type == "dabs"
                else case(
                    [
    ...
    if "period" in filters_provided:
        ...
        filters += [
            (
                Submission.reporting_fiscal_period == str(period)
                if sub_type == "dabs"
                else case(
                    [
    ...
    if filters_provided[-1] == "type":
        for result in query:
            results.append(
                {"id": result.agency_code, "label": "{} - {}".format(result.agency_code, result.agency_name)}
            )
        ...
        for result in query:
            if result.reporting_fiscal_period == 2 and sub_type == "dabs":
                ...
                period = "P{}/Q{}".format(
                    str(result.reporting_fiscal_period).zfill(2), int(result.reporting_fiscal_period / 3)
                )
            else:
...
def submission_report_url(submission, warning, file_type, cross_type):
    ...
    if cross_type:
        ...
        if file_type != cross_pairs[cross_type]:
            return JsonResponse.error(
                ValueError("{} and {} is not a valid cross-pair.".format(file_type, cross_type)),
                StatusCode.CLIENT_ERROR,
            )
...
def get_upload_file_url(submission, file_type):
    ...
    if CONFIG_BROKER["local"]:
        ...
        url = S3Handler().get_signed_url(
            split_name[0], split_name[1], url_mapping=CONFIG_BROKER["submission_bucket_mapping"]
        )
    return JsonResponse.create(StatusCode.OK, {"url": url})
