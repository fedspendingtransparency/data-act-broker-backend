...
from dataactvalidator.validation_handlers.file_generation_handler import (
    check_file_generation, start_d_generation, start_e_f_generation)
...
class FileHandler:
    ...
    def generate_file(self, submission, file_type, start, end, agency_type):
        ...
        if submission.d2_submission:
            error = ValueError("Cannot generate files for FABS submissions.")

        elif file_type in ['D1', 'D2']:
            # D file generation requires start and end date
            if not start or not end:
                error = ValueError("Must have a start and end date for D file generation.")
            # D files can only be generated by awarding or funding agency
            elif agency_type not in ['awarding', 'funding']:
                error = ValueError("agency_type must be either awarding or funding for D file generation.")

        # Only D1, D2, E, and F files can be generated
        elif file_type not in ['E', 'F']:
            error = ResponseException("File type must be either D1, D2, E, or F", StatusCode.CLIENT_ERROR)

        # Return any client errors
        if error:
            return JsonResponse.error(error, StatusCode.CLIENT_ERROR)
        ...
        if not check_generation_prereqs(submission.submission_id, file_type):
        ...
        try:
            if file_type in ['D1', 'D2']:
                response = start_d_generation(job, start, end, agency_type)
            else:
                response = start_e_f_generation(job)
    ...
    def generate_detached_file(self, file_type, cgac_code, frec_code, start, end, agency_type):
        ...
        try:
            response = start_d_generation(new_job, start, end, agency_type, agency_code=agency_code)
            log_data['message'] = 'SQS message response: {}'.format(response)
            logger.debug(log_data)
        except Exception as e:
