...
- `filters`: (dict) additional filters to narrow the results returned by the endpoint. Possible filters are:
    ...
    - `last_modified_range`: (dict) a start and end date for the last modified date range. If both are not provided, the search is unbounded in that direction (e.g. if the end date is not provided, it finds all submissions that have been modified since the start date)
