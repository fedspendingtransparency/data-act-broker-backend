...
def add_user_routes(app,system_email,bcrypt):
    ...
    def confirm():
        ...
        accountManager = AccountHandler(request,bcrypt = bcrypt)
        return RouteUtils.run_instance_function(accountManager, accountManager.createEmailConfirmation, RouteUtils.SYSTEM_EMAIL)
    ...
    def list_users():
        """ list all users """
        accountManager = AccountHandler(request,bcrypt = bcrypt)
        return RouteUtils.run_instance_function(accountManager, accountManager.list_users)
    ...
    def current_user():
        ...
        accountManager = AccountHandler(request,bcrypt = bcrypt)
        return RouteUtils.run_instance_function(accountManager, accountManager.get_current_user, session)
    ...
    def set_skip_guide():
        ...
        accountManager = AccountHandler(request,bcrypt = bcrypt)
        return RouteUtils.run_instance_function(accountManager, accountManager.set_skip_guide, session)
