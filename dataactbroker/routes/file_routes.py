...
from dataactbroker.handlers.fileHandler import (
    ...
    list_published_files as list_published_files_handler,
    get_upload_file_url,
...
from dataactbroker.handlers.submission_handler import (
    delete_all_submission_data,
    get_submission_stats,
    list_banners,
    check_current_submission_page,
    publish_dabs_submission,
    certify_dabs_submission,
    publish_and_certify_dabs_submission,
    get_published_submission_ids,
...
def add_file_routes(app, is_local, server_path):
    ...
    @parser.use_kwargs(
        {
            "type": webargs_fields.String(required=True, validate=webargs_validate.OneOf(("fabs", "dabs"))),
    ...
    @parser.use_kwargs({"published_files_history_id": webargs_fields.Int(required=True)}, location="query")
    @requires_login
    ...
    @requires_submission_perms("reader")
    @parser.use_kwargs(
        {"publish_history_id": webargs_fields.Int(), "certify_history_id": webargs_fields.Int()}, location="query"
    )
    ...
    @parser.use_kwargs(
        {
            ...
            "cross_type": webargs_fields.String(
                validate=webargs_validate.OneOf(["program_activity", "award_financial", "award_procurement", "award"])
            ),
        },
        location="query",
    )
    ...
    @parser.use_kwargs(
        {
            "file_type": webargs_fields.String(
                required=True, validate=webargs_validate.OneOf(FILE_TYPE_DICT_LETTER.values())
            )
        },
        location="query",
    )
    ...
    def get_published_submissions(reporting_fiscal_year, reporting_fiscal_period, **kwargs):
        ...
        return get_published_submission_ids(
            cgac_code, frec_code, reporting_fiscal_year, reporting_fiscal_period, is_quarter
        )
