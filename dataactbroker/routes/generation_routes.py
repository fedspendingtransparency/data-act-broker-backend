...
def add_generation_routes(app, is_local, server_path):
    ...
    @parser.use_kwargs(
        {
            ...
            "agency_type": webargs_fields.String(
                ...
                validate=webargs_validate.OneOf(
                    ("awarding", "funding"), error="Must be either awarding or funding if provided"
                ),
            ),
            "file_format": webargs_fields.String(
                load_default="csv",
                validate=webargs_validate.OneOf(("csv", "txt"), error="Must be either csv or txt if provided"),
            ),
        },
        location="json",
    )
    ...
    @parser.use_kwargs(
        {
            "file_type": webargs_fields.String(
                required=True, validate=webargs_validate.OneOf(("A", "D1", "D2", "BOC"))
            ),
            ...
            "period": webargs_fields.Int(
                validate=webargs_validate.OneOf(list(range(2, 13)), error="Period must be an integer 2-12.")
            ),
            "agency_type": webargs_fields.String(
                load_default="awarding",
                validate=webargs_validate.OneOf(
    ...
    def generate_detached_file(file_type, **kwargs):
        ...
        return generation_handler.generate_detached_file(
            file_type, cgac_code, frec_code, start, end, year, period, agency_type, file_format, element_numbers
        )
