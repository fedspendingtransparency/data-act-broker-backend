...
from dataactbroker.handlers.dashboard_handler import (
    historic_dabs_warning_table,
    list_rule_labels,
    historic_dabs_warning_graphs,
    active_submission_overview,
    active_submission_table,
    get_impact_counts,
...
def add_dashboard_routes(app):
    ...
    @use_kwargs(
        {
            ...
            "error_level": webargs_fields.String(
                validate=webargs_validate.OneOf(
                    ["warning", "error", "mixed"], error="Must be either warning, error, or mixed"
                ),
    ...
    @requires_login
    @use_kwargs({"filters": webargs_fields.Dict(keys=webargs_fields.String(), load_default={})}, location="json")
    ...
    @use_kwargs(
        {
            "file": webargs_fields.String(
                validate=webargs_validate.OneOf(
                    FILE_TYPES, error="Must be one of the following: {}".format(", ".join(FILE_TYPES))
                ),
                required=True,
            ),
            "error_level": webargs_fields.String(
