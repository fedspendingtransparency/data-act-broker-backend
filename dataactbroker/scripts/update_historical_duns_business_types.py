...
def main():
    ...
    for historic_duns in sess.query(HistoricDUNS).all():
        historic_duns.business_types = [SAM_BUSINESS_TYPE_DICT[type_code]
                                        for type_code in historic_duns.business_types_codes
                                        if type_code in SAM_BUSINESS_TYPE_DICT]
    sess.commit()
    sess.close()
    logger.info("Updating historical SAM recipients complete")
