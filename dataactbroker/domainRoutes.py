...
def add_domain_routes(app):
    ...
    def list_agencies(sub_tier_agencies):
        """ List all CGAC and FREC Agencies user has DABS permissions for
            Args:
            sub_tier_agencies - List of all SubTierAgencies that the user has DABS permissions for, generated by the
                get_dabs_sub_tier_agencies decorator, required to list only sub_tier_agencies that user can access
        """
        cgac_list, frec_list = [], []
    ...
    def list_sub_tier_agencies(sub_tier_agencies):
        """ List all Sub-Tier Agencies user has FABS permissions for
            Args:
            sub_tier_agencies - List of all SubTierAgencies that the user has FABS permissions for, generated by the
                get_fabs_sub_tier_agencies decorator, required to list only sub_tier_agencies that user can access
        """
        agencies = []
        for sub_tier in sub_tier_agencies:
...
def get_dabs_sub_tier_agencies(fn):
    """ Decorator which provides a list of all agencies the user has DABS permissions for. The function should have an
    agencies parameter as its first argument. """
    @wraps(fn)
    def wrapped(*args, **kwargs):
        ...
        agencies = cgac_sub_tier_agencies + frec_sub_tier_agencies

        return fn(agencies, *args, **kwargs)
    return wrapped
