...
def add_domain_routes(app):
    ...
    def list_agencies(cgac_sub_tiers, frec_sub_tiers):
        """ List all CGAC and FREC Agencies user has DABS permissions for
            ...
            frec_sub_tiers - List of all FREC SubTierAgencies generated by the get_dabs_sub_tier_agencies decorator, 
                required to list only sub_tier_agencies that user has DABS permissions for
        """
        cgac_list = [{'agency_name': sub_tier.cgac.agency_name, 'cgac_code': sub_tier.cgac.cgac_code, 
                      'priority': sub_tier.priority} for agency in cgac_sub_tiers]
        frec_list = [{'agency_name': sub_tier.frec.agency_name, 'frec_code': sub_tier.frec.frec_code, 
                      'priority': sub_tier.priority} for agency in frec_sub_tiers]
    ...
    def list_all_agencies():
        ...
        cgac_sub_tiers = sess.query(SubTierAgency).filter_by(is_frec=True).distinct(SubTierAgency.frec_id).all()
        agency_list = [{'agency_name': agency.cgac.agency_name, 'cgac_code': agency.cgac.cgac_code,
                        'priority': agency.priority} for agency in cgac_sub_tiers]

        # distinct SubTierAgency FRECs with a True is_frec
        frec_sub_tiers = sess.query(SubTierAgency).filter_by(is_frec=False).distinct(SubTierAgency.cgac_id).all()
        agency_list = [{'agency_name': agency.frec.agency_name, 'frec_code': agency.frec.frec_code,
                        'priority': agency.priority} for agency in frec_sub_tiers]
...
def separate_affiliations(affiliations, perm_type):
    cgac_ids, frec_ids = [], []
    
    for affil in g.user.affiliations:
        if (perm_type == 'dabs' and affil.permission_type_id == FABS_PERM) or (perm_type == 'fabs' and
            affil.permission_type_id >= WRITE_PERM and affil.permission_type_id != FABS_PERM):
            if affil.frec:
