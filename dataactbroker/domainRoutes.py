...
def add_domain_routes(app):
    ...
    def list_agencies(cgac_sub_tiers, frec_sub_tiers):
        """ List all CGAC and FREC Agencies user has DABS permissions for
            ...
            frec_sub_tiers - List of all FREC SubTierAgencies generated by the get_dabs_sub_tier_agencies decorator,
                required to list only sub_tier_agencies that user has DABS permissions for
        """
        print(cgac_sub_tiers)
        print(frec_sub_tiers)
        cgac_list = [{'agency_name': cst.cgac.agency_name, 'cgac_code': cst.cgac.cgac_code, 'priority': cst.priority}
                     for cst in cgac_sub_tiers]
        frec_list = [{'agency_name': fst.frec.agency_name, 'frec_code': fst.frec.frec_code, 'priority': fst.priority}
    ...
    def list_all_agencies():
        ...
        cgac_sub_tiers = sess.query(SubTierAgency).filter_by(is_frec=False).distinct(SubTierAgency.cgac_id).all()
        agency_list = [{'agency_name': cst.cgac.agency_name, 'cgac_code': cst.cgac.cgac_code, 'priority': cst.priority}
                       for cst in cgac_sub_tiers]

        # distinct SubTierAgency FRECs with a True is_frec
        frec_sub_tiers = sess.query(SubTierAgency).filter_by(is_frec=True).distinct(SubTierAgency.frec_id).all()
        shared_list = [{'agency_name': fst.frec.agency_name, 'frec_code': fst.frec.frec_code, 'priority': fst.priority}
                       for fst in frec_sub_tiers]
