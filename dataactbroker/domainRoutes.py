...
def add_domain_routes(app):
    ...
    def list_agencies(cgac_sub_tiers, frec_sub_tiers):
        """ List all CGAC and FREC Agencies user has DABS permissions for
            ...
            frec_sub_tiers - List of all FREC SubTierAgencies generated by the get_dabs_sub_tier_agencies decorator,
                required to list only sub_tier_agencies that user has DABS permissions for
        """
        # create list of CGACs without SubTierAgencies
        other_cgacs = sess.query(CGAC).filter(CGAC.cgac_id.notin_([st.cgac.cgac_id for st in cgac_sub_tiers])).all()

        # combine SubTierAgency CGACs with CGACs without SubTierAgencies into a cgac_list
        all_cgacs = [st.cgac for st in all_subs if st.is_frec is False] + [cgac for cgac in other_cgacs]
        cgac_list = [{'agency_name': cst.agency_name, 'cgac_code': cst.cgac_code} for cst in all_cgacs]

        # turn the list of frec sub_tier_agencies into a list of frec agencies
    ...
    def list_all_agencies():
        ...
        # add distinct CGACs from SubTierAgencies with a False is_frec to CGACS without SubTierAgencies and put them
        # into the agency_list
        cgac_sub_tiers = sess.query(SubTierAgency).distinct(SubTierAgency.cgac_id).all()
        other_cgacs = sess.query(CGAC).filter(CGAC.cgac_id.notin_([st.cgac.cgac_id for st in cgac_sub_tiers])).all()
        all_cgacs = [st.cgac for st in cgac_sub_tiers if st.is_frec is False] + [cgac for cgac in other_cgacs]
        ...
        frec_sub_tiers = sess.query(SubTierAgency).filter(SubTierAgency.is_frec.is_(True)).\
