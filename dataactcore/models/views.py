...
class LatestCertifyDateView:
    def __init__(self):
        """ Create the SubmissionUpdatedView """
        db = GlobalDB.db()
        metadata = MetaData(bind=db.engine)

        new_view = Table('latest_certify_date_view', metadata, autoload=True, info=dict(is_view=True))

        self.table = new_view

        self.submission_id = new_view.columns.submission_id
        self.certified_on = new_view.columns.certified_on
