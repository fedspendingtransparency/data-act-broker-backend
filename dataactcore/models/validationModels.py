...
class FileColumn(Base):
    ...
    file = relationship("FileType", backref="file_type")
    field_types_id = Column(Integer, ForeignKey("field_type.field_type_id"))
    field_type = relationship("FieldType", backref="field_type")
    name = Column(Text)
    description = Column(Text)
...
class Rule(Base):
    ...
    rule_type = relationship("RuleType", backref="rule_type")
    file_column = relationship("FileColumn", backref="file_columns")
