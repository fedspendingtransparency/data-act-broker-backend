...
class SF133(Base):
    ...
    tas = Column(Text, nullable=False, default=concat_tas, index=True)
    display_tas = Column(Text, default=concat_display_tas, index=True)
...
class CountryCode(Base):
    ...
    territory = Column(Boolean, nullable=False, default=False, server_default="False")
    free_state = Column(Boolean, nullable=False, default=False, server_default="False")
...
class GTASBOC(Base):
    ...
    fiscal_year = Column(Integer, nullable=False, index=True)
    period = Column(Integer, nullable=False, index=True)
    ussgl_number = Column(Text)
    dollar_amount = Column(Numeric)
    debit_credit = Column(Text)
    begin_end = Column(Text)
    authority_type = Column(Text)
    reimbursable_flag = Column(Text)
    apportionment_cat_code = Column(Text)
    apportionment_cat_b_prog = Column(Text)
    program_report_cat_number = Column(Text)
    federal_nonfederal = Column(Text)
    trading_partner_agency_ide = Column(Text)
    trading_partner_mac = Column(Text)
    year_of_budget_auth_code = Column(Text)
    availability_time = Column(Text)
    bea_category = Column(Text)
    borrowing_source = Column(Text)
    exchange_or_nonexchange = Column(Text)
    custodial_noncustodial = Column(Text)
    budget_impact = Column(Text)
    prior_year_adjustment_code = Column(Text)
    credit_cohort_year = Column(Integer)
    disaster_emergency_fund_code = Column(Text, index=True)
