...
GROUP_MAPPINGS_FLAT = dict(list(itertools.chain.from_iterable([[(defc, group_name) for defc in defc_list]
                                                               for group_name, defc_list in GROUP_MAPPINGS.items()])))
...
def add_defc_outliers(defc_df):
    ...
    defc_9_df = pd.DataFrame([{'DEFC': '9', 'Public Law': [defc_9_title], 'Public Law Short Title': [defc_9_title],
                               'Is Valid': False, 'URLs': []}])
    ...
    defc_qqq_df = pd.DataFrame([{'DEFC': 'QQQ', 'Public Law': [defc_qqq_title],
                                 'Public Law Short Title': [defc_qqq_title], 'Is Valid': True, 'URLs': []}])
...
def load_defc(base_path, force_reload=False):
    ...
    with create_app().app_context():
        ...
        if force_reload or diff_found:
            ...
            header_order = ['DEFC', 'Public Law', 'Public Law Short Title', 'Group Name', 'URLs', 'Is Valid',
