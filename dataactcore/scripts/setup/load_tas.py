...
def clean_tas(csv_path, metrics=None):
    ...
    """
    if not metrics:
        metrics = {"records_provided": 0, "duplicates_dropped": 0}
        ...
        if not backfill_historic:
            ...
            tas_files = [
                s3connection.generate_presigned_url(
                    ...
                    Params={"Bucket": CONFIG_BROKER["sf_133_bucket"], "Key": "cars_tas.csv"},
                    ExpiresIn=600,
                )
            ]
        else:
            tas_files = s3connection.list_object_versions(Bucket=CONFIG_BROKER["sf_133_bucket"], Prefix="cars_tas.csv")[
                "Versions"
            ]
...
def add_existing_id(data):
    ...
    query = (
        GlobalDB.db().session.query(TASLookup).filter(TASLookup.account_num.in_(int(i) for i in data["account_num"]))
    )
    for tas in query:
