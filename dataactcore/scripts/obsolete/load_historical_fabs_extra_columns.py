...
def parse_fabs_file_new_columns(f, sess):
    ...
    column_header_mapping = {
        ...
        "federal_award_id": 2,
        "uri": 3,
        "awarding office code": 4,
        "awarding office name": 5,
        "funding office name": 6,
        "funding office code": 7,
        "funding agency name": 8,
        "funding agency code": 9,
        "funding sub tier agency code": 10,
        "funding sub tier agency name": 11,
        "legal entity foreign city": 12,
        "legal entity foreign province": 13,
        "legal entity foreign postal code": 14,
        "legal entity foreign location description": 15,
    }
            ...
            with zip_file.open(csv_file) as dat_file:
                ...
                if cdata is not None:
                    for _, row in cdata.iterrows():
                        sess.query(PublishedFABS).filter(
                            func.upper(PublishedFABS.afa_generated_unique) == row["afa_generated_unique"].upper()
                        ).update(
                            {
                                "awarding_office_code": row["awarding_office_code"],
...
def generate_unique_string(row):
    ...
    # TODO: if we ever need this script again, we have to check if they give us assistance listing number
    #       and add it here
...
def main():
    ...
    if CONFIG_BROKER["use_aws"]:
        ...
        new_columns_file = s3_client.generate_presigned_url(
            "get_object",
            {"Bucket": CONFIG_BROKER["archive_bucket"], "Key": "Assistance_DataActFields_2017.csv"},
            ExpiresIn=600,
        )
        parse_fabs_file_new_columns(urllib.request.urlopen(new_columns_file), sess)
    else:
        new_columns_file = os.path.join(
            CONFIG_BROKER["path"], "dataactvalidator", "config", "fabs", "Assistance_DataActFields_2017.csv"
        )
