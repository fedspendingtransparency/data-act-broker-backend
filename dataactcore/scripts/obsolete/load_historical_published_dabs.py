...
def filenames_by_file_type(file_type_id):
    ...
    published_ids = (
        sess.query(func.max(PublishedFilesHistory.published_files_history_id).label("most_recent_id"))
        .filter_by(file_type_id=file_type_id)
        .group_by(PublishedFilesHistory.submission_id)
        .cte("published_ids")
    )

    return sess.query(PublishedFilesHistory.filename, PublishedFilesHistory.submission_id).join(
        published_ids, published_ids.c.most_recent_id == PublishedFilesHistory.published_files_history_id
    )
...
def insert_from_table(file_type_id, submission_id):
    ...
    sess = GlobalDB.db().session
    logger.info(
        "Copying submission {} data from base table into {}".format(submission_id, FTI_TABLENAME_DICT[file_type_id])
    )
...
def insert_file(filename, submission_id, file_type_id, csv_schema, long_to_short_dict):
    ...
    if "disaster_emergency_fund_code" not in data and file_type_id in (
    ...
    job = (
        sess.query(Job)
        .filter_by(
            submission_id=submission_id, file_type_id=file_type_id, job_type_id=JOB_TYPE_DICT["csv_record_validation"]
        )
        .one()
    )
...
def main():
    ...
    long_to_short_dict = {
        FILE_TYPE_DICT["appropriations"]: {
            "budgetauthorityavailableamounttotal_cpe": "total_budgetary_resources_cpe",
