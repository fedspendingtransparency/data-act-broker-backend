...
def get_delete_file():
    ...
    if CONFIG_BROKER["use_aws"]:
        reader = CsvReader()
        pa_file = open(
            reader.get_filename(CONFIG_BROKER["aws_region"], CONFIG_BROKER["sf_133_bucket"], file_name),
            encoding="utf-8",
        )
    else:
...
def get_deletes(sess, data):
    ...
    for index, row in data.iterrows():
        ...
        existing_item = (
            sess.query(model.last_modified, model.detached_award_procurement_id, model.detached_award_proc_unique)
            .filter_by(detached_award_proc_unique=unique_string)
            .one_or_none()
        )
...
def main():
    ...
    logger.info(
        "Finished gathering records in {} seconds. Total records to delete: {}".format(
            gather_end - gather_start, len(delete_list)
        )
    )
    # Delete records
