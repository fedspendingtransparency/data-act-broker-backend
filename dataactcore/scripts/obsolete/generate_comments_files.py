...
if __name__ == "__main__":
    ...
    with create_app().app_context():
        ...
        for submission in commented_submissions:
            ...
            unpublished_query = (
                sess.query(FileType.name, Comment.comment)
        ...
        for published_submission in commented_pub_submissions:
            ...
            existing_pub_history = (
                sess.query(PublishedFilesHistory)
                .filter(
                    PublishedFilesHistory.submission_id == submission_id,
                    PublishedFilesHistory.filename.like("%" + filename),
                )
                .one_or_none()
            )
            ...
            max_pub_id = (
                sess.query(func.max(PublishHistory.publish_history_id).label("pub_id"))
                .filter_by(submission_id=submission_id)
                .one()
            )
            max_cert_id = (
                sess.query(func.max(CertifyHistory.certify_history_id).label("cert_id"))
                .filter_by(submission_id=submission_id)
                .one()
            )
