...
def fix_fabs_le_country(row):
    ...
    if (
        not row.legal_entity_country_name
        and row.legal_entity_country_code
...
def fix_fabs_ppop_country(row):
    ...
    # grab the country name if we have access to it and it isn't already there
    if (
        not row.place_of_perform_country_n
        and row.place_of_perform_country_c
...
def fix_fabs_le_state(row):
    ...
    if (
        not row.legal_entity_state_name
        and row.legal_entity_state_code
...
def fix_fabs_ppop_state(row):
    ...
    if (
        not row.place_of_perform_state_nam
        and row.place_of_perfor_state_code
...
def fix_fabs_le_county(row, zip_data, zip_check):
    ...
    if not row.legal_entity_county_code:
        if (
            row.record_type == 1
            and row.place_of_performance_code
            and re.match(r"^([A-Z]{2}|\d{2})\*\*\d{3}$", row.place_of_performance_code.upper())
        ):
...
def update_historical_fabs(sess, start, end):
    ...
    while True:
        ...
        query_result = (
            ...
            .filter(PublishedFABS.published_fabs_id <= end_slice)
            .all()
        )
...
def fix_fpds_ppop_country(row):
    ...
    # grab the country name if we have access to it and it isn't already there
    if (
        not row.place_of_perf_country_desc
        and row.place_of_perform_country_c
        and row.place_of_perform_country_c.upper() in g_country_list
    ):
...
def fix_fpds_le_state(row):
    ...
    if (
        not row.legal_entity_state_descrip
        and row.legal_entity_state_code
        and row.legal_entity_state_code.upper() in g_state_by_code
    ):
...
def fix_fpds_ppop_county(row):
    ...
    if (
        not row.place_of_perform_county_na
        and state_code in g_county_by_code
...
def update_historical_fpds(sess, start, end):
    ...
    while True:
        ...
        query_result = (
            ...
            .filter(model.detached_award_procurement_id <= end_slice)
            .all()
        )
        found_records += len(query_result)
...
def main():
    ...
    parser.add_argument(
        "-t", "--type", help="Which data type, argument must be fpds or fabs", nargs=1, type=str, required=True
    )
    ...
    while True:
        ...
        zip_codes = (
            sess.query(Zips.zip5, Zips.zip_last4, Zips.state_abbreviation, Zips.county_number)
