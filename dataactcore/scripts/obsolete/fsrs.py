...
class ControlFilter(MessagePlugin):
    ...
    def received(self, context):
        ...
        without_controls = "".join(char if not self.is_control(char) else " " for char in with_controls)
...
def new_client(service_type):
    ...
    if config.get("username") and config.get("password"):
        options["transport"] = WellBehavedHttpsTransport(
            username=config["username"], password=config["password"], timeout=300
        )
...
def to_prime_contract(soap_dict):
    ...
    debug_dict = {
        "id": model_attrs["id"],
        "internal_id": model_attrs["internal_id"],
