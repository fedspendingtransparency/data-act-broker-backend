...
def move_published_error_metadata(sess):
    ...
    sess.query(PublishedErrorMetadata).filter(PublishedErrorMetadata.job_id.in_(published_job_list)).delete(
...
def move_updated_error_metadata(sess):
    ...
    sess.query(PublishedErrorMetadata).filter(PublishedErrorMetadata.job_id.in_(updated_job_list)).delete(
        synchronize_session=False
    )
    ...
    max_publish_history = (
        sess.query(
            func.max(PublishHistory.publish_history_id).label("max_publish_id"),
        ...
        .cte("max_publish_history")
    )

    # Get the publish history associated with all of the warning files
    publish_history_list = (
        sess.query(
            PublishedFilesHistory.publish_history_id,
        ...
        .filter(PublishedFilesHistory.warning_filename.isnot(None))
        .order_by(PublishedFilesHistory.submission_id)
        .distinct()
    )
    ...
    for publish_history in publish_history_list:
        ...
        if not warning_df.empty:
            ...
            if "cross_warning" in warning_file_path:
                ...
                relevant_job = (
                    sess.query(Job)
                    .filter_by(submission_id=publish_history.submission_id, job_type_id=JOB_TYPE_DICT["validation"])
                    .one()
                )
                ...
                file_type_match = re.match(
                    "submission_{}_(.+)_warning_report.csv".format(publish_history.submission_id), file_name
                )
                ...
                relevant_job = (
                    ...
                    .filter_by(
                        ...
                        file_type_id=FILE_TYPE_DICT[file_type],
                    )
                    .one()
                )
