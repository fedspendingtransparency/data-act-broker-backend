...
def update_keys(model, model_type, key_type, filter_content, update_years, concat_list):
    ...
    if update_years:
        ...
        base_query.filter(func.cast_as_date(model.action_date) < "{}-01-01".format(years[0])).update(
            {update_key: concat_list}, synchronize_session=False
        )
        sess.commit()
        ...
        for year in update_years:
            ...
            base_query.filter(
                func.cast_as_date(model.action_date) >= "{}-01-01".format(year),
        ...
        base_query.filter(func.cast_as_date(model.action_date) > "{}-12-31".format(years[-1])).update(
            {update_key: concat_list}, synchronize_session=False
        )
        sess.commit()
    else:
...
if __name__ == "__main__":
    with create_app().app_context():
        ...
        parser.add_argument(
            "-kt",
            "--key_type",
            help="Which type of key to populate, award or transaction",
            required=True,
            choices=["award", "transaction"],
        )
            ...
            if key_type == "award":
                ...
                if "award" in types:
                    update_keys(
                        ...
                        func.concat(
                            "CONT_AWD_",
                            func.coalesce(dap.piid, "-none-"),
                            "_",
                            func.coalesce(dap.agency_id, "-none-"),
                            "_",
                            func.coalesce(dap.parent_award_id, "-none-"),
                            "_",
                ...
                if "IDV" in types:
                    update_keys(
                        ...
                        years,
                        func.concat(
                            "CONT_IDV_", func.coalesce(dap.piid, "-none-"), "_", func.coalesce(dap.agency_id, "-none-")
                        ),
                    )
            else:
                # transactions
                if "award" in types:
                    update_keys(
                        ...
                        func.concat(
                            ...
                            func.coalesce(dap.piid, "-none-"),
                            "_",
                            func.coalesce(dap.award_modification_amendme, "-none-"),
                            "_",
                            func.coalesce(dap.parent_award_id, "-none-"),
                            "_",
                            func.coalesce(dap.transaction_number, "-none-"),
                            "_",
                        ),
                    )
                # IDV
            ...
            if key_type == "award":
                ...
                update_keys(
                    ...
                    func.concat(
                        ...
                        func.coalesce(FABS.uri, "-none-"),
                        "_",
                        func.coalesce(FABS.assistance_listing_number, "-none-"),
                        "_",
            ...
            if key_type == "award":
                ...
                if "AGG" in types:
                    update_keys(
                        ...
                        func.concat(
                            "ASST_AGG_",
                            func.coalesce(PublishedFABS.uri, "-none-"),
                            "_",
                ...
                if "NON" in types:
                    update_keys(
                        ...
                        func.concat(
                            "ASST_NON_",
                            func.coalesce(PublishedFABS.fain, "-none-"),
                            "_",
                            func.coalesce(PublishedFABS.awarding_sub_tier_agency_c, "-none-"),
                        ),
                    )
                ...
                update_keys(
                    ...
                    func.concat(
                        ...
                        func.coalesce(PublishedFABS.uri, "-none-"),
                        "_",
                        func.coalesce(PublishedFABS.assistance_listing_number, "-none-"),
                        "_",
