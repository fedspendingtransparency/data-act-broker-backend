...
def format_fabs_data(data, sess, fips_state_list, state_code_list, sub_tier_list, county_code_list):
    ...
    data["place_of_perform_city"] = data.apply(
        lambda x: derive_place_of_performance_city(x, sess, fips_state_list, state_code_list), axis=1
    )
    logger.info("Starting derive_legal_entity_state_name")
    data["legal_entity_state_name"] = data.apply(
        lambda x: derive_legal_entity_state_name(x, sess, fips_state_list, state_code_list), axis=1
    )
...
def derive_legal_entity_city_code(row, sess):
    ...
    if (
        row["recipient_city_name"]
        and row["recipient_state_code"]
        and not pd.isnull(row["recipient_city_name"])
        and not pd.isnull(row["recipient_state_code"])
    ):
        city_data = (
            sess.query(CityCode.city_code)
            .filter(
                func.lower(CityCode.feature_name) == func.lower(row["recipient_city_name"].strip()),
                func.lower(CityCode.state_code) == func.lower(row["recipient_state_code"].strip()),
            )
            .first()
        )
        if city_data:
...
def derive_place_of_perform_county_na(row, sess, fips_state_list, state_code_list, county_code_list):
    if not row["principal_place_zip"] or pd.isnull(row["principal_place_zip"]):
        ...
        if re.match(r"^([A-Z]{2}|\d{2})\*\*\d{3}$", ppop_code):
            ...
            city_info = (
                sess.query(CityCode.county_name)
                .filter_by(city_code=city_code, state_code=ppop_state.state_code)
                .first()
            )
            if not city_info:
                return None
    ...
    if zip_four:
        zip_info = (
            sess.query(Zips.state_abbreviation, Zips.county_number).filter_by(zip5=zip_five, zip_last4=zip_four).first()
        )
    else:
        zip_info = sess.query(Zips.state_abbreviation, Zips.county_number).filter_by(zip5=zip_five).first()
    if not zip_info:
        return None
    county_info = (
        sess.query(CountyCode.county_name).filter_by(county_number=zip_info[0], state_code=zip_info[1]).first()
    )
    if county_info:
...
def derive_place_of_performance_city(row, sess, fips_state_list, state_code_list):
    if not row["principal_place_zip"] or pd.isnull(row["principal_place_zip"]):
        ...
        if re.match(r"^([A-Z]{2}|\d{2})\d{5}$", ppop_code) and not re.match(r"^([A-Z]{2}|\d{2})0{5}$", ppop_code):
            ...
            city_code = ppop_code[-5:]
            city_info = (
                sess.query(CityCode.feature_name)
                .filter_by(city_code=city_code, state_code=ppop_state.state_code)
                .first()
            )
            if not city_info:
...
def derive_legal_entity_state_name(row, sess, fips_state_list, state_code_list):
    if row["legal_entity_zip5"] and not pd.isnull(row["legal_entity_zip5"]):
        ...
        if row["legal_entity_zip_last4"] and not pd.isnull(row["legal_entity_zip_last4"]):
            zip_data = (
                sess.query(Zips.state_abbreviation)
                .filter_by(zip5=row["legal_entity_zip5"], zip_last4=row["legal_entity_zip_last4"])
                .first()
            )
...
def main():
    ...
    if CONFIG_BROKER["use_aws"]:
        ...
        for obj in file_list.get("Contents", []):
            if re.match(
                rf"^({years})_All_(DirectPayments|Grants|Insurance|Loans|Other)_Full_\d{{{8}}}.csv.zip", obj["Key"]
            ):
                file_path = s3_client.generate_presigned_url(
                    "get_object", {"Bucket": CONFIG_BROKER["archive_bucket"], "Key": obj["Key"]}, ExpiresIn=600
                )
                parse_fabs_file(
        ...
        for file in file_list:
            if re.match(rf"^({years})_All_(Grants|DirectPayments|Insurance|Loans|Other)_Full_\d{{{8}}}.csv.zip", file):
                parse_fabs_file(
                    open(os.path.join(base_path, file)),
                    sess,
                    fips_state_list,
                    state_code_list,
                    sub_tier_list,
