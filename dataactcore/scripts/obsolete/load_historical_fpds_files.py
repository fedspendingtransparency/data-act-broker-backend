...
def format_fpds_data(data, sub_tier_list, naics_data):
    ...
    type_to_description = {
        ...
        "davisbaconact": {"Y": "YES", "N": "NO", "X": "NOT APPLICABLE"},
        "fedbizopps": {"Y": "YES", "N": "NO", "X": "NOT APPLICABLE"},
        "fundedbyforeignentity": {"A": "FOREIGN FUNDS FMS", "B": "FOREIGN FUNDS NON-FMS", "X": "NOT APPLICABLE"},
        "lettercontract": {"A": "LETTER CONTRACT", "B": "OTHER UNDEFINITIZED ACTION", "X": "NO"},
        "research": {
        ...
        "servicecontractact": {"Y": "YES", "N": "NO", "X": "NOT APPLICABLE"},
        "statutoryexceptiontofairopportunity": {
            "URG": "URGENCY",
    ...
    award_contract_type_mappings = {
        "BPA Call Blanket Purchase Agreement": "A",
        "PO Purchase Order": "B",
        "DO Delivery Order": "C",
        "DCA Definitive Contract": "D",
    }
    award_contract_desc_mappings = {
        ...
        "DCA Definitive Contract": "DEFINITIVE CONTRACT",
    }
    idv_type_mappings = {
    ...
    data["contract_award_type_desc"] = data.apply(
        lambda x: map_type_description(x, award_contract_desc_mappings), axis=1
    )
    ...
    data["pulled_from"] = data.apply(
        lambda x: map_pulled_from(x, award_contract_type_mappings, idv_type_mappings), axis=1
    )
    ...
    data["awarding_agency_code"] = data.apply(
        lambda x: map_agency_code(x, "contractingofficeagencyid", sub_tier_list), axis=1
    )
    data["awarding_agency_name"] = data.apply(
        lambda x: map_agency_name(x, "contractingofficeagencyid", sub_tier_list), axis=1
    )
    data["funding_agency_code"] = data.apply(
        lambda x: map_agency_code(x, "fundingrequestingagencyid", sub_tier_list), axis=1
    )
    data["funding_agency_name"] = data.apply(
        lambda x: map_agency_name(x, "fundingrequestingagencyid", sub_tier_list), axis=1
    )
    data["referenced_idv_agency_desc"] = data.apply(
        lambda x: map_sub_tier_name(x, "idvagencyid", sub_tier_list), axis=1
    )
...
def map_legal_entity_state_code(row):
    ...
    if row["vendorcountrycode"] and (
        str(row["vendorcountrycode"]).upper() == "USA" or str(row["vendorcountrycode"]).upper() == "UNITED STATES"
    ):
...
def map_legal_entity_state_descrip(row):
    ...
    if not row["vendorcountrycode"] or (
        str(row["vendorcountrycode"]).upper() != "USA" and str(row["vendorcountrycode"]).upper() != "UNITED STATES"
    ):
        return str(row["vendor_state_code"])
    return None
...
def main():
    ...
    parser.add_argument(
        "-sf", "--subfolder", help="Used to indicate which Subfolder to load files from", nargs="+", type=str
    )
