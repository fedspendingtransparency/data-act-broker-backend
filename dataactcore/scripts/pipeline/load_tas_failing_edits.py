...
def get_tas_failing_edits_file_list(failed_tas_path, aws_prefix="GTAS_FE_DA"):
    ...
    if failed_tas_path is not None:
        ...
        failed_tas_list = [FailedTASFile(failed_tas, os.path.basename(failed_tas)) for failed_tas in failed_tas_files]
    else:
...
def load_tas_failing_edits_file(sess, filename, fiscal_year, fiscal_period, force_load=False, metrics=None):
    ...
    existing_records = sess.query(TASFailedEdits).filter(
        TASFailedEdits.fiscal_year == fiscal_year, TASFailedEdits.period == fiscal_period
    )
    ...
    if existing_records.count() > 0:
        if force_load:
            ...
            logger.info(
                "Failed TAS %s %s already in database (%s records). Skipping file.",
                fiscal_year,
                fiscal_period,
...
def load_all_tas_failing_edits(failed_tas_path=None, force_load=False, aws_prefix="GTAS_FE_DA"):
    ...
    with create_app().app_context():
        ...
        for failed_tas in failed_tas_list:
            ...
            load_tas_failing_edits_file(
                sess,
                failed_tas.full_file,
