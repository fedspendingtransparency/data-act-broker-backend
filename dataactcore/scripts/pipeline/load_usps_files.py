...
def get_login_tokens(login_response_body_dict):
    ...
    """
    return {"logonkey": login_response_body_dict["logonkey"], "tokenkey": login_response_body_dict["tokenkey"]}
    ...
    if args.download:
        if not last_load_date_obj:
            new_external_data_load_date = ExternalDataLoadDate(
                last_load_date=file_fulfilled_date, external_data_type_id=EXTERNAL_DATA_TYPE_DICT["usps_download"]
            )
