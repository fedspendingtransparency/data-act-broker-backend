...
def award_id_values(data, obj):
    ...
    value_map = {
        "agencyID": "referenced_idv_agency_iden",
...
def competition_values(data, obj):
    ...
    value_map = {
        "A76Action": "a_76_fair_act_action_desc",
...
def total_dollar_values_values(data, obj):
    ...
    value_map = {
        "totalBaseAndAllOptionsValue": "potential_total_value_awar",
...
def legislative_mandates_values(data, obj):
    ...
    # get descriptions for things in the value map
    value_map = {
        "ClingerCohenAct": "clinger_cohen_act_pla_desc",
...
def place_of_performance_values(data, obj):
    ...
    value_map = {
        "placeOfPerformanceCongressionalDistrict": "place_of_performance_congr",
    ...
    value_map = {"stateCode": "place_of_performance_state", "countryCode": "place_of_perform_country_c"}
    ...
    # get descriptions for things in the value map
    value_map = {"countryCode": "place_of_perf_country_desc", "stateCode": "place_of_perfor_state_desc"}
...
def contract_marketing_data_values(data, obj):
    ...
    value_map = {
        "feePaidForUseOfService": "fee_paid_for_use_of_serv",
...
def vendor_site_details_values(data, obj):
    ...
    value_map = {
        "isFederalGovernment": "us_federal_government",
    ...
    value_map = {"DUNSNumber": "awardee_or_recipient_uniqu", "globalParentDUNSNumber": "ultimate_parent_unique_ide"}
    ...
    value_map = {
        "domesticParentUEI": "domestic_parent_uei",
    ...
    value_map = {
        "isAsianPacificAmericanOwnedBusiness": "asian_pacific_american_own",
...
def calculate_ppop_fields(obj, sess, county_by_name, county_by_code, state_code_list, country_list):
    ...
    if obj["place_of_perform_country_c"] in country_code_map:
        ...
        if (
            obj["place_of_performance_state"]
            and not obj["place_of_perfor_state_desc"]
        ...
        if (
            not obj["place_of_perform_county_na"]
            and obj["place_of_performance_state"] in county_by_code
            and obj["place_of_perform_county_co"] in county_by_code[obj["place_of_performance_state"]]
        ):
            obj["place_of_perform_county_na"] = county_by_code[obj["place_of_performance_state"]][
                obj["place_of_perform_county_co"]
            ]
...
def calculate_legal_entity_fields(obj, sess, county_by_code, state_code_list, country_list):
    ...
    if obj["legal_entity_country_code"] in country_code_map:
        ...
        if (
            ...
            and not obj["legal_entity_state_descrip"]
    ...
    if obj["funding_sub_tier_agency_co"]:
        try:
            ...
            logger.info(
                ...
                obj["funding_sub_tier_agency_na"],
            )
...
def process_data(
    ...
    obj = calculate_remaining_fields(
        obj,
        sess,
        sub_tier_list,
        county_by_name,
...
def create_processed_data_list(
    data,
    contract_type,
    sess,
    sub_tier_list,
    ...
    data_list = []
    for value in data:
        tmp_obj = process_data(
            value["content"][contract_type],
            sess,
            atom_type=contract_type,
            sub_tier_list=sub_tier_list,
            county_by_name=county_by_name,
...
def add_processed_data_list(data, sess):
    try:
        ...
        for fpds_obj in data:
            insert_statement = (
                insert(DetachedAwardProcurement)
                .values(**fpds_obj)
    ...
    if threaded:
        ...
        for value in data:
            ...
            try:
                ...
                sess.rollback()
                tmp_obj["updated_at"] = now
                sess.query(DetachedAwardProcurement).filter_by(
                    detached_award_proc_unique=tmp_obj["detached_award_proc_unique"]
...
def get_total_expected_records(base_url):
    ...
    if final_request_count >= FPDS_MAX_RETURNED_ENTRIES:
        raise Exception(
            f"Max start count of {final_request_count} exceeds the maximum number of entries FPDS"
...
def get_data(
    ...
    while True:
        ...
        if entries_processed > total_expected_records:
            ...
            raise Exception(
                "Total number of expected records has changed\nExpected: {}\nRetrieved so far: {}".format(
                    total_expected_records, len(data)
                )
            )
        ...
        if data:
            ...
            logger.info(
                "Retrieved %s lines of get %s: %s feed, writing next %s to DB",
                entries_processed,
                contract_type,
                award_type,
                len(data),
            ...
            logger.info(
                "Successfully inserted %s lines of get %s: %s feed, continuing feed retrieval",
                len(data),
                contract_type,
                award_type,
            )
        ...
        if len(data) < (MAX_ENTRIES * MAX_REQUESTS_AT_ONCE):
            ...
            if entries_processed != total_expected_records:
                raise Exception(
                    "Records retrieved != Total expected records\nExpected: {}\nRetrieved: {}".format(
...
def get_delete_data(contract_type, now, sess, last_run, start_date=None, end_date=None, metrics=None):
    ...
    while True:
        ...
        try:
            ...
            if exception_retries < len(retry_sleep_times):
                logger.info(
                    "Connection exception caught. Sleeping {}s and then retrying with a max wait of {}s...".format(
        ...
        if processed_deletions > total_expected_records:
            raise Exception(
                "Total number of expected records has changed\nExpected: {}\nRetrieved so far: {}".format(
                    total_expected_records, len(processed_deletions)
                )
            )
        ...
        if len(listed_data) < 10:
            ...
            if processed_deletions != total_expected_records:
                raise Exception(
                    "Records retrieved != Total expected records\nExpected: {}\nRetrieved: {}".format(
                        total_expected_records, len(listed_data)
                    )
                )
            else:
    ...
    for value in data:
        ...
        existing_item = (
            sess.query(DetachedAwardProcurement).filter_by(detached_award_proc_unique=unique_string).one_or_none()
        )
    ...
    if delete_list:
        if "records_deleted" not in metrics:
            ...
            metrics["records_deleted"] += len(delete_list)
        sess.query(DetachedAwardProcurement).filter(
            DetachedAwardProcurement.detached_award_procurement_id.in_(delete_list)
...
def create_lookups(sess):
    ...
    county_codes = sess.query(
        CountyCode.county_number, CountyCode.state_code, func.upper(CountyCode.county_name).label("county_name")
    ).all()
    ...
    for recipient in sam_recipient_list:
        exec_comp_dict[recipient.uei.upper()] = {
            "officer1_name": recipient.high_comp_officer1_full_na,
            "officer1_amt": recipient.high_comp_officer1_amount,
            "officer2_name": recipient.high_comp_officer2_full_na,
            "officer2_amt": recipient.high_comp_officer2_amount,
            "officer3_name": recipient.high_comp_officer3_full_na,
            "officer3_amt": recipient.high_comp_officer3_amount,
            "officer4_name": recipient.high_comp_officer4_full_na,
            "officer4_amt": recipient.high_comp_officer4_amount,
            "officer5_name": recipient.high_comp_officer5_full_na,
            "officer5_amt": recipient.high_comp_officer5_amount,
        }
...
def main():
    ...
    parser.add_argument(
        "-d", "--delivery", help="Used in conjunction with -a to indicate delivery order feed", action="store_true"
    )
    parser.add_argument(
        "-o",
        "--other",
    ...
    if args.all:
        if (not args.delivery and not args.other) or (args.delivery and args.other):
            logger.error(
                "When using the -a flag, please include either -d or -o "
