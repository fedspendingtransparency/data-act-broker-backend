...
def load_offices(sess, filename, update_db, pull_all, updated_date_from, export_office, metrics):
    ...
    if filename:
        ...
        file_headers = [
            ...
            "fhorgaddresslist_0_city",
            "fhorgaddresslist_0_state",
            "fhorgaddresslist_0_country_code",
            "fhorgaddresslist_0_addresstype",
            "fhorgnamehistory_0_fhorgname",
...
def parse_raw_office(org):
    ...
    effective_end_date = (
        org.get("effectiveenddate") if org["status"] == "ACTIVE" else org.get("effectiveenddate") or "2000-01-02 00:00"
    )
...
def main():
    ...
    if not args.all and not updated_date_from:
        last_pull_date = (
            sess.query(ExternalDataLoadDate.last_load_date_start)
            .filter_by(external_data_type_id=EXTERNAL_DATA_TYPE_DICT["office"])
            .one_or_none()
        )
