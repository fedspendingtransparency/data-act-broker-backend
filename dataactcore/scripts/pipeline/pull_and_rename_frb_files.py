...
By default, does nothing if there was no S3 files found in the past 24 hours, can be forced to pull the most recent.
"""
...
def check_for_new_file(recent_files, bucket_source, data_source):
    ...
    found_files = [
        re.search(data_source.old_filename, x) for x in recent_files if re.search(data_source.old_filename, x)
    ]
    if found_files:
        pulled = True
...
def main():
    ...
    parser.add_argument(
        "--force_pull_latest", action="store_true", help="Pulls the most recent file," " regardless of when it came in."
    )
    parser.add_argument(
        "--data_types",
        nargs="+",
        choices=DATA_TYPES_NAMES + ["all"],
    ...
    if args.force_pull_latest:
        ...
        recent_files = [
            x
            for x in all_files
            if (datetime.datetime.utcnow() - x.last_modified.replace(tzinfo=None) < datetime.timedelta(hours=24))
        ]
    ...
    for data_type in data_types:
        ...
        if data_source.name == "cars" and new_file:
            ...
            data["Financial Indicator Type 2"] = data.apply(
                lambda x: replace_null(x, "Financial Indicator Type 2"), axis=1
            )
