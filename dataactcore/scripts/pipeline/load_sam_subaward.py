...
from dataactbroker.helpers.script_helper import (
...
def load_subawards(
    ...
    params = {"pageSize": LIMIT, "status": load_type.capitalize()}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--data_type",
        help="Which API (assistance, contract, or both) to load." " Defaults to both.",
        required=False,
        default="both",
        choices=["assistance", "contract", "both"],
    )
    parser.add_argument(
        "-l",
        "--load_type",
        help="Which status (published, deleted, or both) to load." " Defaults to both.",
        required=False,
        default="both",
        choices=["published", "deleted", "both"],
    )
    parser.add_argument(
        "--start_date",
    ...
    with create_app().app_context():
        ...
        if not args.load_missing:
            ...
            if args.award_id and args.data_type == "both":
                raise ValueError(
                    "When using award_id argument, specify data_type must be assistance or contract, not both."
                )
            elif args.award_id:
                ...
                for load_type in load_types:
                    if args.unlinked and load_type != "deleted":
                        ...
                        for unlinked_id in unlinked_ids:
                            ...
                            logger.info(
                                f"Loaded SAM Subaward reports for {data_type} with award_id {unlinked_id.award_id}"
                            )
                    else:
        ...
        for loader_portion, report_nums in pulled_report_nums.items():
            ...
            if load_type != "deleted":
                ...
                populate_subaward_table(
                    sess,
                    data_type,
                    min_date=start_ingestion_datetime if not args.load_missing else None,
