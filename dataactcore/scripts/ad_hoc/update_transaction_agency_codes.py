...
def update_table(sess, agency_type, table, args):
    ...
    sess.execute(
        ...
        """.format(
            table_name=table, agency_type=agency_type, condition_sql=condition_sql, suffix=suffix
        )
    )
    sess.commit()
...
def print_report(initial, final, agency_type, is_updated=False):
    ...
    """
    logger.info(
        "{} codes {}: {} rows".format(agency_type.title(), "updated" if is_updated else "to update", initial - final)
    )


def main():
    parser = argparse.ArgumentParser(
        description="Update contract transaction and/or fabs rows based on updates to the " "agency list"
    )
    parser.add_argument(
        "-a",
        "--missing_agency",
        help="Perform an update on 999 agency codes",
        action="store_true",
        required=False,
        default=False,
    )
    parser.add_argument(
        "-s",
        "--subtier_codes",
        help="Select specific subtiers to update. Must be a 4-digit code",
        type=str,
        nargs="+",
        required=False,
    )
    parser.add_argument(
        "-t",
        "--tables",
        help="Which tables (fabs, fpds, or both) to update. Defaults to both.",
        required=False,
        default="both",
