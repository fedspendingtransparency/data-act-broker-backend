...
HD_COLUMNS = [
    col.key for col in HistoricDUNS.__table__.columns if col.key not in ("duns_id", "created_at", "updated_at")
]
...
def run_sam_batches(file, sess, block_size=LOAD_BATCH_SIZE):
    ...
    for sam_df in sam_dfs:
        ...
        if not recps_to_load.empty:
            ...
            logger.info(
                "Finished updating {} SAM rows in {} s".format(
                    len(recps_to_load.index), (datetime.now() - start).total_seconds()
                )
            )
...
def clean_historic_recipients(sess):
    ...
    new_recps = [
        x[0]
        for x in sess.query(SAMRecipient.awardee_or_recipient_uniqu)
        .filter(
            SAMRecipient.awardee_or_recipient_uniqu == HistoricDUNS.awardee_or_recipient_uniqu,
            SAMRecipient.historic.is_(False),
        )
        .all()
    ]
    if new_recps:
        logger.info(
            "Found {} new DUNS that were previously only available as a historic DUNS. Removing the historic"
            " records from the historic duns table.".format(len(new_recps))
        )
        sess.query(HistoricDUNS).filter(HistoricDUNS.awardee_or_recipient_uniqu.in_(new_recps)).delete(
...
def import_historic_recipients(sess):
    ...
    update_cols = [
        "{col} = hd.{col}".format(col=col)
        for col in HD_COLUMNS
    ...
    """.format(
        columns=", ".join(HD_COLUMNS), from_columns=", ".join(from_columns)
    )
...
def main():
    ...
    action.add_argument(
        "--reload_file", "-r", action="store_true", help="Reload HistoricDUNS table from file and" " update from SAM"
    )
    action.add_argument(
        "--update_from_sam",
        "-u",
        action="store_true",
        help="Update the current HistoricDUNS with any" "new columns or updated data",
    )
    ...
    if reload_file:
        ...
        if CONFIG_BROKER["use_aws"]:
            ...
            recps_file = s3_client.generate_presigned_url(
                "get_object",
                {"Bucket": CONFIG_BROKER["archive_bucket"], "Key": "DUNS_export_deduped.csv"},
                ExpiresIn=10000,
            )
        else:
