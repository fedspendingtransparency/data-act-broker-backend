...
if __name__ == "__main__":
    ...
    with create_app().app_context():
        ...
        sess.execute(
            """CREATE TEMP TABLE duplicated_fabs AS
        ...
        executed = sess.execute(
            """ DELETE FROM published_fabs AS pf
                                    WHERE is_active IS TRUE
                                        AND EXISTS (SELECT 1
                                            FROM duplicated_fabs AS df
                                            WHERE df.afa_generated_unique = UPPER(pf.afa_generated_unique)
        ...
        logger.info(
            "Deleted {} duplicate rows from published_fabs. Determining if any "
        ...
        logger.info(
            "The following submissions have been completely invalidated by the deletes, unpublishing them: "
        ...
        for sub in cleared_submissions:
            ...
            sess.query(Submission).filter_by(submission_id=sub).update(
