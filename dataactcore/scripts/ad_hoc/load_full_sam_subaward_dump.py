...
def get_full_dump_filepath(file_filters, file_type):
    ...
    if CONFIG_BROKER["use_aws"]:
        ...
        subaward_file = s3_client.generate_presigned_url(
            ClientMethod="get_object",
            Params={"Bucket": CONFIG_BROKER["data_extracts_bucket"], "Key": f"sam_subaward_bulk_dump/{filename}"},
            ExpiresIn=600,
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--file_type",
        help="Which file (assistance, contract, or both) to load." " Defaults to both.",
        required=False,
        default="both",
    ...
    parser.add_argument(
        "-r",
        "--reload",
        help="If provided, reloads the subaward table from scratch after" " loading the raw tables",
        required=False,
        action="store_true",
    )
