...
def update_assistance_listing(sess):
    ...
    for row in update_list:
        ...
        if len(current_assistance_listing.split(".")) < 2:
            logger.info(
                "assistance_listing_number %s is not a valid assistance_listing_number", current_assistance_listing
            )
        ...
        logger.info(
            "%s entries updated, %s entries remaining, %s percent complete", complete, count, (complete / total) * 100
        )
        logger.info("Assistance Listing %s took %s to complete", row.assistance_listing_number, end_time - start_time)
        logger.info(
            '%s entries with assistance_listing_number %s have been updated with title "%s"',
            assistance_listing_query,
