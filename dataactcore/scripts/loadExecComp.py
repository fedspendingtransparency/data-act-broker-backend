...
def parse_sam_file(file, sess):
    ...
    csv_data = pd.read_csv(zfile.open(csv_file), dtype=str, header=None, skiprows=1, nrows = nrows,
    ...
    parsed_data = pd.DataFrame(total_data["exec_comp_str"].apply(lambda ecs: pd.Series(parse_exec_comp(ecs).values())),
                               columns = list(parse_exec_comp().keys()))
    del total_data["exec_comp_str"]
    total_data = pd.concat([total_data, parsed_data], axis=1)
...
def parse_exec_comp(exec_comp_str = None):
    ...
    for index in range(1,6):
        exec_comp_data["high_comp_officer{}_full_na".format(index)] = ""
        exec_comp_data["high_comp_officer{}_amount".format(index)] = ""
