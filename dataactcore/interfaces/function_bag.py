...
def create_user_with_password(email, password, bcrypt, website_admin=False):
    ...
    user = User(email=email, name="Administrator", title="System Admin", website_admin=website_admin)
...
def populate_job_error_info(job):
    ...
    sess = GlobalDB.db().session
    job.number_of_errors = (
        sess.query(func.sum(ErrorMetadata.occurrences))
...
def get_error_type(job_id):
    ...
    sess = GlobalDB.db().session
    file_status_name = (
        sess.query(File).options(joinedload("file_status")).filter(File.job_id == job_id).one().file_status.name
    )
...
def get_error_metrics_by_job_id(job_id, include_file_types=False, severity_id=None):
    ...
    if not query_result.file_status.file_status_id == FILE_STATUS_DICT["complete"]:
        return [
            {
                "field_name": "File Level Error",
                "error_name": query_result.file_status.name,
                "error_description": query_result.file_status.description,
                "occurrences": 1,
                "rule_failed": "",
            }
        ]
    ...
    query_result = (
        ...
        .options(joinedload("error_type"))
        .filter(ErrorMetadata.job_id == job_id, ErrorMetadata.severity_id == severity_id)
        .all()
    )
    for result in query_result:
        record_dict = {
            ...
            "error_description": result.error_type.description,
            "occurrences": result.occurrences,
            "rule_failed": result.rule_failed,
            "original_label": result.original_rule_label,
        }
...
def get_email_template(email_type):
    ...
    sess = GlobalDB.db().session
    type_result = (
        sess.query(EmailTemplateType.email_template_type_id).filter(EmailTemplateType.name == email_type).one()
    )
    template_result = (
        sess.query(EmailTemplate).filter(EmailTemplate.template_type_id == type_result.email_template_type_id).one()
    )
    ...
    for dependency in dependencies:
        ...
        if dependency.dependent_job.job_status_id != JOB_STATUS_DICT["waiting"]:
            ...
            unfinished_prerequisites = (
                ...
                .filter(
                    or_(Job.job_status_id != JOB_STATUS_DICT["finished"], Job.number_of_errors > 0),
...
def create_jobs(upload_files, submission, existing_submission=False):
    ...
    if existing_submission and not submission.is_fabs:
        ...
        val_job = sess.query(Job).filter_by(submission_id=submission_id, job_type_id=JOB_TYPE_DICT["validation"]).one()
...
def add_jobs_for_uploaded_file(upload_file, submission_id, existing_submission):
    ...
    if existing_submission:
        ...
        upload_job = (
            sess.query(Job)
            .filter_by(submission_id=submission_id, file_type_id=file_type_id, job_type_id=JOB_TYPE_DICT["file_upload"])
            .one()
        )
    ...
    if existing_submission:
        ...
        sess.query(ErrorMetadata).filter(ErrorMetadata.job_id == val_job.job_id).delete(synchronize_session="fetch")
...
def get_latest_published_date(submission, is_fabs=False):
    if (
        ...
        and submission.publish_status_id != PUBLISH_STATUS_DICT["publishing"]
    ):
        sess = GlobalDB.db().session
        last_published = (
        ...
        if is_fabs:
            published_files = (
                ...
                .filter_by(publish_history_id=last_published.publish_history_id)
                .first()
            )
...
def get_certification_deadline(submission):
    ...
    if not submission.is_fabs:
        ...
        sub_window = sess.query(SubmissionWindowSchedule).filter_by(year=sub_year, period=sub_period).one_or_none()
...
def filename_fyp_sub_format(submission):
    ...
    """
    return filename_fyp_format(
        submission.reporting_fiscal_year, submission.reporting_fiscal_period, submission.is_quarter_format
    )
...
def get_last_validated_date(submission_id):
    ...
    jobs = sess.query(Job).filter(Job.submission_id == submission_id, Job.job_type_id.in_(validation_job_types)).all()
...
def get_action_dates(submission_id):
    ...
    return (
        sess.query(
            func.min(FABS.action_date).label("min_action_date"), func.max(FABS.action_date).label("max_action_date")
        )
...
def get_last_modified(submission_id):
    ...
    sess = GlobalDB.db().session
    last_modified = (
        sess.query(submission_updated_view.updated_at)
        .filter(submission_updated_view.submission_id == submission_id)
        .first()
    )
...
def update_external_data_load_date(start_time, end_time, data_type):
    ...
    last_stored_obj = (
        ...
        .filter_by(external_data_type_id=EXTERNAL_DATA_TYPE_DICT[data_type])
        .one_or_none()
    )
    if not last_stored_obj:
        last_stored_obj = ExternalDataLoadDate(
            external_data_type_id=EXTERNAL_DATA_TYPE_DICT[data_type],
            last_load_date_start=start_time,
            last_load_date_end=end_time,
        )
