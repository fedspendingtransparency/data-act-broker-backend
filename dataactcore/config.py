...
CONFIG_DB = {}
CONFIG_LOGGING = {}
CONFIG_CATEGORIES = {"broker": CONFIG_BROKER, "services": CONFIG_SERVICES, "db": CONFIG_DB, "logging": CONFIG_LOGGING}
...
for config_path in path_list:
    try:
        ...
        raise IOError(
            "Error reading a config file. Please make sure this file exists"
...
if CONFIG_BROKER["use_aws"] is True or CONFIG_BROKER["use_aws"] == "true":
    ...
    for k in required_aws_keys:
        try:
            ...
            raise KeyError(
                "Config error: use_aws is True, but the {} key is missing from the config.yml file".format(k)
            )
...
CONFIG_SERVICES["broker_api_host"] = re.sub("http://|:(.*)", "", CONFIG_SERVICES["broker_api_host"])
CONFIG_SERVICES["validator_host"] = re.sub("http://|:(.*)", "", CONFIG_SERVICES["validator_host"])
...
if log_message:
    ...
    _logger.addHandler(logging.FileHandler(os.path.join(CONFIG_LOGGING["log_files"], "info.log")))
