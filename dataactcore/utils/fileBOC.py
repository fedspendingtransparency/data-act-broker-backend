...
def query_data(session, agency_code, period, year):
    ...
    submission_query = session.query(Submission.submission_id).filter_by(
        reporting_fiscal_year=year, reporting_fiscal_period=period, publish_status_id=PUBLISH_STATUS_DICT["published"]
    )
    ...
    rows = initial_query(session, ussgl_pub_file_b.c, summed_gtas_boc, period, year).filter(
        or_(ussgl_pub_file_b.c.dollar_amount != 0, func.coalesce(summed_gtas_boc.c.sum_dollar_amount, 0) != 0)
    )
...
def sum_published_file_b(session, submission_id):
    ...
    summed_b = (
        session.query(
            ...
            func.sum(fileb_model.ussgl480110_rein_undel_ord_cpe).label("sum_ussgl480110_rein_undel_ord_cpe"),
            ...
            func.sum(fileb_model.ussgl490110_rein_deliv_ord_cpe).label("sum_ussgl490110_rein_deliv_ord_cpe"),
...
def ussgl_values(model):
    ...
    ussgl_list = [
        model.sum_ussgl480100_undelivered_or_cpe,
        model.sum_ussgl480100_undelivered_or_fyb,
        model.sum_ussgl480110_rein_undel_ord_cpe,
        model.sum_ussgl480200_undelivered_or_cpe,
        model.sum_ussgl480200_undelivered_or_fyb,
        model.sum_ussgl483100_undelivered_or_cpe,
        model.sum_ussgl483200_undelivered_or_cpe,
        model.sum_ussgl487100_downward_adjus_cpe,
        model.sum_ussgl487200_downward_adjus_cpe,
        model.sum_ussgl488100_upward_adjustm_cpe,
        model.sum_ussgl488200_upward_adjustm_cpe,
        model.sum_ussgl490100_delivered_orde_cpe,
        model.sum_ussgl490100_delivered_orde_fyb,
        model.sum_ussgl490110_rein_deliv_ord_cpe,
        model.sum_ussgl490200_delivered_orde_cpe,
        model.sum_ussgl490800_authority_outl_cpe,
        model.sum_ussgl490800_authority_outl_fyb,
        model.sum_ussgl493100_delivered_orde_cpe,
        model.sum_ussgl497100_downward_adjus_cpe,
        model.sum_ussgl497200_downward_adjus_cpe,
        model.sum_ussgl498100_upward_adjustm_cpe,
        model.sum_ussgl498200_upward_adjustm_cpe,
    ]
    val_data = []
...
def ussgl_published_file_b_cte(session, ussgl_vals, model):
    ...
    ussgl_pub_file_b = (
        ...
        .join(
            ussgl_vals,
            and_(
                ussgl_vals.c.display_tas == model.display_tas,
...
def sum_gtas_boc(session, period, year, agency_code):
    ...
    exists_query = session.query(TASLookup).filter(
        TASLookup.display_tas == boc_model.display_tas,
        func.coalesce(func.upper(TASLookup.financial_indicator2), "") != "F",
        or_(*agency_filters),
    )
    ...
    ussgl_list = list(
        set(
            [
                re.findall(r"ussgl(\d+)_.*", col.name)[0]
                for col in fileb_model.__table__.columns
                if col.name.startswith("ussgl")
            ]
        )
    )
    ...
    sum_boc = (
        session.query(
            ...
            func.sum(boc_model.dollar_amount * case([(boc_model.debit_credit == "D", 1)], else_=-1)).label(
                "sum_dollar_amount"
            ),
        )
        .filter(
...
def initial_query(session, model, summed_boc_model, period, year):
    ...
    return session.query(
        ...
        func.coalesce(model.allocation_transfer_agency, summed_boc_model.c.allocation_transfer_agency).label(
            "allocation_transfer_agency"
        ),
        func.coalesce(model.agency_identifier, summed_boc_model.c.agency_identifier).label("agency_identifier"),
        func.coalesce(model.beginning_period_of_availa, summed_boc_model.c.beginning_period_of_availa).label(
            "beginning_period_of_availa"
        ),
        func.coalesce(model.ending_period_of_availabil, summed_boc_model.c.ending_period_of_availabil).label(
            "ending_period_of_availabil"
        ),
        func.coalesce(model.availability_type_code, summed_boc_model.c.availability_type_code).label(
        ...
        func.coalesce(model.by_direct_reimbursable_fun, summed_boc_model.c.reimbursable_flag).label(
            "by_direct_reimbursable_fun"
        ),
        func.coalesce(model.disaster_emergency_fund_code, summed_boc_model.c.disaster_emergency_fund_code).label(
            "disaster_emergency_fund_code"
        ),
        func.coalesce(model.prior_year_adjustment, summed_boc_model.c.prior_year_adjustment).label(
        ...
        (func.coalesce(summed_boc_model.c.sum_dollar_amount, 0) - func.coalesce(model.dollar_amount, 0)).label(
            "dollar_amount_diff"
        ),
        ...
        summed_boc_model,
        and_(
            model.display_tas == summed_boc_model.c.display_tas,
