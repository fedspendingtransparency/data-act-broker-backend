...
SAM_IQAAS_MAPPINGS = {"ueiDUNS": "awardee_or_recipient_uniqu", "ueiSAM": "uei"}
EXCLUDE_FROM_API = [
    "registration_date",
    "expiration_date",
    "last_sam_mod_date",
...
def update_sam_recipient(
    sess, sam_recipient_data, table_name="sam_recipient", metrics=None, deletes=False, includes_uei=True
):
    ...
    if includes_uei:
        ...
        if (
            "ultimate_parent_unique_ide" in list(sam_recipient_data.columns)
    ...
    join_condition = " OR ".join(
        [
            "{table_name}.{key_col} = {tmp_abbr}.{key_col}".format(
                table_name=table_name, tmp_abbr=tmp_abbr, key_col=key_col
            )
            for key_col in key_cols
        ]
    )
    null_condition = " AND ".join(
        ["{table_name}.{key_col} IS NULL".format(table_name=table_name, key_col=key_col) for key_col in key_cols]
    )
    insert_cols = ", ".join(
        [
            (
                "{tmp_abbr}.{key_col}".format(tmp_abbr=tmp_abbr, key_col=key_col)
                if key_col in key_cols
                else 'NULL AS "{key_col}"'.format(key_col=key_col)
            )
    ...
    added_uei_list = [
        "{}/{}".format(row["awardee_or_recipient_uniqu"], row["uei"]) for row in sess.execute(insert_sql).fetchall()
    ]
    update_sql = """
    ...
    """.format(
        tmp_name=tmp_name, tmp_abbr=tmp_abbr, table_name=table_name, join_condition=join_condition
    )
    updated_uei_list = [
        "{}/{}".format(row["awardee_or_recipient_uniqu"], row["uei"]) for row in sess.execute(update_sql).fetchall()
    ]
    ...
    if len(added_uei_list) + len(updated_uei_list) != len(sam_recipient_data):
        raise ValueError(
            "Unable to add/update sam data. A record matched on more than one recipient: {}".format(updated_uei_list)
        )

    logger.info("Adding/updating recipients based on {}".format(tmp_name))
    if deletes:
        update_cols = [
            "{col} = {tmp_abbr}.{col}".format(col=col, tmp_abbr=tmp_abbr) for col in key_cols + ["deactivation_date"]
        ]
    else:
        update_cols = [
            "{col} = {tmp_abbr}.{col}".format(col=col, tmp_abbr=tmp_abbr)
    ...
    """.format(
        ...
        tmp_name=tmp_name,
        tmp_abbr=tmp_abbr,
        historic_col=insert_historic[0],
        historic_val=insert_historic[1],
        join_condition=join_condition,
    )
...
def parse_exec_comp_file(file_path, metrics=None):
    ...
    """
    if not metrics:
        metrics = {"files_processed": [], "records_received": 0, "records_processed": 0}
    ...
    v1_column_header_mapping = {"awardee_or_recipient_uniqu": 0, "sam_extract_code": 4, "exec_comp_str": 89}
    v2_column_header_mapping = {"uei": 0, "awardee_or_recipient_uniqu": 1, "sam_extract_code": 5, "exec_comp_str": 91}
    column_header_mapping = v1_column_header_mapping if version == "v1" else v2_column_header_mapping
    column_header_mapping_ordered = OrderedDict(sorted(column_header_mapping.items(), key=lambda c: c[1]))
    key_col = "awardee_or_recipient_uniqu" if version == "v1" else "uei"
    ...
    total_data = total_data[total_data[key_col].notnull() & total_data["sam_extract_code"].isin(["2", "3", "A", "E"])]
...
def update_missing_parent_names(sess, updated_date=None):
    ...
    distinct_parent_recipients = (
        sess.query(SAMRecipient.ultimate_parent_uei, SAMRecipient.ultimate_parent_legal_enti)
        .filter(
            and_(
    ...
    missing_parent_name = sess.query(SAMRecipient).filter(
        and_(
            func.coalesce(SAMRecipient.ultimate_parent_legal_enti, "") == "",
            SAMRecipient.ultimate_parent_uei.isnot(None),
        )
    ...
    while batch <= batches:
        ...
        logger.info(
            "Processing row {} - {} with missing parent recipient name".format(
                str(batch * block_size + 1), str(missing_count if batch == batches else (batch + 1) * block_size)
            )
        )

        missing_parent_name_block = missing_parent_name.order_by(SAMRecipient.sam_recipient_id).slice(
            batch_start, batch_start + block_size
        )
        ...
        logger.info(
            "Updated {} rows in {} with the parent name in {} s".format(
                updated_count, SAMRecipient.__name__, time.time() - start
            )
        )
...
def request_sam_iqaas_uei_api(filters):
    ...
    Returns:
        json list of SAM objects representing entities
    """
    params = {"api_key": CONFIG_BROKER["sam"]["api_key"]}
    if not filters:
        filters = {}
...
@sleep_and_retry
@on_exception(expo, RETRY_REQUEST_EXCEPTIONS, max_tries=11, logger=logger, giveup=give_up)
    ...
    for sam_obj in json.loads(request_api_method(filters).content)["entityData"]:
        ...
        for sam_prop_path, sam_props_name in sam_props_mappings.items():
            ...
            if sam_props_name == "business_types_codes":
                ...
                sam_props_dict["business_types"] = [
                    SAM_BUSINESS_TYPE_DICT[bus_type] for bus_type in value if bus_type in SAM_BUSINESS_TYPE_DICT
                ]
                ...
                if nested_obj:
                    for index, exec_comp in enumerate(nested_obj.get("listOfExecutiveCompensation", []), start=1):
                        if exec_comp["execName"] is not None:
                            ...
                            sam_props_dict["high_comp_officer{}_amount".format(index)] = exec_comp["compensationAmount"]
                continue
