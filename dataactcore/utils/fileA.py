...
def query_data(session, agency_code, period, year):
    ...
    rows = (
        initial_query(session, tas_gtas_fail.c, year)
        .filter(func.coalesce(tas_gtas_fail.c.financial_indicator2, "") != "F")
        .filter(or_(*agency_filters))
        .group_by(
            tas_gtas_fail.c.allocation_transfer_agency,
    ...
    if not submission_period:
        ...
        if not has_period:
            ...
            subquery = (
                session.query(
                    fail_model.severity,
                    fail_model.approved_override_exists,
                    fail_model.atb_submission_status,
                    fail_model.tas,
                    func.row_number()
                    .over(
                        fail_model.tas,
                ...
                .filter(
                    func.upper(fail_model.severity) == "FATAL",
            ...
            query = session.query(
                ...
                case(
                    [
                        ...
                        (
                            and_(
                                grouped_fail.c.atb_submission_status == "F",
                                func.upper(grouped_fail.c.severity) == "FATAL",
                        ...
                        (
                            and_(
                                grouped_fail.c.atb_submission_status == "E",
                                func.upper(grouped_fail.c.severity) == "FATAL",
                                grouped_fail.c.approved_override_exists.is_(True),
                            ),
                        ...
                        (
                            and_(
                                grouped_fail.c.atb_submission_status == "P",
                                func.upper(grouped_fail.c.severity) == "FATAL",
                                grouped_fail.c.approved_override_exists.is_(True),
                            ),
                        ...
                        (
                            and_(
                                grouped_fail.c.atb_submission_status == "C",
                                func.upper(grouped_fail.c.severity) == "FATAL",
                                grouped_fail.c.approved_override_exists.is_(True),
                            ),
                    ...
                    else_=literal_column("'passed required edits'"),
                ).label("gtas_status"),
...
def initial_query(session, model, year):
    ...
    return session.query(
        ...
        func.sum(case([(model.line.in_([1160, 1180, 1260, 1280]), model.amount)], else_=0)).label(
            "budget_authority_appropria_cpe"
        ),
        func.sum(case([(model.line == 1000, model.amount)], else_=0)).label("budget_authority_unobligat_fyb"),
        func.sum(
            case([(and_(model.line >= 1010, model.line <= budget_authority_line_max), model.amount)], else_=0)
        ).label("adjustments_to_unobligated_cpe"),
        func.sum(case([(model.line.in_([1540, 1640, 1340, 1440, 1750, 1850]), model.amount)], else_=0)).label(
