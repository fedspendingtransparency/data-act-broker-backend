...
class AbstractQueueError(Exception, metaclass=ABCMeta):
    ...
    def __init__(self, *args, worker_process_name=None, queue_message=None, **kwargs):
        ...
        if queue_message:
            default_message = (
                "Queue processing error with non-zero exit code for worker process"
                ' "{}" working on message: {}'.format(worker_process_name, queue_message)
            )
        else:
            default_message = (
                "Queue processing error with non-zero exit code for worker process"
                ' "{}". Queue Message None or not provided.'.format(worker_process_name)
            )
