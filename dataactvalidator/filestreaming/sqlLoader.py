        ...
        with create_app().app_context():
            ...
            sql_data = clean_data(
                ...
                RuleSql,
                {
                    ...
                    "rule_error_message": "rule_error_message",
                    "query_name": "query_name",
                    "expected_value": "expected_value",
                    "category": "category",
                    "file_type": "file_type",
                    "target_file": "target_file",
                    "rule_cross_file_flag": "rule_cross_file_flag",
            ...
            if sql_data["file_id"].isnull().values.any():
                raise Exception(
                    "Invalid file_type value found in sqlLoader. Must be one of the following: {}".format(
                        ", ".join(list(FILE_TYPE_DICT.keys()))
                    )
                )
            sql_data["target_file_id"] = sql_data["target_file"].apply(lambda type: FILE_TYPE_DICT.get(type, None))
            sql_data["rule_cross_file_flag"] = sql_data["rule_cross_file_flag"].apply(
                lambda flag: flag in ("true", "t", "y", "yes")
            )
            sql_data["rule_severity_id"] = sql_data["severity_name"].apply(
                lambda severity_name: RULE_SEVERITY_DICT.get(severity_name, None)
            )
            ...
            if sql_data["rule_severity_id"].isnull().values.any():
                raise Exception(
                    "Invalid severity_name value found in sqlLoader Must be one of the following: {}".format(
                        ", ".join(list(RULE_SEVERITY_DICT.keys()))
                    )
                )
            sql_data.drop(["file_type", "severity_name", "target_file"], axis=1, inplace=True)

            # Final check if we need to actually reload
            if check_dataframe_diff(
                sql_data,
                RuleSql,
                del_cols=["rule_sql_id", "created_at", "updated_at"],
