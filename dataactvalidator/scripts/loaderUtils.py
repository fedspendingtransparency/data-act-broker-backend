...
def padFunction(field, padTo, keepNull):
...
def cleanData(data, model, fieldMap, fieldOptions):
    ...
    for col, options in fieldOptions.items():
        if 'pad_to_length' in options:
            ...
            data[col] = data[col].apply(padFunction, args=(options['pad_to_length'], options.get('keep_null')))
