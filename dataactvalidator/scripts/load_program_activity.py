...
def get_stored_pa_last_upload():
    ...
    last_stored_obj = sess.query(ExternalDataLoadDate).join(ExternalDataType).filter(ExternalDataType.name=="program_activity_upload").one_or_none()  
    if not last_stored_obj:
        # return epoch ts to make sure we load the data the first time through, and ideally any time the data might have been wiped
...
def set_stored_pa_last_upload(load_datetime):
    ...
    last_stored_obj = sess.query(ExternalDataLoadDate).join(ExternalDataType).filter(ExternalDataType.name=="program_activity_upload").one_or_none()
    if not last_stored_obj:
        data_type = sess.query(ExternalDataType).filter(ExternalDataType.name=="program_activity_upload").one_or_none()
        last_stored_obj = ExternalDataLoadDate(external_data_type_id=data_type.external_data_type_id, last_load_date=make_date_tz_aware(load_datetime))
        ...
        try:
            ...
            if e.count == 0:
               log_blank_file()
                ...
                logger.error(LOG_ALERT_STRING + "Loading of program activity file failed due to exceeded failure threshold. " + count_str + LOG_ALERT_STRING)
            return False
...
def lowercase_or_notify(x):
    ...
    try:
        ...
        if x and not np.isnan(x):
            ...
            logger.info(" Null value found for program activity name. Entered default value.")
            return "(not provided)"
...
def log_blank_file():
    ...
    """
    logger.error(LOG_ALERT_STRING+" File was blank! Not loaded, routine aborted." +LOG_ALERT_STRING)
