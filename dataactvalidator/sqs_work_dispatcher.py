        ...
        if self._worker_can_start_child_processes:
            ...
            for spawn_of_worker in worker.children(recursive=True):
                if with_signal:
                    log_job_message(
                        logger=self._logger,
                        message="Attempting to terminate child process with PID {} and name {} using signal {}".format(
                            spawn_of_worker.pid, spawn_of_worker.name, with_signal
                        ),
                        is_warning=True,
                    )
                    try:
                    ...
                    log_job_message(
                        ...
                        message="Attempting to terminate child process with PID {} and name {}".format(
                            spawn_of_worker.pid, spawn_of_worker.name
                        ),
                        is_warning=True,
                    )
                    try:
