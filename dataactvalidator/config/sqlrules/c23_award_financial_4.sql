SELECT
  ...
  (SELECT SUM(sub_afa.federal_action_obligation::numeric) AS obligation_sum
		...
		WHERE submission_id = {0} AND sub_afa.uri = af.uri and COALESCE(sub_afa.assistance_type,"") in ('07','08')) AS federal_action_obligation_sum,
	(SELECT SUM(sub_afa.original_loan_subsidy_cost::numeric) AS obligation_sum
		FROM award_financial_assistance as sub_afa
		WHERE submission_id = {0} AND sub_afa.uri = af.uri and COALESCE(sub_afa.assistance_type,"") not in ('07','08')) AS original_loan_subsidy_cost_sum
...
HAVING (SELECT SUM(sub_af.transaction_obligated_amou) AS transaction_sum
		FROM award_financial as sub_af
		WHERE submission_id = {0} AND sub_af.uri = af.uri) <> (-1*(SELECT SUM(sub_afa.federal_action_obligation::numeric) AS obligation_sum
		FROM award_financial_assistance as sub_afa
		WHERE submission_id = {0} AND sub_afa.uri = af.uri and COALESCE(sub_afa.assistance_type,"") in ('07','08')) - (SELECT SUM(sub_afa.original_loan_subsidy_cost::numeric) AS obligation_sum
		FROM award_financial_assistance as sub_afa
