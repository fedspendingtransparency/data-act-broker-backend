...
HAVING
		(SELECT COALESCE(SUM(sub_af.transaction_obligated_amou::numeric),0) AS transaction_sum
		FROM award_financial_c23_3_{0} as sub_af WHERE submission_id = {0} AND sub_af.fain = af.fain) <>
		...
		AND NOT EXISTS (SELECT sub_af.allocation_transfer_agency FROM award_financial_c23_3_{0} as sub_af WHERE sub_af.fain = af.fain
