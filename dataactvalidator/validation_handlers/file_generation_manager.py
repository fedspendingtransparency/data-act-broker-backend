...
class FileGenerationManager:
    ...
    def generate_file(self, agency_code=None):
        ...
        if self.job and self.job.submission:
            ...
            if self.job.file_type.letter_name == "A":
                ...
                fillin_vals.update(
                    {"submission_id": self.job.submission_id, "FYP": filename_fyp_sub_format(self.job.submission)}
                )
        ...
        if self.file_generation:
            ...
            log_data.update(
                {
                    "agency_code": self.file_generation.agency_code,
                    "agency_type": self.file_generation.agency_type,
                    "start_date": self.file_generation.start_date,
                    "end_date": self.file_generation.end_date,
            ...
            e = (
                "No FileGeneration object for D file generation."
                if self.file_type in ["D1", "D2"]
                else "Cannot generate file for {} file type.".format(self.file_type if self.file_type else "empty")
            )
    ...
    def generate_d_file(self, file_path):
        ...
        log_data = {
            "message": "Starting file {} generation".format(self.file_type),
            "message_type": "ValidatorInfo",
            "agency_code": self.file_generation.agency_code,
            "agency_type": self.file_generation.agency_type,
            "start_date": self.file_generation.start_date,
        ...
        if self.file_type == "D1":
            ...
            raise ResponseError(
                "Failed to generate_d_file with file_type:{} (must be D1 or D2).".format(self.file_type)
            )
        headers = [val[header_index] for key, val in file_utils.mapping.items()]

        log_data["message"] = "Writing {} file {}: {}".format(
            self.file_type, self.file_generation.file_format.upper(), original_filename
        )
        logger.info(log_data)
        ...
        query_utils = {
            ...
            "agency_code": self.file_generation.agency_code,
            "agency_type": self.file_generation.agency_type,
            "start": self.file_generation.start_date,
        ...
        log_data["message"] = "Finished writing {} file {}: {}".format(
            self.file_type, self.file_generation.file_format.upper(), original_filename
        )
        logger.info(log_data)
    ...
    def generate_a_file(self, agency_code, file_path):
        ...
        log_data = {
            "message": "Starting file A generation",
            "message_type": "ValidatorInfo",
            "job_id": self.job.job_id,
            "agency_code": agency_code,
            "file_type": self.job.file_type.letter_name,
        ...
        # Generate the file and put in S3
        write_stream_query(
            self.sess, a_file_query(query_utils), local_file, self.job.filename, self.is_local, header=headers
        )
    ...
    def generate_boc_file(self, agency_code, file_path):
        ...
        log_data = {
            "message": "Starting BOC comparison file generation",
            "message_type": "ValidatorInfo",
            "job_id": self.job.job_id,
            "agency_code": agency_code,
            "file_type": self.job.file_type.letter_name,
        ...
        # Generate the file and put in S3
        write_stream_query(
            self.sess, boc_file_query(query_utils), local_file, self.job.filename, self.is_local, header=headers
        )
