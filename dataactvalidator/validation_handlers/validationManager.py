...
from dataactbroker.helpers.validation_helper import (
    derive_fabs_awarding_sub_tier,
    derive_fabs_afa_generated_unique,
    derive_fabs_unique_award_key,
    check_required,
    check_type,
    check_length,
    concat_flex,
    process_formatting_errors,
    parse_fields,
    simple_file_scan,
    check_field_format,
    clean_numbers_vectorized,
...
class ValidationManager:
    ...
    def run_validation(self, job):
        ...
        self.fields = (
            sess.query(FileColumn)
            .filter(FileColumn.file_id == FILE_TYPE_DICT[self.file_type.name])
    ...
    def load_file_data(self, sess, bucket_name, region_name):
        ...
        reader_obj = pd.read_csv(
            self.reader.file,
            dtype=str,
            delimiter=self.reader.delimiter,
            na_filter=False,
        ...
        if (
            ...
            record_row_error(
                ...
                self.job.job_id,
                self.file_name,
                empty_file["Field Name"],
                empty_file["error_type"],
                empty_file["Row Number"],
                empty_file["Rule Label"],
                self.file_type.file_type_id,
                None,
                RULE_SEVERITY_DICT["warning"],
            )
            empty_file_df.to_csv(
                self.warning_file_path,
                columns=self.report_headers,
                index=False,
                quoting=csv.QUOTE_ALL,
    ...
    def process_data_chunk(self, chunk_df, shared_data, file_row_count, sess=None, lockable=None):
        ...
        if self.file_type_name not in ["award", "award_procurement"]:
            ...
            req_errors = check_required(
                chunk_df,
                self.parsed_fields["required"],
                required_list,
                self.report_headers,
            ...
            type_errors = check_type(
                chunk_df,
                self.parsed_fields["number"] + self.parsed_fields["boolean"],
                type_list,
                self.report_headers,
                self.csv_schema,
                self.short_to_long_dict[self.file_type_id],
            ...
            length_errors = check_length(
                chunk_df,
                self.parsed_fields["length"],
                self.report_headers,
                self.csv_schema,
                self.short_to_long_dict[self.file_type_id],
                flex_data,
            ...
            field_format_errors = check_field_format(
                chunk_df,
                self.parsed_fields["format"],
                self.report_headers,
                self.short_to_long_dict[self.file_type_id],
                flex_data,
            )
            ...
            with lockable:
                for index, row in total_errors.iterrows():
                    shared_data["error_list"] = record_row_error(
                        shared_data["error_list"],
                        self.job_id,
                        self.file_name,
                        row["Field Name"],
                        row["error_type"],
    ...
    def run_sql_validations(self, short_colnames, writer, warning_writer):
        ...
        """
        for failure in validate_file_by_sql(
            self.job,
            self.file_type.name,
            self.short_to_long_dict[self.file_type.file_type_id],
    ...
    def _kill_spawned_processes(self):
        ...
        for spawn_of_job in job.children(recursive=True):
            logger.error(
                {
                    "message": "Attempting to terminate child process with PID: {} and name {}".format(
                        spawn_of_job.pid, spawn_of_job.name
                    ),
                    "message_type": "ValidatorInfo",
                    "submission_id": self.submission_id,
                    "job_id": self.job_id,
...
def update_account_nums(model_class, submission_id):
    ...
    """
    full_query = update_query.format(
        start=start_date,
        end=end_date,
        day_after_end=day_after_end,
        submission_id=submission_id,
        model=model_class.__table__.name,
    )
